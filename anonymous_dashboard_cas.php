...
if (isset($_POST['delete_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE anonymous_board_passers SET is_deleted = 1 WHERE id = ? AND department = 'Arts and Sciences'");
    $stmt->bind_param('i', $delete_id);
    $stmt->execute();
    $stmt->close();
    header("Location: anonymous_dashboard_cas.php");
    exit();
}
...
if (isset($_POST['edit_id']) && isset($_POST['edit_result'])) {
    ...
    if (in_array($edit_result, ['Passed', 'Failed', 'Conditional']) && in_array($edit_exam_type, ['First Timer', 'Repeater'])) {
        $stmt = $conn->prepare("UPDATE anonymous_board_passers SET result = ?, exam_type = ? WHERE id = ? AND department = 'Arts and Sciences'");
...
$passers = $conn->query("SELECT * FROM anonymous_board_passers WHERE department='Arts and Sciences' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY id ASC");
...
<head>
    ...
    <title>Anonymous Dashboard - Arts and Sciences</title>
