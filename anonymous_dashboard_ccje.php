...
if (isset($_POST['delete_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE anonymous_board_passers SET is_deleted = 1 WHERE id = ? AND department = 'Criminal Justice Education'");
    $stmt->bind_param('i', $delete_id);
    $stmt->execute();
    $stmt->close();
    header("Location: anonymous_dashboard_Criminal Justice Education.php");
    exit();
}
...
if (isset($_POST['edit_id']) && isset($_POST['edit_result'])) {
    ...
    if (in_array($edit_result, ['Passed', 'Failed', 'Conditional']) && in_array($edit_exam_type, ['First Timer', 'Repeater'])) {
        $stmt = $conn->prepare("UPDATE anonymous_board_passers SET result = ?, exam_type = ? WHERE id = ? AND department = 'Criminal Justice Education'");
...
$passers = $conn->query("SELECT * FROM anonymous_board_passers WHERE department='Criminal Justice Education' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY id ASC");
...
<head>
    ...
    <title>Anonymous Dashboard - Criminal Justice Education</title>
    ...
    <style>
        ...
        * {
        ...
        .stat-card.conditional .icon {
            background: linear-gradient(135deg, #FAD6A5 0%, #E63946 100%);
            color: #fff;
        }
        ...
        /* Logout Modal Styles - CCJE Red Theme */
        #logoutModal.modal {
        ...
        #logoutModal .modal-content {
            background: linear-gradient(135deg, #D32F2F 0%, #C62828 50%, #FAD6A5 100%) !important;
            padding: 48px 44px !important;
            border-radius: 28px !important;
            box-shadow: 0 32px 64px -12px rgba(211, 47, 47, 0.4) !important;
        ...
        #logoutModal .modal-icon {
            ...
            box-shadow: 0 20px 40px rgba(211, 47, 47, 0.4), 0 0 0 4px rgba(255, 255, 255, 0.8), 0 0 0 6px rgba(211, 47, 47, 0.2) !important;
        }
    ...
    <div id="logoutModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-buttons">
                <button class="modal-btn logout-confirm" onclick="executeLogout()">
                    ...
                    <i class="fas fa-check btn-check" style="display: none;"></i>
                </button>
                <button class="modal-btn logout-cancel" onclick="closeLogoutModal()">
                    <i class="fas fa-times-circle"></i>
                    <span>Cancel</span>
                </button>
            </div>
