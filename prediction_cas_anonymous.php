...
require_once 'check_and_start_cas_api.php';

function callPredictionAPI($endpoint, $method = 'GET', $data = null) {
    // Use port 5003 for CAS to avoid conflicts with Engineering (port 5000), CCJE (port 5001), and CBAA (port 5002)
    $url = "http://localhost:5003/api/" . $endpoint;
    $ch = curl_init();
...
<head>
    ...
    <title>AI Board Exam Predictions - CAS Anonymous Data</title>
    ...
    body::before {
        ...
        body .sidebar .logo, html body .sidebar .logo { 
            color: #4F0024 !important; 
            font-weight: 800;
        }
        body .sidebar-nav a, html body .sidebar-nav a { color: #830034 !important; }
        body .sidebar-nav i, html body .sidebar-nav i,
        body .sidebar-nav ion-icon, html body .sidebar-nav ion-icon { 
        ...
        body .sidebar-nav a.active i, body .sidebar-nav a.active ion-icon,
        body .sidebar-nav a:hover i, body .sidebar-nav a:hover ion-icon,
        html body .sidebar-nav a.active i, html body .sidebar-nav a.active ion-icon,
        html body .sidebar-nav a:hover i, html body .sidebar-nav a:hover ion-icon {
            color: #FFF0FC !important;
        }
        ...
        .prediction-card {
            ...
            padding: 32px;
            margin-bottom: 24px;
            box-shadow: 0 10px 40px rgba(199, 91, 155, 0.12);
            border: 2px solid rgba(199, 91, 155, 0.15);
        }
        ...
        .prediction-item {
            ...
            padding: 24px;
            border-radius: 16px;
            border-left: 4px solid #C75B9B;
            transition: all 0.3s;
        }
        
        .prediction-item:hover {
            transform: translateY(-4px);
            box-shadow: 0 12px 30px rgba(199, 91, 155, 0.3);
        }
        ...
        .ci-bar {
            height: 30px;
            background: linear-gradient(90deg, #FFB6C1, #C75B9B);
            border-radius: 8px;
            position: relative;
            margin: 10px 0;
        }
        ...
        .model-info-card {
            background: linear-gradient(135deg, #C75B9B 0%, #9f1239 100%);
            color: white;
            padding: 24px;
            border-radius: 16px;
            margin-top: 24px;
        ...
        .algorithm-item.best {
            background: linear-gradient(90deg, rgba(199, 91, 155, 0.1), rgba(255, 182, 193, 0.1));
            border-left-color: #C75B9B;
        }
        ...
        .btn:hover {
            ...
            box-shadow: 0 8px 25px rgba(199, 91, 155, 0.4);
        }
...
<body>
    ...
    <div class="topbar">
        <div class="dashboard-title">
            ...
            AI Board Exam Predictions - CAS Anonymous Data
        </div>
        ...
        <?php if ($predictionsData && $predictionsData['success']): ?>
            ...
            <div class="prediction-card">
                <h2>
                    ...
                    Passing Rate Predictions for <?php 
                        $year = date('Y') + 1;
                        if (isset($predictionsData['data']['predictions'][0]['predicted_year'])) {
                            $year = $predictionsData['data']['predictions'][0]['predicted_year'];
                ...
                <div class="btn-group">
                    ...
                    <button onclick="downloadTrainingReport()" class="btn" style="background: linear-gradient(135deg, #9f1239 0%, #C75B9B 100%);">
                    ...
                    <?php foreach ($predictionsData['data']['predictions'] as $pred): ?>
                        ...
                        <div class="prediction-item">
                            ...
                            <div class="rate-comparison">
                                ...
                                <div>
                                    <i class="fas fa-arrow-right" style="color: #C75B9B; font-size: 1.5rem;"></i>
                                </div>
            ...
            <?php if ($modelInfo && $modelInfo['success']): ?>
                <div class="model-info-card">
                    ...
                    <div class="model-metrics">
                        ...
                        <div class="metric">
                            ...
                            <div class="metric-value"><?php echo isset($modelInfo['data']['best_model_metrics']['test_mse']) ? number_format($modelInfo['data']['best_model_metrics']['test_mse'], 4) : 'N/A'; ?></div>
                        </div>
                        <div class="metric">
                            <div class="metric-label">RMSE</div>
                            <div class="metric-value"><?php echo isset($modelInfo['data']['best_model_metrics']['test_rmse']) ? number_format($modelInfo['data']['best_model_metrics']['test_rmse'], 2) . '%' : 'N/A'; ?></div>
                        </div>
                        <div class="metric">
                    ...
                    <div class="algorithm-comparison">
                        ...
                        <div class="algorithm-list">
                            <?php 
                            if (isset($modelInfo['data']['all_models']) && is_array($modelInfo['data']['all_models'])):
                                $allModels = $modelInfo['data']['all_models'];
                                usort($allModels, function($a, $b) {
                                    return ($b['test_r2'] ?? 0) <=> ($a['test_r2'] ?? 0);
                                });
                            ...
                            ?>
                                <div class="algorithm-item <?php echo $isBest ? 'best' : ''; ?>">
                                    <span class="algorithm-name">
                                        ...
                                        <?php endif; ?>
                                        <?php echo $model['model'] ?? 'Unknown'; ?>
                                    </span>
                                    <span class="algorithm-score">
                                        R²: <?php echo number_format($model['test_r2'] ?? 0, 4); ?> | 
                                        MAE: <?php echo number_format($model['test_mae'] ?? 0, 2); ?>% |
                                        MSE: <?php echo number_format($model['test_mse'] ?? 0, 4); ?>
                                    </span>
                                </div>
                            <?php 
                                endforeach;
                ...
                <div class="model-performance">
                    <h3 style="color: #9f1239; margin-bottom: 15px;">
                    ...
                    <div id="backtestResults" style="margin-top: 20px;">
                        <div style="text-align: center; padding: 20px;">
                            <i class="fas fa-spinner fa-spin" style="color: #C75B9B; font-size: 24px;"></i>
                ...
                <div class="prediction-card">
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #9f1239; margin: 0;">Model R² Score Comparison</h3>
                            <button onclick="showChartInfo('r2comparison')" style="background: linear-gradient(135deg, #E56D85 0%, #BF3853 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(229,109,133,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5003/api/graphs/model_comparison" alt="Model Comparison" style="max-width: 100%; height: auto;"
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #9f1239; margin: 0;">Performance Metrics</h3>
                            <button onclick="showChartInfo('performance')" style="background: linear-gradient(135deg, #E56D85 0%, #BF3853 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(229,109,133,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5003/api/graphs/accuracy_comparison" alt="Accuracy Comparison" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                            <i class="fas fa-chart-bar"></i>
                            <p>Graph not available. Please retrain the models.</p>
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #9f1239; margin: 0;">Actual vs Predicted</h3>
                            <button onclick="showChartInfo('actualvspredicted')" style="background: linear-gradient(135deg, #E56D85 0%, #BF3853 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(229,109,133,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5003/api/graphs/predictions_vs_actual" alt="Predictions vs Actual" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #9f1239; margin: 0;">Residual Analysis</h3>
                            <button onclick="showChartInfo('residuals')" style="background: linear-gradient(135deg, #E56D85 0%, #BF3853 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(229,109,133,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5003/api/graphs/residual_analysis" alt="Residual Analysis" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #9f1239; margin: 0;">Historical Trend</h3>
                            <button onclick="showChartInfo('historical')" style="background: linear-gradient(135deg, #E56D85 0%, #BF3853 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(229,109,133,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5003/api/graphs/historical_trends" alt="Historical Trends" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                            <i class="fas fa-chart-line"></i>
                            <p>Graph not available. Please retrain the models.</p>
        ...
        <?php else: ?>
            <div class="error-message">
                ...
                <ol style="margin-left: 20px; margin-top: 8px;">
                    ...
                    <li>Navigate to: <code>C:\laragon\www\SYSTEMNAERROR-3\FINALSYSTEMNAERROR\prediction_cas\</code></li>
                    <li>Run: <code>setup.bat</code> (first time only)</li>
                    <li>Run: <code>train.bat</code> to train the models on CAS anonymous data</li>
                    <li>Run: <code>start_api.bat</code> to start the API server on port 5003</li>
                    <li>Refresh this page</li>
                </ol>
                <p style="margin: 8px 0; padding: 12px; background: #fef3c7; border-left: 4px solid #f59e0b; border-radius: 8px;">
                    <strong>Note:</strong> This prediction system uses anonymous board passer data from CAS and runs on a separate port (5003) to avoid conflicts with the Engineering (port 5000), CCJE (port 5001), and CBAA (port 5002) prediction systems.
                </p>
            </div>
        <?php endif; ?>
    </div>
    ...
    <script>
        ...
        async function loadBacktestResults() {
            try {
                const response = await fetch('http://localhost:5003/api/backtest?test_year=2023&train_until=2022');
                const data = await response.json();
                
                if (data.success && data.data && data.data.predictions) {
                    const results = data.data;
                    
                    let html = '<div style="background: linear-gradient(135deg, #FFF0F5 0%, #FFE4EC 100%); border: 3px solid #C75B9B; border-radius: 16px; padding: 25px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(199, 91, 155, 0.15);">';
                    html += `<div style="text-align: center; margin-bottom: 20px;">`;
                    html += `<div style="display: inline-block; background: linear-gradient(135deg, #9f1239 0%, #C75B9B 100%); color: white; padding: 15px 30px; border-radius: 50px; margin-bottom: 15px; box-shadow: 0 4px 12px rgba(159, 18, 57, 0.3);">`;
                    ...
                    html += `</div>`;
                    html += `<div style="color: #9f1239; font-size: 16px; font-weight: 600; letter-spacing: 0.5px;">Average Prediction Accuracy</div>`;
                    html += `</div>`;
                    
                    html += `<div style="background: white; border-radius: 12px; padding: 20px; margin-top: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border: 2px solid #FFE4EC;">`;
                    html += `<table style="width: 100%; border-collapse: collapse;">`;
                    html += `<tr style="border-bottom: 3px solid #C75B9B; background: linear-gradient(to right, #9f1239, #C75B9B);">`;
                    ...
                    results.predictions.forEach(pred => {
                        ...
                        html += `<tr style="border-bottom: 1px solid #FFE4EC; transition: background 0.2s;" onmouseover="this.style.background='#FFF0F5'" onmouseout="this.style.background='white'">`;
                        html += `<td style="padding: 14px; color: #555; font-size: 13px;">`;
                        html += `<i class="fas fa-chart-line" style="color: #C75B9B; margin-right: 8px;"></i>`;
                        html += `${pred.exam_type}</td>`;
                        html += `<td style="padding: 14px; text-align: center; font-weight: 700; font-size: 16px; color: #9f1239;">${pred.actual.toFixed(2)}%</td>`;
                        html += `<td style="padding: 14px; text-align: center; font-weight: 700; font-size: 16px; color: #1565C0;">${pred.predicted.toFixed(2)}%</td>`;
                        html += `<td style="padding: 14px; text-align: center; font-weight: 600; font-size: 14px; color: #FF99CC;">${pred.error.toFixed(2)}%</td>`;
                        html += `</tr>`;
                    });
                    
                    // Summary row
                    html += `<tr style="background: linear-gradient(135deg, #FFF0F5 0%, #FFE8F0 100%); border-top: 2px solid #C75B9B;">`;
                    html += `<td style="padding: 14px; color: #9f1239; font-weight: 700; font-size: 14px;">`;
                    html += `<i class="fas fa-calculator" style="margin-right: 8px;"></i>Average Results</td>`;
                    html += `<td style="padding: 14px; text-align: center; font-weight: 700; font-size: 14px; color: #9f1239;">-</td>`;
                    ...
                    html += `<div style="margin-top: 20px; padding: 18px; background: linear-gradient(135deg, #FFF0F5 0%, #FFE8F0 100%); border-radius: 12px; border-left: 5px solid #FF99CC; box-shadow: 0 2px 8px rgba(255, 182, 193, 0.3);">`;
                    html += `<div style="display: flex; align-items: start; gap: 12px;">`;
                    html += `<div style="flex-shrink: 0; width: 40px; height: 40px; background: linear-gradient(135deg, #9f1239, #C75B9B); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 6px rgba(159, 18, 57, 0.3);">`;
                    html += `<i class="fas fa-lightbulb" style="color: #FFB6C1; font-size: 18px;"></i>`;
                    html += `</div>`;
                    html += `<div style="flex: 1;">`;
                    html += `<p style="margin: 0; color: #831843; font-size: 14px; line-height: 1.7; font-weight: 500;">`;
                    html += `<strong style="color: #9f1239; font-size: 15px;">What this means:</strong><br>`;
                    html += `Our AI model achieved <strong style="color: #9f1239;">${results.accuracy.toFixed(1)}%</strong> accuracy when predicting ${results.test_year} results using only ${results.trained_until} and earlier training data. `;
                    html += `The average prediction error was only <strong style="color: #FF99CC;">${results.mae.toFixed(2)}%</strong>. `;
                    html += `This validates that our machine learning model can reliably predict future CAS board exam passing rates.`;
                    html += `</p>`;
                    html += `</div>`;
                    html += `</div>`;
                    html += `</div>`;
                    ...
                    document.getElementById('backtestResults').innerHTML = 
                        `<div style="padding: 25px; text-align: center; color: #999; background: #FFF0F5; border-radius: 12px; border: 2px dashed #C75B9B;">
                            <i class="fas fa-database" style="font-size: 40px; color: #C75B9B; margin-bottom: 10px;"></i>
                            <p style="margin: 0; font-size: 14px;">${data.data.error}</p>
                            <p style="margin-top: 10px; font-size: 12px; color: #666;">Please ensure you have CAS anonymous data for 2022 and 2023 years.</p>
                        </div>`;
                } else {
                    document.getElementById('backtestResults').innerHTML = 
                        `<div style="padding: 25px; text-align: center; color: #999; background: #FFF0F5; border-radius: 12px; border: 2px dashed #C75B9B;">
                ...
                document.getElementById('backtestResults').innerHTML = 
                    `<div style="padding: 25px; text-align: center; color: #999; background: #FFF0F5; border-radius: 12px; border: 2px dashed #C75B9B;">
                        <i class="fas fa-exclamation-circle" style="font-size: 40px; color: #C75B9B; margin-bottom: 10px;"></i>
        ...
        async function exportToPDF() {
            ...
            try {
                ...
                const response = await fetch('http://localhost:5003/api/export/pdf', {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    a.download = `LSPU_CAS_Predictions_${year}_${new Date().getTime()}.pdf`;
        ...
        async function downloadTrainingReport() {
            ...
            try {
                const response = await fetch('http://localhost:5003/api/cas/export/training-report', {
                    method: 'GET'
                });
        ...
        async function retrainModel() {
            if (!confirm('This will retrain all 7 prediction algorithms with the latest CAS anonymous data. This may take a few minutes. Continue?')) {
                return;
            }
            ...
            try {
                const response = await fetch('http://localhost:5003/api/train', {
                    method: 'POST'
                });
        ...
        function showChartInfo(chartType) {
            const chartInfoData = {
                'r2comparison': {
                    ...
                    useCase: 'Use this to identify which AI algorithm is most reliable for CAS board exam predictions.',
                    icon: 'fa-chart-bar'
                },
                'performance': {
                ...
                'historical': {
                    ...
                    explanation: 'This line chart displays CAS board exam passing rates over multiple years, showing actual historical data and the trend line. This helps identify if performance is improving, declining, or staying stable over time.',
                    howToRead: [
            ...
            // Create modal with CAS pink/rose theme
            ...
            modal.innerHTML = `
                <div style="background: white; border-radius: 20px; padding: 40px; max-width: 650px; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 80px rgba(0,0,0,0.5); animation: slideUp 0.3s ease; position: relative;">
                    <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 24px; padding-bottom: 20px; border-bottom: 3px solid #E56D85;">
                        <div style="width: 60px; height: 60px; background: linear-gradient(135deg, #E56D85 0%, #BF3853 100%); border-radius: 15px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 12px rgba(229, 109, 133, 0.4);">
                    ...
                    <div style="margin-bottom: 24px; padding: 20px; background: linear-gradient(135deg, #FFF5F7 0%, #FFE8EC 100%); border-radius: 12px; border-left: 4px solid #E56D85;">
                        <h3 style="margin: 0 0 12px 0; color: #0f1724; font-size: 1.2rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-bullseye" style="color: #E56D85;"></i> Purpose
                        </h3>
                    ...
                    <div style="margin-bottom: 24px;">
                        <h3 style="margin: 0 0 12px 0; color: #0f1724; font-size: 1.1rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-align-left" style="color: #E56D85;"></i> What This Chart Shows
                        </h3>
                    ...
                    <div style="margin-bottom: 24px;">
                        <h3 style="margin: 0 0 16px 0; color: #0f1724; font-size: 1.1rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-book-reader" style="color: #E56D85;"></i> How to Read This Chart
                        </h3>
                        <ul style="margin: 0; padding-left: 0; list-style: none;">
                            ${info.howToRead.map(tip => `
                                <li style="margin-bottom: 10px; padding: 12px 16px; background: #f8fafc; border-radius: 8px; border-left: 3px solid #E56D85; color: #334155; font-size: 0.95rem;">
                                    <i class="fas fa-check-circle" style="color: #E56D85; margin-right: 8px;"></i>${tip}
                                </li>
                            `).join('')}
                        </ul>
                    </div>

                    <div style="padding: 20px; background: linear-gradient(135deg, #E56D85 0%, #BF3853 100%); border-radius: 12px; color: white;">
                    ...
                    <button onclick="this.closest('div[style*=fixed]').remove()" 
                        style="width: 100%; margin-top: 24px; padding: 14px; background: #E56D85; color: white; border: none; border-radius: 12px; font-weight: 700; cursor: pointer; font-size: 1.05rem; transition: all 0.3s;"
                        onmouseover="this.style.background='#BF3853'; this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(229,109,133,0.4)'"
                        onmouseout="this.style.background='#E56D85'; this.style.transform='translateY(0)'; this.style.boxShadow='none'">
