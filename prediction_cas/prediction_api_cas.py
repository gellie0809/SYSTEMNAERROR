"""
Flask API for CAS Board Exam Predictions
...
from advanced_predictor_cas import CASBoardExamPredictor
from training_report_generator_cas import CASTrainingReportGenerator
import os
import json
import math
...
def predict():
    ...
    try:
        ...
        df = predictor.fetch_cas_anonymous_data()
...
def export_predictions_pdf():
    ...
    try:
        ...
        # Custom styles with CAS pink colors
        ...
        elements.append(Paragraph("This report was generated by the LSPU CAS AI Board Exam Prediction System", footer_style))
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f'CAS_Predictions_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf'
        )
    ...
    try:
        ...
        if report_file and os.path.exists(report_file):
            return send_file(
                ...
                mimetype='application/pdf',
                as_attachment=True,
                download_name=f'CAS_Training_Report_{datetime.now().strftime("%Y%m%d")}.pdf'
            )
        else:
            return jsonify({
                'success': False,
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'service': 'CAS Board Exam Prediction API',
        'port': 5003,
        'timestamp': datetime.now().isoformat()
    })
...
if __name__ == '__main__':
    print("=" * 70)
    print("🎓 CAS Board Exam Prediction API")
    print("=" * 70)
    print("🌐 Starting server on http://localhost:5003")
    print("📊 Department: Arts and Sciences")
    ...
    print("   - GET  /api/cas/export/training-report - Download training report PDF")
