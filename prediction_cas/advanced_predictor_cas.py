"""
Advanced Board Exam Passing Rate Predictor for CAS Department
...
class CASBoardExamPredictor:
    ...
    def fetch_cas_anonymous_data(self):
        """Fetch CAS anonymous board passer data"""
        ...
        try:
            query = """
                SELECT 
                    ...
                    MAX(MONTH(board_exam_date)) as exam_month,
                    1 as exam_day
            ...
            print(f"✅ Fetched {len(df)} CAS anonymous records")
            return df
    ...
    def prepare_features(self, df):
        ...
        if 'exam_month' in df.columns and df['exam_month'].notna().any():
            ...
            df['exam_month_num'] = df['exam_month'].map(month_mapping).fillna(df['exam_month'])
        else:
            df['exam_month_num'] = 3  # Default to March (typical LET month)
        
        # Encode board exam type
        exam_types = df['board_exam_type'].unique()
        for exam_type in exam_types:
            col_name = f'is_{exam_type.replace(" ", "_").replace("(", "").replace(")", "").replace("-", "_")}'
            df[col_name] = (df['board_exam_type'] == exam_type).astype(int)
    ...
    def train_all_models(self):
        ...
        print("\n🤖 Starting CAS Board Exam Prediction Training...")
        print("=" * 70)
        
        # Fetch data
        df = self.fetch_cas_anonymous_data()
        ...
        # For small datasets (< 10 records), use all data for training
        # and evaluate using Leave-One-Out or simple metrics
        if len(df) < 10:
            print(f"⚠️ Small dataset detected ({len(df)} records). Using full data for training.")
            X_train = X
            y_train = y
            X_test = X  # Use same data for evaluation (training fit)
            y_test = y
        else:
        ...
        for name, model in self.algorithms.items():
            ...
            # Handle NaN R² (happens with very limited data)
            if np.isnan(test_r2) or np.isinf(test_r2):
                test_r2 = 0.0
            if np.isnan(train_r2) or np.isinf(train_r2):
                train_r2 = 0.0
            if np.isnan(cv_mean) or np.isinf(cv_mean):
                cv_mean = 0.0
            ...
            # Track best model (use MAE if R² is not meaningful)
            if test_r2 > best_r2 or (best_r2 <= 0 and test_mae < getattr(self, '_best_mae', float('inf'))):
                best_r2 = max(test_r2, 0)
                self.best_model_name = name
                self._best_mae = test_mae
    ...
    def save_models(self):
        ...
        metadata = {
            ...
            'training_records': len(self.training_data) if hasattr(self, 'training_data') and self.training_data is not None else 0,
    ...
    def generate_visualizations(self, X_test, y_test):
        ...
        # Ensure we have a best model selected (fallback to first model if none)
        ...
        # CAS Pink theme colors
        primary_color = '#FF99CC'
        secondary_color = '#C75B9B'
        dark_color = '#9f1239'
        ...
        bars = ax.barh(models, r2_scores, color=colors, edgecolor=dark_color, linewidth=2)
        ax.set_xlabel('R² Score', fontsize=12, fontweight='bold')
        ax.set_title('CAS Model Comparison - R² Scores', fontsize=14, fontweight='bold', pad=20)
        ax.set_xlim(0, 1.1)
        ...
        for i, (model, score, orig) in enumerate(zip(models, r2_scores, original_r2)):
            ...
            color = '#2E7D32' if orig > 0.8 else '#E08600' if orig > 0 else '#D32F2F'
        ...
        bars = ax.bar(models, accuracies, color=secondary_color, edgecolor=dark_color, linewidth=2)
        ax.set_ylabel('Accuracy (%)', fontsize=12, fontweight='bold')
        ax.set_title('CAS Model Accuracy Comparison', fontsize=14, fontweight='bold', pad=20)
        ax.set_ylim(0, 100)
        ...
        bars = ax.bar(models, mae_scores, color=primary_color, edgecolor=dark_color, linewidth=2)
        ax.set_ylabel('Mean Absolute Error (%)', fontsize=12, fontweight='bold')
        ax.set_title('CAS Model Error Comparison (Lower is Better)', fontsize=14, fontweight='bold', pad=20)
        plt.xticks(rotation=45, ha='right')
        ...
        ax.scatter(y_test, y_pred, alpha=0.6, s=100, color=secondary_color, edgecolors=dark_color, linewidth=2)
        ...
        ax.set_title(f'CAS Predictions vs Actual - {self.best_model_name}', 
        ...
        axes[0].hist(residuals, bins=15, color=secondary_color, edgecolor=dark_color, linewidth=2, alpha=0.7)
        ...
        axes[1].scatter(y_pred, residuals, alpha=0.6, s=100, color=primary_color, edgecolors=dark_color, linewidth=2)
        ...
        plt.suptitle(f'CAS Residual Analysis - {self.best_model_name}', fontsize=14, fontweight='bold', y=1.02)
        plt.tight_layout()
        ...
        if self.training_data is not None and len(self.training_data) > 0:
            ...
            colors_list = ['#FF99CC', '#C75B9B', '#ec4899', '#9f1239', '#be185d', '#831843', '#500724']
            
            for idx, exam_type in enumerate(self.training_data['board_exam_type'].unique()):
                data = self.training_data[self.training_data['board_exam_type'] == exam_type]
                color = colors_list[idx % len(colors_list)]
                ax.plot(data['exam_year'], data['passing_rate'], 
                       marker='o', linewidth=2, markersize=8, label=exam_type, color=color)
            
            ax.set_xlabel('Year', fontsize=12, fontweight='bold')
            ax.set_ylabel('Passing Rate (%)', fontsize=12, fontweight='bold')
            ax.set_title('CAS Historical Passing Rate Trends', fontsize=14, fontweight='bold', pad=20)
    ...
    def predict_next_year(self):
        ...
        for exam_type in df['board_exam_type'].unique():
            ...
            available_features = [f for f in features if f in exam_data.columns]
            next_year_features = exam_data[available_features].copy()
            
            if 'year_numeric' in next_year_features.columns:
                next_year_features['year_numeric'] = next_year
            
            # Add missing features with default values
            for f in features:
                if f not in next_year_features.columns:
                    next_year_features[f] = 0
            
            # Ensure correct column order
            next_year_features = next_year_features[features]
    ...
    def backtest(self, test_year=2023, train_until_year=2022):
        ...
        if len(train_df) == 0 or len(test_df) == 0:
            return {'error': 'Insufficient data for backtesting', 'train_count': len(train_df), 'test_count': len(test_df)}
...
if __name__ == "__main__":
    predictor = CASBoardExamPredictor()
