...
// Allow College of Engineering admin or ICTS admin
...
$passers = $conn->query("SELECT * FROM anonymous_board_passers WHERE department='Engineering' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY id DESC");
...
// Fetch courses for dropdown (exclude deleted if column exists)
$check_courses_col = $conn->query("SHOW COLUMNS FROM courses LIKE 'is_deleted'");
if ($check_courses_col && $check_courses_col->num_rows > 0) {
    $courses_result = $conn->query("SELECT course_name FROM courses WHERE department='Engineering' AND is_deleted = 0 ORDER BY course_name ASC");
} else {
...
// Fetch board exam types for dropdown (include id for client-side filtering, exclude deleted if column exists)
$check_bet_col = $conn->query("SHOW COLUMNS FROM board_exam_types LIKE 'is_deleted'");
if ($check_bet_col && $check_bet_col->num_rows > 0) {
    $board_exam_types_result = $conn->query("SELECT id, exam_type_name FROM board_exam_types WHERE department='Engineering' AND is_deleted = 0 ORDER BY exam_type_name ASC");
} else {
...
// Fetch board exam dates for dropdown (include associated board exam type name for client-side filtering, exclude deleted if column exists)
$check_bed_col = $conn->query("SHOW COLUMNS FROM board_exam_dates LIKE 'is_deleted'");
if ($check_bed_col && $check_bed_col->num_rows > 0) {
    $board_exam_dates_result = $conn->query(
      ...
      . "WHERE bed.department='Engineering' AND bed.is_deleted = 0 AND (bet.is_deleted = 0 OR bet.is_deleted IS NULL) ORDER BY bed.exam_date DESC"
    );
} else {
    $board_exam_dates_result = $conn->query(
      "SELECT bed.id, bed.exam_date, bed.exam_description, bed.exam_type_id, IFNULL(bet.exam_type_name, '') AS exam_type_name \n"
...
<head>
    ...
    <title>CTE - BOARD PASSING RATE SYSTEM</title>
    ...
    * {
    ...
    .shortcuts-btn {
        background: rgba(139, 164, 154, 0.08);
        color: #3B6255;
        /* COE dark green text */
        border: 2px solid rgba(139, 164, 154, 0.35);
        border-radius: 12px;
        padding: 10px 16px;
        font-size: 0.9rem;
        font-weight: 600;
    ...
    .shortcuts-btn:hover {
        background: rgba(139, 164, 154, 0.15);
        border-color: rgba(139, 164, 154, 0.55);
        transform: translateY(-2px) scale(1.02);
        box-shadow: 0 8px 25px rgba(59, 98, 85, 0.2);
    }
    ...
    .guide-close-btn:hover {
        ...
        box-shadow: 0 8px 25px rgba(59, 98, 85, 0.35);
    }
    ...
    .card h2 {
        ...
        /* COE Earthy Harmony gradient */
        padding: 20px 32px;
        border-radius: 12px;
        box-shadow: 0 4px 12px rgba(59, 98, 85, 0.3);
        text-align: center;
    ...
    .board-table th {
        ...
        top: 0;
        z-index: 10;
        box-shadow: 0 2px 8px rgba(145, 179, 142, 0.25);
        border-bottom: 2px solid rgba(90, 133, 95, 0.25);
    }
    ...
    .board-table tbody tr:hover {
        background: linear-gradient(90deg, #d3ecdc 0%, #CBDED3 100%);
        /* Light cyan gradient */
        transform: translateY(-1px);
        box-shadow: 0 4px 16px rgba(90, 133, 95, 0.2);
    }
    ...
    .exam-first-timer {
        ...
        color: #ffffff;
        border-color: #3B6255;
        box-shadow: 0 2px 8px rgba(90, 133, 95, 0.3);
        text-align: center;
    }
    ...
    /* Colors - Professional Cyan Theme */
    .edit-btn {
        background: #8BA49A !important;
        /* Cyan */
        color: white !important;
    }

    .edit-btn:hover {
        background: #3B6255 !important;
        /* Dark Cyan */
        box-shadow: 0 6px 20px rgba(145, 179, 142, 0.4) !important;
    }
    ...
    .no-records {
        text-align: center;
        padding: 80px 40px;
        background: linear-gradient(135deg, #f0fdfa 0%, #E2DFDA 100%);
    ...
    .stat-card::before {
        ...
        right: 0;
        height: 4px;
        background: linear-gradient(90deg, #8BA49A, #7a9d77, #3B6255, #D2C49E);
        opacity: 0;
        transition: opacity 0.3s ease;
    }
    ...
    .filter-section {
        background: linear-gradient(135deg, #E2DFDA 0%, #d3ecdc 100%);
        border: 2px solid #8BA49A;
        border-radius: 16px;
        margin-bottom: 24px;
        overflow: visible;
        box-shadow: 0 4px 16px rgba(145, 179, 142, 0.15);
        position: relative;
        z-index: 10;
    }
    ...
    .filter-header {
        ...
        background: linear-gradient(135deg, #8BA49A 0%, #3B6255 100%);
        border-bottom: 1px solid #3B6255;
        border-radius: 16px;
    }
    ...
    .toggle-btn {
        ...
        gap: 8px;
        transition: all 0.3s ease;
        box-shadow: 0 2px 8px rgba(145, 179, 142, 0.2);
    }
    ...
    .filter-input:focus {
        border-color: #8BA49A;
        box-shadow: 0 0 0 3px rgba(145, 179, 142, 0.25);
    }
    ...
    .search-input:focus {
        border-color: #8BA49A;
        box-shadow: 0 0 0 4px rgba(145, 179, 142, 0.18);
        transform: translateY(-1px);
    }
    ...
    .export-btn {
        background: linear-gradient(135deg, #7a9d77 0%, #3B6255 100%);
        color: white;
        box-shadow: 0 4px 12px rgba(122, 157, 119, 0.2);
        position: relative !important;
    ...
    .export-btn:hover {
        background: linear-gradient(135deg, #3B6255 0%, #3B6255 100%);
        transform: translateY(-1px);
        box-shadow: 0 6px 16px rgba(122, 157, 119, 0.3);
    }
    ...
    .filter-count {
        margin-left: auto;
        padding: 8px 16px;
        background: linear-gradient(135deg, #d3ecdc 0%, #D2C49E 100%);
        border: 1px solid #8BA49A;
        border-radius: 8px;
        font-size: 0.9rem;
        color: #3B6255;
    ...
    #editStudentModal .modal-header {
        background: linear-gradient(135deg, #0ea5b1 0%, #8BA49A 45%, #60a5fa 100%);
        color: #fff;
    }
    ...
    #editStudentModal .tab-header .tab-icon {
        background: linear-gradient(135deg, #8BA49A 0%, #3b82f6 100%);
    ...
    #editStudentModal .btn-primary {
        background: linear-gradient(135deg, #8BA49A 0%, #0ea5b1 100%);
        color: #fff;
    ...
    #exportModal .excel-btn {
        background: linear-gradient(135deg, #8BA49A 0%, #3B6255 100%) !important;
        color: white !important;
        box-shadow: 0 4px 15px rgba(145, 179, 142, 0.3) !important;
    }

    #exportModal .excel-btn:hover {
        background: linear-gradient(135deg, #3B6255 0%, #3B6255 100%) !important;
        box-shadow: 0 8px 25px rgba(145, 179, 142, 0.4) !important;
    }

    #exportModal .csv-btn {
        background: linear-gradient(135deg, #7a9d77 0%, #3B6255 100%) !important;
        color: white !important;
        box-shadow: 0 4px 15px rgba(122, 157, 119, 0.3) !important;
    }

    #exportModal .csv-btn:hover {
        background: linear-gradient(135deg, #3B6255 0%, #3B6255 100%) !important;
        box-shadow: 0 8px 25px rgba(122, 157, 119, 0.4) !important;
    }
    ...
    #logoutModal .modal-icon {
        ...
        overflow: visible !important;
        background: linear-gradient(135deg, #8BA49A 0%, #3B6255 100%) !important;
        box-shadow: 0 15px 35px rgba(90, 133, 95, 0.3) !important;
    }
    ...
    #logoutModal .modal-btn.logout-confirm {
        ...
        overflow: hidden !important;
        background: linear-gradient(135deg, #8BA49A 0%, #3B6255 100%) !important;
        box-shadow: 0 8px 25px rgba(145, 179, 142, 0.3) !important;
    }
    ...
    .export-option-card {
        ...
        position: relative;
        overflow: hidden;
        box-shadow: 0 4px 12px rgba(145, 179, 142, 0.06);
    }
    ...
    .export-option-card::before {
        ...
        width: 100%;
        height: 100%;
        background: linear-gradient(90deg, transparent, rgba(145, 179, 142, 0.08), transparent);
        transition: left 0.6s ease;
    }
    ...
    .export-option-card:hover {
        transform: translateY(-3px) scale(1.02);
        border-color: #8BA49A;
        box-shadow: 0 12px 30px rgba(145, 179, 142, 0.2);
...
<body>
    ...
    <div class="topbar">
        ...
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <a href="dashboard_icts.php?dept=Engineering" class="icts-back-btn" title="Back to ICTS Dashboard">
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #8BA49A 0%, #3B6255 100%);">
          ...
          ?>
            ...
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #3B6255 0%, #2A4A3E 100%);">
          ...
          $passing_rate_query = $conn->query("SELECT 
            ...
            FROM anonymous_board_passers WHERE department='Engineering' AND (is_deleted IS NULL OR is_deleted = 0)");
          ...
          ?>
            ...
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #CBDED3 0%, #8BA49A 100%);">
                    <i class="fas fa-medal"></i>
                </div>
                <div class="stat-content">
                    <?php
          // Get best course with highest passing rate (exclude soft-deleted records)
    ...
    */
      ...
      `;
        ...
        ">
          ...
          <div class="confirm-modal-dark-bg" style="
            background: linear-gradient(135deg, #0ea5b1 0%, #8BA49A 100%);
          ...
          ">
            <div style="background: #f7fafc; padding: 24px; border-radius: 16px; border: 1px solid #e2e8f0; margin-bottom: 24px;">
              <h4 style="color: #2d3748; margin: 0 0 16px 0; font-weight: 700; font-size: 1.1rem;">
                <i class="fas fa-user-graduate" style="margin-right: 8px; color: #8BA49A;"></i>
              ...
              <div style="color: #0ea5b1; margin-bottom: 16px; font-weight: 800; border-top: 1px solid #e2e8f0; padding-top: 16px; letter-spacing: .2px;">
                <i class="fas fa-clipboard-list" style="margin-right: 8px; color: #8BA49A;"></i>
                Summary of Changes:
              </div>
        ...
        Object.keys(originalData).forEach(key => {
            ...
            ">
              ...
              <div style="display: flex; align-items: center; gap: 10px; font-size: 0.92rem;">
                ...
                <i class="fas fa-arrow-right" style="color: #8BA49A; font-size: 0.75rem;"></i>
        ...
        ">
              ...
              <div style="display: flex; align-items: flex-start; color: #92400e;">
                ...
                <div>
                  ...
                  <div style="line-height: 1.5;">
                    This will permanently delete the student's record from the database. This action cannot be reversed and all associated data will be lost.
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style="background: #fee2e2; border: 2px solid #ef4444; border-radius: 12px; padding: 20px;">
              <div style="display: flex; align-items: flex-start; color: #dc2626;">
                ...
                <div>
                  ...
                  <div style="line-height: 1.5;">
                    You are about to permanently remove this student's academic record. Make sure this is the correct student before proceeding. Consider backing up data if needed.
                  </div>
                </div>
              </div>
            </div>
          ...
          ">
            ...
            <button id="cancelDeleteBtn" style="
              ...
              min-height: 50px;
            " onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 20px rgba(145, 179, 142, 0.4)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px rgba(145, 179, 142, 0.3)'">
    ...
    function showFilterMessage(message) {
        ...
        <div style="
          ...
          padding: 16px 24px;
          border-radius: 12px;
          box-shadow: 0 8px 25px rgba(145, 179, 142, 0.3);
          z-index: 10000;
          font-family: Inter;
          font-weight: 600;
          text-align: center;
  ...
  <div class="footer">
    ...
    <div id="logoutModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(15,23,42,0.85); backdrop-filter:blur(10px); -webkit-backdrop-filter:blur(10px); z-index:999999; justify-content:center; align-items:center;">
        <div style="background:linear-gradient(145deg,#1e293b,#0f172a); border-radius:20px; padding:40px; max-width:420px; width:90%; box-shadow:0 25px 60px rgba(0,0,0,0.5); text-align:center; border:1px solid rgba(255,255,255,0.1);">
            <div style="width:80px; height:80px; background:linear-gradient(135deg,#5B7B5A,#4A6349); border-radius:50%; margin:0 auto 20px; display:flex; align-items:center; justify-content:center;">
            ...
            <p style="color:#64748b; font-size:13px; margin-bottom:30px; line-height:1.6;">
                You are about to log out from the Engineering Admin Dashboard. Any unsaved changes will be lost.
            </p>
            <div style="display:flex; gap:15px; justify-content:center;">
                ...
                <button onclick="window.location.href='logout.php';" style="padding:12px 30px; border:none; border-radius:10px; font-size:15px; font-weight:500; cursor:pointer; background:linear-gradient(135deg,#5B7B5A,#4A6349); color:#fff; display:flex; align-items:center; gap:8px; transition:transform 0.2s;">
