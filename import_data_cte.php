...
/* -------------------------------------------------
   1. SECURITY – only CTE admin can access
   ------------------------------------------------- */
...
/* -------------------------------------------------
   2. ENSURE TABLE EXISTS (shared table – works for all colleges)
   ------------------------------------------------- */
function ensureDatabaseExists($conn) {
    ...
    if (!$tableCheck || $tableCheck->num_rows == 0) {
        ...
        CREATE TABLE board_passers (
            ...
            department VARCHAR(100) NOT NULL DEFAULT 'Teacher Education',
            exam_type VARCHAR(50) DEFAULT NULL,           -- First Timer / Repeater
            board_exam_type VARCHAR(255) DEFAULT NULL,    -- LET - Elementary / Secondary
            rating DECIMAL(5,2) DEFAULT NULL,             -- General Average (for LET)
...
/* -------------------------------------------------
   3. CSV PROCESSING – CTE / LET VERSION
   ------------------------------------------------- */
function processCSVImport($filePath, $conn) {
    ...
    try {
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            // Support both with/without header
            ...
            while (($data = fgetcsv($handle)) !== FALSE) {
                ...
                // Allow flexible column count (minimum 7)
                if (count($data) < 7) {
                    $errors[] = "Row $rowNumber: Insufficient columns (need at least 7)";
                    $stats['errors']++;
                    continue;
                }

                // Flexible column mapping – supports different CSV layouts
                $firstName     = trim($data[0] ?? '');
                $lastName      = trim($data[1] ?? '');
                $middleName    = trim($data[2] ?? '');
                $suffix        = trim($data[3] ?? '');
                $course        = trim($data[4] ?? '');
                $boardExamDate = trim($data[5] ?? '');
                $sex           = strtoupper(trim($data[6] ?? ''));
                $examType      = trim($data[7] ?? 'First Timer');      // First Timer / Repeater
                $letLevel      = trim($data[8] ?? '');                 // Elementary / Secondary
                $rating        = trim($data[9] ?? '');                 // General Average (optional)
                ...
                // Build full name (LSPU format: Last, First Middle Suffix)
                $fullNameParts = [$lastName . ','];
                if (!empty($firstName)) $fullNameParts[] = $firstName;
                ...
                if (!$dateObj) $dateObj = DateTime::createFromFormat('F j, Y', $boardExamDate);
                if (!$dateObj) {
                    $errors[] = "Row $rowNumber: Invalid date '$boardExamDate'";
                    $stats['errors']++;
                    continue;
                }
                ...
                if ($year < 2019 || $year > 2025) {
                    $errors[] = "Row $rowNumber: LET year must be 2019–2025 (got $year)";
                    $stats['errors']++;
                    continue;
                }
                ...
                if (!in_array($sex, ['MALE','FEMALE','M','F'])) {
                    $errors[] = "Row $rowNumber: Invalid sex '$sex'";
                    $stats['errors']++;
                    continue;
                }
                ...
                $normalizedLevel = ucwords(strtolower($letLevel));
                if (!empty($letLevel) && !in_array($normalizedLevel, array_map('ucwords', array_map('strtolower', $validLetLevels)))) {
                    $errors[] = "Row $rowNumber: Invalid LET Level '$letLevel'";
                    $stats['errors']++;
                    continue;
                }
                $boardExamType = $normalizedLevel ?: 'Licensure Examination for Teachers (LET)';

                // Rating (optional)
                $ratingValue = null;
                if (!empty($rating) && is_numeric($rating)) {
                    $ratingValue = round((float)$rating, 2);
                    if ($ratingValue < 0 || $ratingValue > 100) $ratingValue = null;
                }
                ...
                if (isset($processedRecords[$recordKey])) {
                    $errors[] = "Row $rowNumber: Duplicate in CSV – $fullName";
                    $stats['duplicates']++;
                    continue;
                }
                ...
                $checkStmt = $conn->prepare(
                    "SELECT id FROM board_passers 
                     WHERE name = ? AND course = ? AND board_exam_date = ? AND department = 'Teacher Education'"
                );
                $checkStmt->bind_param("sss", $fullName, $course, $boardExamDate);
                $checkStmt->execute();
                $res = $checkStmt->get_result();
                if ($res->num_rows > 0) {
                    $errors[] = "Row $rowNumber: Already exists – $fullName";
                    $stats['duplicates']++;
                    $checkStmt->close();
                    continue;
                }
                ...
                $insertStmt = $conn->prepare("
                    ...
                    (name, first_name, last_name, middle_name, suffix, sex, course, year_graduated,
                     board_exam_date, result, department, exam_type, board_exam_type, rating)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Passed', 'Teacher Education', ?, ?, ?)
                ");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES['csvFile'])) {
    ...
    if ($file['error'] === 0) {
        ...
        if ($ext !== 'csv') {
        ...
        } elseif ($file['size'] > 15_000_000) { // 15MB max
            $error = "File too large (max 15 MB).";
        } else {
            $uploadDir = 'uploads/';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);

            $newName = 'import_let_cte_' . date('Ymd_His') . '.csv';
            ...
            if (move_uploaded_file($file['tmp_name'], $dest)) {
                ...
                // Count total LET records
                $totalQ = $conn->query("SELECT COUNT(*) AS cnt FROM board_passers WHERE department = 'Teacher Education'");
                $totalRecords = $totalQ ? $totalQ->fetch_assoc()['cnt'] : 0;

                if ($stats['imported'] > 0) {
                    $message = "<strong>Success!</strong> Imported <strong>{$stats['imported']}</strong> out of {$stats['total']} LET records.<br>
                                Total LET Passers in database: <strong>$totalRecords</strong>";
                } else {
                    $error = "No records were imported. Check the CSV format and error list below.";
                }
...
<head>
    ...
    <title>Import Data - CTE Dashboard</title>
...
<body>
    <?php include './includes/cte_nav.php'; ?>
        ...
        <div class="card">
            <div class="card-header">
                <div class="header-content">
                    ...
                    <div>
                        <h1 class="header-title">Import Board Passers – College of Teacher Education</h1>
                        <p class="header-subtitle">Upload CSV files (2019-2024) for College of Teacher Education passers
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-content">
                ...
                <div class="template-section">
                    ...
                    <p class="template-description">Use this format for College of Teacher Education board passer data.
                    </p>
    ...
    function downloadTemplate(event) {
        ...
        a.href = url;
        a.download = 'cte_board_passers_template.csv';
