"""
Flask API for CTE Board Exam Predictions
Port: 5004 (separate from Engineering - port 5000, CCJE - port 5001, CBAA - port 5002, CAS - port 5003)
"""

from flask import Flask, jsonify, request, send_file
from flask.json.provider import DefaultJSONProvider
from flask_cors import CORS
from advanced_predictor_cte import CTEBoardExamPredictor
from training_report_generator_cte import CTETrainingReportGenerator
import os
import json
import math
...
def predict():
    ...
    try:
        ...
        df = predictor.fetch_cte_anonymous_data()
...
def export_predictions_pdf():
    ...
    try:
        ...
        # Custom styles with CTE Blue colors
        ...
        if predictions and len(predictions) > 0:
            ...
            table.setStyle(TableStyle([
                ...
                ('BACKGROUND', (0, 1), (-1, -1), colors.HexColor('#e1f1fd')),
        ...
        elements.append(Paragraph("This report was generated by the LSPU CTE AI Board Exam Prediction System", footer_style))
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f'CTE_Predictions_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf'
        )
...
@app.route('/api/cte/export/training-report', methods=['GET'])
    ...
    try:
        ...
        if report_file and os.path.exists(report_file):
            return send_file(
                ...
                mimetype='application/pdf',
                as_attachment=True,
                download_name=f'CTE_Training_Report_{datetime.now().strftime("%Y%m%d")}.pdf'
            )
        else:
            return jsonify({
                'success': False,
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'service': 'CTE Board Exam Prediction API',
        'port': 5003,
        'timestamp': datetime.now().isoformat()
    })
...
if __name__ == '__main__':
    print("=" * 70)
    print("🎓 CTE Board Exam Prediction API")
    print("=" * 70)
    print("🌐 Starting server on http://localhost:5003")
    print("📊 Department: Teacher Education")
    ...
    print("   - GET  /api/cte/export/training-report - Download training report PDF")
