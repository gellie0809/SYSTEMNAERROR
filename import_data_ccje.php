...
/* -------------------------------------------------
   1. SECURITY – only CCJE admin can access
   ------------------------------------------------- */
...
/* -------------------------------------------------
   2. ENSURE TABLE EXISTS (shared table)
   ------------------------------------------------- */
function ensureDatabaseExists($conn) {
    ...
    if (!$tableCheck || $tableCheck->num_rows == 0) {
        ...
        CREATE TABLE board_passers (
            ...
            department VARCHAR(100) NOT NULL DEFAULT 'Criminal Justice Education',
            exam_type VARCHAR(255) DEFAULT NULL,
            board_exam_type VARCHAR(255) DEFAULT 'Criminologist Licensure Examination',
...
/* -------------------------------------------------
   3. CSV PROCESSING (adapted for CCJE)
   ------------------------------------------------- */
    ...
    try {
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle)) !== FALSE) {
                ...
                if (!$dateObj) $dateObj = DateTime::createFromFormat('m/d/Y', $boardExamDate);
                if (!$dateObj) $dateObj = DateTime::createFromFormat('d-m-Y', $boardExamDate);
                if (!$dateObj) $dateObj = DateTime::createFromFormat('Y/m/d', $boardExamDate);

                if (!$dateObj) {
                    $errors[] = "Row $rowNumber: Invalid date format '$boardExamDate'";
                    $stats['errors']++;
                    continue;
                }
                ...
                if ($year < 2019 || $year > 2024) {
                    $errors[] = "Row $rowNumber: Year must be 2019-2024 (got $year)";
                    $stats['errors']++;
                    continue;
                }
                ...
                if (isset($processedRecords[$recordKey])) {
                    $errors[] = "Row $rowNumber: Duplicate inside CSV – $fullName";
                    $stats['duplicates']++;
                    continue;
                }

                $checkStmt = $conn->prepare(
                    "SELECT id FROM board_passers WHERE name = ? AND course = ? AND board_exam_date = ? AND department = 'Criminal Justice Education'"
                );
                $checkStmt->bind_param("sss", $fullName, $course, $boardExamDate);
                $checkStmt->execute();
                $res = $checkStmt->get_result();
                if ($res->num_rows > 0) {
                    $errors[] = "Row $rowNumber: Already in DB – $fullName";
                    $stats['duplicates']++;
                    $checkStmt->close();
                    continue;
                }
                ...
                $insertStmt = $conn->prepare(
                    "INSERT INTO board_passers
                     (name, sex, course, year_graduated, board_exam_date, result, department, exam_type, board_exam_type)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
                );
                $yearGraduated = $year;
                $resultStatus  = 'PASSED';
                $department    = 'Criminal Justice Education';
                $boardExamType = 'Criminologist Licensure Examination';

                $insertStmt->bind_param(
                    "sssisssss",
                    $fullName, $sex, $course, $yearGraduated, $boardExamDate,
                    $resultStatus, $department, $examType, $boardExamType
                );

                if ($insertStmt->execute()) {
                    $stats['imported']++;
                } else {
                    $errors[] = "Row $rowNumber: DB error – " . $insertStmt->error;
                    $stats['errors']++;
                }
                $insertStmt->close();
            }
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES['csvFile'])) {
    ...
    if ($file['error'] === 0) {
        ...
        } elseif ($file['size'] > 10_000_000) {
            $error = "File too large (max 10 MB).";
        } else {
            $uploadDir = 'uploads/';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);

            $newName = 'import_ccje_' . time() . '.csv';
            $dest    = $uploadDir . $newName;

            if (move_uploaded_file($file['tmp_name'], $dest)) {
                $result    = processCSVImport($dest, $conn);
                $stats     = $result['stats'];
                $errorList = $result['errors'];

                $vq = $conn->query("SELECT COUNT(*) AS cnt FROM board_passers WHERE department = 'Criminal Justice Education'");
                $actual = $vq ? $vq->fetch_assoc()['cnt'] : 0;

                if ($stats['imported'] > 0) {
                    $message = "Imported {$stats['imported']} / {$stats['total']} CCJE records. DB now has $actual Criminology records.";
                } else {
                    $error = "No records imported – check CSV format.";
                }
                unlink($dest);
            } else {
...
<head>
    ...
    <title>Import Data - CCJE Dashboard</title>
    ...
    * {
    ...
    body::before {
        ...
        bottom: 0;
        background:
            radial-gradient(circle at 20% 20%, rgba(211, 47, 47, 0.1) 0%, transparent 50%),
            radial-gradient(circle at 80% 60%, rgba(128, 0, 32, 0.08) 0%, transparent 50%),
            radial-gradient(circle at 40% 80%, rgba(250, 214, 165, 0.1) 0%, transparent 50%);
        pointer-events: none;
        z-index: 0;
    }
    ...
    .import-card::before {
        ...
        right: 0;
        bottom: 0;
        background: linear-gradient(135deg, rgba(211, 47, 47, 0.02) 0%, rgba(128, 0, 32, 0.03) 100%);
        pointer-events: none;
        z-index: 1;
    }
    ...
    .import-card:hover {
        ...
        box-shadow:
            0 40px 80px rgba(211, 47, 47, 0.2),
    ...
    .header-icon {
        ...
        box-shadow:
            0 15px 35px rgba(211, 47, 47, 0.35),
    ...
    /* File Upload Styles */
    .upload-area {
        border: 3px dashed rgba(211, 47, 47, 0.4);
        border-radius: 20px;
        padding: 50px;
        text-align: center;
        transition: all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1);
        background: linear-gradient(145deg, rgba(255, 255, 255, 0.95) 0%, rgba(253, 243, 231, 0.9) 100%);
        cursor: pointer;
        position: relative;
        overflow: hidden;
    ...
    .upload-area::before {
        ...
        right: 0;
        bottom: 0;
        background: linear-gradient(135deg, rgba(211, 47, 47, 0.05) 0%, rgba(128, 0, 32, 0.03) 100%);
        opacity: 0;
    ...
    .upload-area:hover {
        border-color: #D32F2F;
        background: linear-gradient(145deg, rgba(255, 255, 255, 1) 0%, rgba(250, 214, 165, 0.95) 100%);
    ...
    .upload-area.dragover {
        border-color: #C62828;
        background: linear-gradient(145deg, rgba(253, 243, 231, 0.98) 0%, rgba(250, 214, 165, 0.95) 100%);
    ...
    /* Import Stats */
    .import-stats {
        background: linear-gradient(145deg, rgba(255, 255, 255, 0.9) 0%, rgba(253, 243, 231, 0.8) 100%);
        border-radius: 20px;
        padding: 30px;
        margin-top: 30px;
    ...
    #importConfirmModal.modal {
        ...
        background: linear-gradient(135deg, rgba(211, 47, 47, 0.95) 0%, rgba(128, 0, 32, 0.9) 100%) !important;
    ...
    #importConfirmModal .modal-content::before {
        ...
        background: linear-gradient(135deg, #D32F2F 0%, #C62828 25%, #800020 50%, #C62828 75%, #D32F2F 100%) !important;
    ...
    #importConfirmModal .modal-header {
        ...
        border: 2px solid rgba(211, 47, 47, 0.15) !important;
        position: relative !important;
        overflow: hidden !important;
        box-shadow: 0 8px 25px rgba(211, 47, 47, 0.1) !important;
    }
    ...
    .import-confirm:hover {
        background: linear-gradient(135deg, #C62828 0%, #800020 100%) !important;
        border-color: rgba(211, 47, 47, 0.5) !important;
        transform: translateY(-2px) scale(1.02) !important;
        box-shadow: 0 12px 30px rgba(211, 47, 47, 0.45) !important;
    }
    ...
    #logoutModal .modal-content::before {
        ...
        right: -2px !important;
        bottom: -2px !important;
        background: linear-gradient(135deg, #800020 0%, #D32F2F 25%, #E63946 50%, #D32F2F 75%, #800020 100%) !important;
        border-radius: 30px !important;
        z-index: -1 !important;
    ...
    #logoutModal .modal-header {
        ...
        position: relative !important;
        overflow: hidden !important;
        box-shadow: 0 8px 25px rgba(211, 47, 47, 0.2) !important;
    }
    ...
    #logoutModal .modal-header::before {
        ...
        background: linear-gradient(90deg, #800020 0%, #D32F2F 50%, #E63946 100%) !important;
    ...
    #logoutModal .modal-header::after {
        ...
        background: linear-gradient(135deg, rgba(211, 47, 47, 0.15) 0%, rgba(250, 214, 165, 0.08) 100%) !important;
    ...
    #logoutModal .modal-icon {
        ...
        background: linear-gradient(135deg, #D32F2F 0%, #C62828 50%, #800020 100%) !important;
        border-radius: 50% !important;
        display: flex !important;
        align-items: center !important;
        justify-content: center !important;
        ...
        box-shadow:
            0 20px 40px rgba(211, 47, 47, 0.45),
            0 0 0 4px rgba(255, 255, 255, 0.8),
            0 0 0 6px rgba(211, 47, 47, 0.25) !important;
        position: relative !important;
        z-index: 1 !important;
    ...
    @keyframes iconPulse {
        ...
        100% {
            box-shadow:
                0 20px 40px rgba(211, 47, 47, 0.45),
                0 0 0 4px rgba(255, 255, 255, 0.8),
                0 0 0 6px rgba(211, 47, 47, 0.25);
            transform: scale(1);
        }

        50% {
            box-shadow:
                0 25px 50px rgba(211, 47, 47, 0.65),
                0 0 0 6px rgba(255, 255, 255, 0.9),
                0 0 0 8px rgba(211, 47, 47, 0.35);
            transform: scale(1.05);
        }
    }
    ...
    #logoutModal .modal-icon::before {
        ...
        background: linear-gradient(135deg, #E63946, #D32F2F, #C62828, #800020) !important;
        border-radius: 50% !important;
        z-index: -1 !important;
        opacity: 0.6 !important;
    ...
    #logoutModal .modal-btn.logout-confirm {
        background: linear-gradient(135deg, #C62828 0%, #D32F2F 50%, #E63946 100%) !important;
    ...
    #logoutModal .modal-btn.logout-confirm:hover {
        background: linear-gradient(135deg, #C62828 0%, #800020 50%, #5D0016 100%) !important;
    ...
    #logoutModal .modal-btn.logout-confirm:active {
        ...
        background: linear-gradient(135deg, #800020 0%, #5D0016 50%, #C62828 100%) !important;
    }
...
<body>
    <?php include './includes/ccje_nav.php'; ?>
        ...
        <div class="card">
            <div class="card-header">
                <div class="header-content">
                    ...
                    <div>
                        <h1 class="header-title">Import Board Passers – CCJE</h1>
                        <p class="header-subtitle">Upload CSV files (2019-2024) for College of Criminal Justice
                            Education passers</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-content">
                ...
                <div class="template-section">
                    <div class="template-icon"><i class="fas fa-download"></i></div>
                    <h3 class="template-title">Download CSV Template</h3>
                    <p class="template-description">Use this format for College of Criminal Justice Education board
                        passer data.</p>
                    <a href="#" class="template-btn" onclick="downloadTemplate(event)">
                ...
                <form action="" method="POST" enctype="multipart/form-data" id="importForm">
                    <div class="upload-area" onclick="document.getElementById('csvFile').click()"
                        ...
                        <div class="upload-text">Drop CSV here</div>
                        <div class="upload-hint">or click to browse</div>
                        <input type="file" id="csvFile" name="csvFile" accept=".csv" class="file-input"
                            onchange="fileSelected(event)" required>
                    ...
                    <div style="text-align:center;margin-top:30px;">
                        <button type="submit" class="btn btn-primary" id="importBtn" disabled
                            style="background: linear-gradient(135deg, #D32F2F 0%, #C62828 100%); color: white; border: none; border-radius: 14px; padding: 16px 40px; font-size: 1.1rem; font-weight: 700; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 8px 24px rgba(211, 47, 47, 0.35); letter-spacing: 0.5px;">
    ...
    <div id="importConfirmModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <div class="modal-icon"><i class="fas fa-file-upload"></i></div>
            ...
            <div class="import-file-preview" id="importFilePreview">
                <div class="file-preview-icon"><i class="fas fa-file-csv"></i></div>
            ...
            <p class="modal-text">
                <i class="fas fa-info-circle" style="color:#D32F2F;margin-right:8px;"></i>
            ...
            <div class="modal-buttons">
                <button id="importConfirmYes" class="modal-btn import-confirm">
                    <i class="fas fa-upload"></i><span class="btn-text">Yes, Import Data</span>
                    <i class="fas fa-spinner btn-spinner"></i><i class="fas fa-check-circle btn-check"></i>
                </button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-buttons">
                <button id="logoutConfirmYes" class="modal-btn logout-confirm">
                    <i class="fas fa-check"></i><span class="btn-text">Yes, Logout</span>
                    <i class="fas fa-spinner btn-spinner"></i><i class="fas fa-check-circle btn-check"></i>
                </button>
    ...
    <script>
    /* -------------------------------------------------
       ALL JAVASCRIPT FROM THE ENGINEERING FILE
       (drag-drop, modals, template download, logout)
       ------------------------------------------------- */
    function confirmLogout(event) {
        ...
        if (modal) {
            ...
            document.getElementById('logoutConfirmYes').onclick = function(e) {
                e.preventDefault();
    ...
    function handleInteractiveLogout(button) {
        ...
        const cancel = document.getElementById('logoutConfirmNo');
        if (cancel) {
    ...
    function fileSelected(event) {
        ...
        if (file) {
            ...
            fs.textContent = `Size: ${(file.size/1024).toFixed(2)} KB`;
            fi.style.display = 'block';
            ib.disabled = false;
            fi.style.animation = 'alertSlideIn .4s ease';
        } else {
    ...
    function downloadTemplate(event) {
        event.preventDefault();
        const csv =
            `First Name,Last Name,Middle Name,Suffix,Sex,Course,Year Graduated,Board Exam Date,Take Attempt,Result,Overall Rating (%)\nJuan,Dela Cruz,A.,Jr.,Male,BS Psychology,2023,2024-03-15,First Timer,Passed,88.50\n`;
        ...
        a.href = url;
        a.download = 'ccje_board_passers_template.csv';
    ...
    document.getElementById('importConfirmYes').addEventListener('click', function() {
        ...
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> <span class="btn-text">Importing...</span>';
    ...
    document.getElementById('importConfirmNo').addEventListener('click', hideImportConfirmModal);
    document.getElementById('importConfirmModal').onclick = function(e) {
        if (e.target === this) hideImportConfirmModal();
    };
    </script>
</body>
