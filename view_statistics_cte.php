...
// QUERY PARA SA CTE BOARD EXAMS LAMANG
// Karaniwang board exams sa CTE: LET Elementary & LET Secondary (at mga sub-majors)
$betStmt = $conn->prepare("
    SELECT id, exam_type_name 
    FROM board_exam_types 
    WHERE department = 'Teacher Education' 
    ORDER BY exam_type_name ASC
");
$betStmt->execute();
...
// Lahat ng exam dates under Teacher Education
$bedStmt = $conn->prepare("
    SELECT id, exam_date, exam_description, exam_type_id 
    FROM board_exam_dates 
    WHERE department = 'Teacher Education' 
...
<head>
    ...
    <title>View Statistics - CTE</title>
...
<body>
    ...
    <div class="topbar">
        <div class="dashboard-title">CTE Admin Dashboard</div>
        ...
        <div class="layout">
            ...
            <div class="filter-panel">
                <div class="filter-header">
                    ...
                    <button id="printBtn" class="print-btn" title="Print the statistics">
                        <i class="fas fa-print"></i> Print Data Statistics
                    </button>
                </div>
                ...
                <div class="step">
                    ...
                    <div class="tip-pill">
                        ...
                        <span>You can also click a year chip below to view all dates in that year.</span>
                    </div>
                    <div id="yearChips" class="year-chips">
                        <?php
                    $years = [];
                    foreach ($boardExamDates as $d) {
                        $year = date('Y', strtotime($d['exam_date']));
                        if (!in_array($year, $years)) {
                            $years[] = $year;
                            echo "<div class='year-chip' data-year='$year'>$year</div>";
                        }
                    }
                    ?>
                    </div>
                ...
                <div class="filter-actions">
                    <button id="confirmBtn" class="btn-primary" disabled title="Load charts for the selected type/date">
                        <i class="fas fa-check-circle"></i> Confirm & View
                    </button>
                    <button id="clearBtn" class="btn-clear" title="Reset filters">
                ...
                <div style="margin-top:14px;">
                    <div class="selected-header" style="display:flex; gap:12px; flex-wrap:wrap;">
                        <div class="selected-pill" style="display:flex;align-items:center;gap:8px;">
                            <i class="fa-solid fa-layer-group"></i><span id="selectedType">Overall</span>
                        </div>
                        <div class="selected-pill" style="display:flex;align-items:center;gap:8px;">
                            <i class="fa-solid fa-calendar"></i><span id="selectedDate">Overall</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="visual-panel">
                ...
                <div id="statsContainer" class="printable" style="position:relative;">
                    <!-- Print Header -->
                    <div id="printHeader" class="print-header" style="margin-bottom:12px; display:none;">
                        <h1 style="margin:0; font-size:20px;">CTE Board Exam Statistics</h1>
                        <div id="printMeta" style="margin-top:6px; color:var(--muted);">&nbsp;</div>
                    </div>

                    <div class="charts-grid">
                        <!-- Group 1: Sex Comparison -->
                    ...
                    <!-- Group 3: First Timer vs Repeater -->
                    ...
                    <!-- Overall CTE Passing Rate Trend -->
                    <div id="deptPassingCard" class="chart-card" style="grid-column: 1 / -1;">
                        <div
                            style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <h4 style="margin: 0;">Board Passing Rate in CTE (2019–2024)</h4>
                        ...
                        <div class="print-caption">Board passing rate in CTE, 2019–2024</div>
                    </div>
                    ...
                    <div id="forecastTrendsCard" class="chart-card" style="grid-column: 1 / -1;">
                        ...
                        <div class="print-caption">Linear regression forecast of passing rates per board exam type using
                            latest history.</div>
                    </div>

                    <!-- Subjects Performance (CPA Subjects) -->
                    <div class="chart-card" style="grid-column: 1 / -1;">
                        <h4 id="subjectsHeader">CPA Subjects Performance</h4>
                        ...
                        <div class="print-caption">CPA subjects (Passed vs Failed) for the selected exam date</div>
                    </div>
                </div>
    ...
    function renderOverallSubjectsBars(groups) {
        ...
        function fetchSubjectsTotalsForType(typeId) {
            return fetch(`stats_cte.php?action=subjects&boardExamTypeId=${encodeURIComponent(typeId)}`)
                .then(r => r.json())
