...
// Only allow College of Teacher Education (CTE) admin
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["add_exam_type"])) {
  ...
  if ($name === '') {
    header("Location: manage_data_cte.php?error=LET Level name is required"); exit();
  }
  $chk = $conn->prepare("SELECT id FROM board_exam_types WHERE LOWER(TRIM(exam_type_name)) = LOWER(TRIM(?)) AND department='Teacher Education'");
  $chk->bind_param('s', $name); $chk->execute(); $chk->store_result();
  if ($chk->num_rows > 0) { $chk->close(); header("Location: manage_data_cte.php?error=LET Level already exists"); exit(); }
  $chk->close();

  $ins = $conn->prepare("INSERT INTO board_exam_types (exam_type_name, department) VALUES (?, 'Teacher Education')");
  $ins->bind_param('s', $name);
  if ($ins->execute()) { 
    $ins->close(); 
    header("Location: manage_data_cte.php?success=LET Level added successfully"); 
    exit(); 
  }
  $ins->close(); 
  header("Location: manage_data_cte.php?error=Failed to add LET Level"); 
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_exam_type'])) {
  ...
  if ($id <= 0 || $newname === '') { 
    header("Location: manage_data_cte.php?error=Invalid LET Level data"); exit(); 
  }
  $chk = $conn->prepare("SELECT id FROM board_exam_types WHERE LOWER(TRIM(exam_type_name)) = LOWER(TRIM(?)) AND department='Teacher Education' AND id != ?");
  $chk->bind_param('si', $newname, $id); $chk->execute(); $res = $chk->get_result();
  if ($res && $res->num_rows > 0) { 
    $chk->close(); 
    header("Location: manage_data_cte.php?error=LET Level name already exists"); exit(); 
  }
  $chk->close();

  $up = $conn->prepare("UPDATE board_exam_types SET exam_type_name = ? WHERE id = ? AND department='Teacher Education'");
  $up->bind_param('si', $newname, $id);
  if ($up->execute()) { 
    $up->close(); 
    header("Location: manage_data_cte.php?success=LET Level updated"); 
    exit(); 
  }
  $up->close(); 
  header("Location: manage_data_cte.php?error=Failed to update LET Level"); 
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_exam_type'])) {
  $id = intval($_POST['exam_type_id'] ?? 0);
  if ($id <= 0) { header("Location: manage_data_cte.php?error=Invalid LET Level"); exit(); }

  // Check if used in subjects or passers
  $chk = $conn->prepare("SELECT COUNT(*) as cnt FROM subject_exam_types WHERE exam_type_id = ?");
  $chk->bind_param('i', $id); $chk->execute(); $cres = $chk->get_result(); 
  $crow = $cres ? $cres->fetch_assoc() : null; $cnt = intval($crow['cnt'] ?? 0); $chk->close();
  if ($cnt > 0) { header("Location: manage_data_cte.php?error=Cannot delete: LET Level has subjects"); exit(); }

  $del = $conn->prepare("UPDATE board_exam_types SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department='Teacher Education'"); 
  $del->bind_param('i', $id);
  if ($del->execute()) { 
    $del->close(); 
    header("Location: manage_data_cte.php?success=LET Level deleted"); 
    exit(); 
  }
  $del->close(); 
  header("Location: manage_data_cte.php?error=Failed to delete LET Level"); 
  exit();
}

// ================================
// 2. SUBJECTS (LET Parts)
// ================================

// Add Subject (e.g., Professional Education, General Education, Major)
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_subject'])) {
  $exam_type_id = intval($_POST['subject_exam_type'] ?? 0);
  $subject_name = trim($_POST['new_subject'] ?? '');
  $total_items  = intval($_POST['new_subject_total_items'] ?? 150);
  if ($exam_type_id <= 0 || $subject_name === '') { 
    header("Location: manage_data_cte.php?error=Please fill all subject fields"); exit(); 
  }

  $ins = $conn->prepare("INSERT INTO subjects (subject_name, total_items, department) VALUES (?, ?, 'Teacher Education')");
  $ins->bind_param('si', $subject_name, $total_items);
  if (!$ins->execute()) { 
    $ins->close(); 
    header("Location: manage_data_cte.php?error=Failed to add subject"); exit(); 
  }
  $sid = $ins->insert_id; $ins->close();

  $m = $conn->prepare("INSERT INTO subject_exam_types (subject_id, exam_type_id) VALUES (?, ?)");
  $m->bind_param('ii', $sid, $exam_type_id); $m->execute(); $m->close();

  header("Location: manage_data_cte.php?success=Subject added&exam_type_id=" . $exam_type_id);
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_subject'])) {
  ...
  $items = intval($_POST['edit_subject_items'] ?? 150);
  if ($sid <= 0 || $name === '') { 
    header("Location: manage_data_cte.php?error=Invalid subject data"); exit(); 
  }

  // Prevent edit if already used by a passer
  $chk = $conn->prepare("SELECT COUNT(*) as cnt FROM board_passer_subjects WHERE subject_id = ?");
  $chk->bind_param('i', $sid); $chk->execute(); $cres = $chk->get_result(); 
  $crow = $cres ? $cres->fetch_assoc() : null; $countUsage = intval($crow['cnt'] ?? 0); $chk->close();
  if ($countUsage > 0) { 
    header("Location: manage_data_cte.php?error=Cannot edit: Subject already used by passers"); exit(); 
  }

  $up = $conn->prepare("UPDATE subjects SET subject_name = ?, total_items = ? WHERE id = ? AND department = 'Teacher Education'");
  $up->bind_param('sii', $name, $items, $sid);
  if ($up->execute()) { 
    $up->close(); 
    header("Location: manage_data_cte.php?success=Subject updated"); 
    exit(); 
  }
  $up->close(); 
  header("Location: manage_data_cte.php?error=Failed to update subject"); 
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_subject'])) {
  $sid = intval($_POST['subject_id'] ?? 0);
  if ($sid <= 0) { header("Location: manage_data_cte.php?error=Invalid subject"); exit(); }

  $chk = $conn->prepare("SELECT COUNT(*) as cnt FROM board_passer_subjects WHERE subject_id = ?");
  $chk->bind_param('i', $sid); $chk->execute(); $cres = $chk->get_result(); 
  $crow = $cres ? $cres->fetch_assoc() : null; $countUsage = intval($crow['cnt'] ?? 0); $chk->close();
  if ($countUsage > 0) { 
    header("Location: manage_data_cte.php?error=Cannot delete: Subject used by passers"); exit(); 
  }

  $del = $conn->prepare("UPDATE subjects SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department = 'Teacher Education'");
  $del->bind_param('i', $sid);
  if ($del->execute()) { 
    $del->close(); 
    header("Location: manage_data_cte.php?success=Subject deleted"); 
    exit(); 
  }
  $del->close(); 
  header("Location: manage_data_cte.php?error=Failed to delete subject"); 
  exit();
}

// ================================
// 3. EXAM DATES (LET Schedules)
// ================================
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_exam_date'])) {
  ...
  if ($exam_type_id <= 0 || $exam_date === '') { 
    header("Location: manage_data_cte.php?error=Invalid LET date"); exit(); 
  }

  $ins = $conn->prepare("INSERT INTO board_exam_dates (exam_date, exam_description, exam_type_id, department) VALUES (?, ?, ?, 'Teacher Education')");
  $ins->bind_param('ssi', $exam_date, $exam_desc, $exam_type_id);
  if ($ins->execute()) { 
    $ins->close(); 
    header("Location: manage_data_cte.php?success=LET date added&exam_type_id=" . $exam_type_id); 
    exit(); 
  }
  $ins->close(); 
  header("Location: manage_data_cte.php?error=Failed to add LET date"); 
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_exam_date'])) {
  ...
  $up = $conn->prepare("UPDATE board_exam_dates SET exam_date = ?, exam_description = ? WHERE id = ? AND department = 'Teacher Education'");
  $up->bind_param('ssi', $edate, $edesc, $eid);
  if ($up->execute()) { 
    $up->close(); 
    header("Location: manage_data_cte.php?success=LET date updated"); 
    exit(); 
  }
  $up->close(); 
  header("Location: manage_data_cte.php?error=Failed to update LET date"); 
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_exam_date'])) {
  $eid = intval($_POST['exam_date_id'] ?? 0);
  if ($eid <= 0) { header("Location: manage_data_cte.php?error=Invalid LET date"); exit(); }

  $check = $conn->prepare("SELECT COUNT(*) as cnt FROM board_passers WHERE board_exam_date = (SELECT exam_date FROM board_exam_dates WHERE id = ?) AND department = 'Teacher Education'");
  $check->bind_param('i', $eid); $check->execute(); $res = $check->get_result(); 
  $row = $res ? $res->fetch_assoc() : null; $used = intval($row['cnt'] ?? 0); $check->close();
  if ($used > 0) { 
    header("Location: manage_data_cte.php?error=Cannot delete: LET date used by $used passer(s)"); exit(); 
  }

  $del = $conn->prepare("UPDATE board_exam_dates SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department = 'Teacher Education'");
  $del->bind_param('i', $eid);
  if ($del->execute()) { 
    $del->close(); 
    header("Location: manage_data_cte.php?success=LET date deleted"); 
    exit(); 
  }
  $del->close(); 
  header("Location: manage_data_cte.php?error=Failed to delete LET date"); 
  exit();
}

// ================================
// 4. COURSES (BEEd, BSEd-Math, etc.)
// ================================
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_course'])) {
  ...
  if ($new_course === '') { 
    header("Location: manage_data_cte.php?error=Course name required"); exit(); 
  }
  $ins = $conn->prepare("INSERT INTO courses (course_name, department) VALUES (?, 'Teacher Education')");
  $ins->bind_param('s', $new_course);
  if ($ins->execute()) { 
    $ins->close(); 
    header("Location: manage_data_cte.php?success=Course added"); 
    exit(); 
  }
  $ins->close(); 
  header("Location: manage_data_cte.php?error=Failed to add course"); 
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_course'])) {
  ...
  if ($cid <= 0 || $cname === '') { 
    header("Location: manage_data_cte.php?error=Invalid course data"); exit(); 
  }
  $up = $conn->prepare("UPDATE courses SET course_name = ? WHERE id = ? AND department = 'Teacher Education'");
  $up->bind_param('si', $cname, $cid);
  if ($up->execute()) { 
    $up->close(); 
    header("Location: manage_data_cte.php?success=Course updated"); 
    exit(); 
  }
  $up->close(); 
  header("Location: manage_data_cte.php?error=Failed to update course"); 
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_course'])) {
  $cid = intval($_POST['course_id'] ?? 0);
  if ($cid <= 0) { header("Location: manage_data_cte.php?error=Invalid course"); exit(); }
  
  $g = $conn->prepare("SELECT course_name FROM courses WHERE id = ? AND department = 'Teacher Education'"); 
  $g->bind_param('i', $cid); $g->execute(); $gres = $g->get_result(); 
  $grow = $gres ? $gres->fetch_assoc() : null; $g->close();
  $cname = trim($grow['course_name'] ?? '');
  if ($cname === '') { header("Location: manage_data_cte.php?error=Course not found"); exit(); }
  
  $chk = $conn->prepare("SELECT COUNT(*) as cnt FROM board_passers WHERE course = ? AND department = 'Teacher Education'");
  $chk->bind_param('s', $cname); $chk->execute(); $res = $chk->get_result(); 
  $row = $res ? $res->fetch_assoc() : null; $used = intval($row['cnt'] ?? 0); $chk->close();
  if ($used > 0) { 
    header("Location: manage_data_cte.php?error=Cannot delete: Course used by $used passer(s)"); exit(); 
  }

  $del = $conn->prepare("UPDATE courses SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department = 'Teacher Education'");
  $del->bind_param('i', $cid);
  if ($del->execute()) { 
    $del->close(); 
    header("Location: manage_data_cte.php?success=Course deleted"); 
    exit(); 
  }
  $del->close(); 
  header("Location: manage_data_cte.php?error=Failed to delete course"); 
  exit();
}
...
// LET Levels (Elementary / Secondary)
$etres = $conn->query("SELECT id, TRIM(exam_type_name) as name FROM board_exam_types WHERE department='Teacher Education' AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY exam_type_name ASC");
$exam_types = [];
while ($r = $etres->fetch_assoc()) { $exam_types[] = $r; }

// Default selected LET Level
$selected_exam_type_id = intval($_GET['exam_type_id'] ?? 0);
if ($selected_exam_type_id === 0 && !empty($exam_types)) { 
  $selected_exam_type_id = $exam_types[0]['id']; 
}

// Subjects for selected LET Level
$subjects = [];
if ($selected_exam_type_id > 0) {
  $stmt = $conn->prepare("
    SELECT s.id, TRIM(s.subject_name) as subject_name, s.total_items 
    FROM subjects s 
    JOIN subject_exam_types setmap ON setmap.subject_id = s.id 
    WHERE setmap.exam_type_id = ? AND s.department = 'Teacher Education' AND (s.is_deleted = 0 OR s.is_deleted IS NULL)
    ORDER BY s.subject_name ASC
  ");
  $stmt->bind_param('i', $selected_exam_type_id); 
  $stmt->execute(); $sres = $stmt->get_result();
...
// Courses
$courses = [];
$cres = $conn->query("SELECT id, TRIM(course_name) as name FROM courses WHERE department='Teacher Education' AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY course_name ASC");
...
if ($selected_exam_type_id > 0) {
  $dstmt = $conn->prepare("
    SELECT id, exam_date, TRIM(exam_description) as exam_description 
    FROM board_exam_dates 
    WHERE exam_type_id = ? AND department = 'Teacher Education' AND (is_deleted = 0 OR is_deleted IS NULL)
    ORDER BY exam_date DESC
  ");
  $dstmt->bind_param('i', $selected_exam_type_id); 
...
<head>
   ...
   /* CTE-specific sidebar color overrides */
    ...
    .modal-content::before {
        ...
        right: 0;
        height: 4px;
        background: linear-gradient(90deg, #4663ac 0%, #6B8DD6 50%, #4663ac 100%);
    ...
    .modal-content input[type="date"]:focus {
        outline: none;
        border-color: #4663ac;
        box-shadow: 0 0 0 4px rgba(70, 99, 172, 0.15), 0 4px 12px rgba(70, 99, 172, 0.2);
        transform: translateY(-2px);
    }
    ...
    .modal-content .btn {
        ...
        font-size: 1.05rem;
        font-weight: 700;
        box-shadow: 0 6px 20px rgba(70, 99, 172, 0.35);
        letter-spacing: 0.3px;
    }
    ...
    .modal-content .btn:hover {
        ...
        box-shadow: 0 12px 30px rgba(70, 99, 172, 0.5);
    }
    ...
    .confirm-icon {
        ...
        font-size: 3rem;
        color: white;
        box-shadow: 0 12px 35px rgba(70, 99, 172, 0.5);
...
<body>
    ...
    <div class="topbar">
        <h1 class="dashboard-title">CTE Admin Dashboard</h1>
    ...
    <div class="main-content">
        ...
        <div class="panel" style="margin-bottom:24px;">
            ...
            <form method="post" action="manage_data_cte.php" class="add-form-section" id="addCourseForm">
        ...
        <div class="grid">
            <div class="panel" style="min-height:480px;">
                ...
                <form method="post" action="manage_data_cte.php" class="add-form-section" id="addExamTypeForm">
                    <div class="form-row">
                ...
                <div style="margin-top:6px;">
                    ...
                    <div class="list-item">
                        ...
                        <div class="actions">
                            <form method="get" action="manage_data_cte.php" style="display:inline;"><input type="hidden"
            ...
            <div class="right-column">
                <div class="panel">
                    ...
                    <div
                        ...
                        <strong style="color:#4663ac; font-size:1.05rem;"><i class="fas fa-check-circle"></i>
                            <?= htmlspecialchars($etname ?? 'Selected') ?></strong>
                    </div>

                    <form method="post" action="manage_data_cte.php?exam_type_id=<?= $selected_exam_type_id ?>"
    ...
    <div id="editExamTypeModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form id="editExamTypeForm" method="post" action="manage_data_cte.php">
                <input type="hidden" name="exam_type_id" id="editExamTypeId">
                <div style="margin-bottom:20px;">
                    <label
                        style="display:block; margin-bottom:8px; color:#4663ac; font-weight:600; font-size:0.9rem;"><i
    ...
    <div id="editSubjectModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form id="editSubjectForm" method="post" action="manage_data_cte.php">
    ...
    <div id="editCourseModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form id="editCourseForm" method="post" action="manage_data_cte.php">
    ...
    <div id="editExamDateModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form id="editExamDateForm" method="post" action="manage_data_cte.php">
    ...
    function showCustomConfirm(message, callback, icon) {
        ...
        if (icon) {
            ...
            if (icon === 'Plus') {
                iconEl.style.background = 'linear-gradient(135deg, #4663ac 0%, #6B8DD6 100%)';
            } else if (icon === 'Trash') {
