...
// Ensure table exists (shared table – works for all departments)
function ensureDatabaseExists($conn) {
    ...
    if (!$tableCheck || $tableCheck->num_rows == 0) {
        ...
        CREATE TABLE board_passers (
            ...
            board_exam_type VARCHAR(255) DEFAULT NULL,    -- LET - Elementary / LET - Secondary
            rating DECIMAL(5,2) DEFAULT NULL,             -- General Average (optional)
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            INDEX idx_dept_date (department, board_exam_date),
            INDEX idx_name (name)
...
// Optional: Filter by year or LET level
$filter_year = $_GET['year'] ?? '';
$filter_level = $_GET['level'] ?? '';

// Build dynamic query
$where = "WHERE department = 'Teacher Education'";
$params = [];
$types = '';

if (!empty($filter_year) && preg_match('/^\d{4}$/', $filter_year)) {
    $where .= " AND YEAR(board_exam_date) = ?";
    $params[] = $filter_year;
    $types .= 'i';
}
if (!empty($filter_level)) {
    $where .= " AND board_exam_type LIKE ?";
    $params[] = "%$filter_level%";
    $types .= 's';
}
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    $query = "SELECT 
              ...
              FROM board_passers 
              $where 
              ORDER BY board_exam_date DESC, name ASC";
    ...
    if ($result->num_rows > 0) {
        ...
        header('Content-Disposition: attachment; filename="cte_let_passers_' . date('Y-m-d_H-i-s') . '.csv"');
        header('Pragma: no-cache');
        header('Expires: 0');
        
        // Add BOM for Excel to properly read UTF-8 (especially Filipino names)
        echo "\xEF\xBB\xBF";

        $output = fopen('php://output', 'w');

        // CSV Header (CTE/LET friendly)
        fputcsv($output, [
        ...
        while ($row = $result->fetch_assoc()) {
            $letDate = date('F j, Y', strtotime($row['board_exam_date'])); // e.g., September 29, 2024
            $average = $row['rating'] ? number_format($row['rating'], 2) : '-';
            ...
            fputcsv($output, [
                ...
                $letDate,
                $row['result'],
                $row['exam_type'] ?: 'First Timer',
                strip_tags($row['board_exam_type']), // clean HTML if any
                $average
            ]);
        }
        ...
        // No data found
        $error_message = "No LET passers found for the selected filters.";
    }
}
...
<head>
    ...
    <title>Export Data - CTE Dashboard</title>
        ...
        <div class="card">
            <div class="card-header">
                <div class="header-content">
                    ...
                    <div>
                        <h1 class="header-title">Export Board Passers Data – College of Teacher Education</h1>
