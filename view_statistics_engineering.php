...
// Fetch board exam types and dates for filters (exclude deleted)
$betStmt = $conn->prepare("SELECT id, exam_type_name FROM board_exam_types WHERE department='Engineering' AND is_deleted = 0 ORDER BY exam_type_name ASC");
$betStmt->execute();
...
<head>
    ...
    <style>
    /* Modern sage green theme with animations */
    :root {
        --primary: #8BA49A;
        --primary-dark: #3B6255;
        --primary-darker: #3B6255;
        --magenta: #f472b6;
        --muted: #64748b;
        --success: #10b981;
        --danger: #ef4444;
    ...
    * {
    ...
    body::before {
        ...
        right: 0;
        bottom: 0;
        background: radial-gradient(circle at 20% 50%, rgba(145, 179, 142, 0.1) 0%, transparent 50%),
            radial-gradient(circle at 80% 80%, rgba(145, 179, 142, 0.08) 0%, transparent 50%);
        pointer-events: none;
        z-index: 0;
    }
    ...
    .filter-panel {
        width: 100%;
        background: linear-gradient(145deg, rgba(255, 255, 255, 0.95) 0%, rgba(240, 249, 240, 0.9) 100%);
        border-radius: 20px;
        padding: 28px;
        box-shadow: 0 10px 35px rgba(145, 179, 142, 0.15);
    ...
    .visual-panel {
        background: linear-gradient(145deg, rgba(255, 255, 255, 0.9) 0%, rgba(240, 249, 240, 0.85) 100%);
        border-radius: 20px;
        padding: 28px;
        box-shadow: 0 14px 40px rgba(145, 179, 142, 0.2);
    ...
    select {
        ...
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        appearance: none;
        background-image: linear-gradient(45deg, transparent 50%, #8BA49A 50%),
            linear-gradient(135deg, #8BA49A 50%, transparent 50%);
    ...
    select:focus {
        outline: none;
        border-color: #8BA49A;
        box-shadow: 0 0 0 3px rgba(145, 179, 142, 0.2);
    ...
    .filter-header .print-btn:hover {
        background: linear-gradient(135deg, #3B6255 0%, #3B6255 100%);
        transform: translateY(-2px);
    ...
    .selected-pill {
        ...
        padding: 10px 18px;
        border-radius: 999px;
        background: linear-gradient(145deg, #fff 0%, #E2DFDA 100%);
        color: #0f1724;
        border: 2px solid #8BA49A;
        box-shadow: 0 4px 15px rgba(145, 179, 142, 0.2);
    ...
    .tip-pill {
        ...
        padding: 12px 16px;
        border-radius: 14px;
        background: linear-gradient(145deg, #E2DFDA 0%, #CBDED3 100%);
        color: #3B6255;
        border: 2px solid #D2C49E;
        font-size: 0.92rem;
        font-weight: 600;
        animation: fadeIn 0.6s ease;
    }
    ...
    .chip.active {
        ...
        box-shadow: 0 4px 15px rgba(145, 179, 142, 0.4);
    }
    ...
    .btn-primary {
        ...
        cursor: pointer;
        font-weight: 700;
        box-shadow: 0 8px 24px rgba(145, 179, 142, 0.3);
        display: flex;
        align-items: center;
        gap: 8px;
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
    ...
    .btn-primary:hover {
        background: linear-gradient(135deg, #3B6255 0%, #3B6255 100%);
        transform: translateY(-2px);
        box-shadow: 0 12px 30px rgba(145, 179, 142, 0.4);
    }
    ...
    .chart-card {
        ...
        border-radius: 18px;
        border: 2px solid rgba(145, 179, 142, 0.15);
        box-shadow: 0 8px 25px rgba(145, 179, 142, 0.12);
        display: flex;
    ...
    .chart-card::before {
        ...
        right: 0;
        height: 3px;
        background: linear-gradient(90deg, #8BA49A 0%, #3B6255 50%, #D2C49E 100%);
        opacity: 0;
    ...
    .chart-card:hover {
        ...
        box-shadow: 0 16px 40px rgba(145, 179, 142, 0.25);
        border-color: rgba(145, 179, 142, 0.3);
    }
    ...
    .group-divider {
        ...
        background: linear-gradient(145deg, rgba(255, 255, 255, 0.95) 0%, rgba(240, 249, 240, 0.9) 100%);
        border: 2px solid rgba(145, 179, 142, 0.2);
        box-shadow: 0 8px 25px rgba(145, 179, 142, 0.15);
        backdrop-filter: blur(12px);
        -webkit-backdrop-filter: blur(12px);
        animation: fadeIn 0.6s ease;
    }
    ...
    .chart-card.small-kpi {
        display: flex;
        align-items: center;
        justify-content: center;
    }

    .chart-card.small-kpi .donut-wrapper::before {
        width: 96px;
        height: 96px
    }

    /* prettier overall bars */
    .chart-card .overall-track {
    ...
    .chart-card .seg-pass:hover,
    .chart-card .seg-fail:hover {
        filter: brightness(1.03) saturate(120%);
    }
    ...
    #logoutModal .modal-content::before {
        ...
        background: linear-gradient(135deg, #3B6255 0%, #8BA49A 25%, #D2C49E 50%, #8BA49A 75%, #3B6255 100%) !important;
    ...
    #logoutModal .modal-header {
        ...
        border: 2px solid #D2C49E !important;
        position: relative !important;
    ...
    #logoutModal .modal-header::before {
        ...
        background: linear-gradient(90deg, #3B6255 0%, #8BA49A 50%, #D2C49E 100%) !important;
    ...
    #logoutModal .modal-icon {
        ...
        background: linear-gradient(135deg, #3B6255 0%, #8BA49A 50%, #3B6255 100%) !important;
        border-radius: 50% !important;
        display: flex !important;
        align-items: center !important;
        justify-content: center !important;
    ...
    #logoutModal .modal-icon::before {
        ...
        background: linear-gradient(135deg, #D2C49E, #8BA49A, #3B6255, #3B6255) !important;
        border-radius: 50% !important;
        z-index: -1 !important;
    ...
    #logoutModal .modal-title {
        ...
        background: linear-gradient(135deg, #3B6255 0%, #3B6255 100%) !important;
    ...
    #logoutModal .modal-btn.logout-confirm:hover {
        background: linear-gradient(135deg, #3B6255 0%, #3B6255 50%, #8BA49A 100%) !important;
    ...
    /* Force logout buttons to show with highest specificity */
...
<body>
        ...
        <div class="layout">
            <div class="filter-panel">
                <div class="filter-header">
                    ...
                    <button id="printBtn" class="print-btn" title="Print the statistics"><i class="fas fa-print"></i>
                        Print Data Statistics</button>
                </div>
                <div class="step">
                    <div class="step-header">
                        <h4><i class="fa-solid fa-clipboard-list" style="opacity:.9"></i>&nbsp; Choose Board Exam Type
                        </h4>
                    </div>
                    <div id="examTypeCheckboxes" style="max-height: 250px; overflow-y: auto; border: 2px solid #e2e8f0; border-radius: 12px; padding: 12px; background: #ffffff;">
                        <?php foreach ($boardExamTypes as $t): ?>
                        <label style="display: flex; align-items: center; padding: 10px; margin: 4px 0; border-radius: 8px; cursor: pointer; transition: all 0.2s; background: #f8fafc;" 
                               onmouseover="this.style.background='#E2DFDA'" 
                               onmouseout="this.style.background='#f8fafc'">
                            <input type="checkbox" class="exam-type-checkbox" value="<?= htmlspecialchars($t['id']) ?>" 
                                   style="width: 18px; height: 18px; margin-right: 12px; cursor: pointer; accent-color: #8BA49A;">
                            <span style="font-size: 0.95rem; color: #334155; font-weight: 500;"><?= htmlspecialchars($t['exam_type_name']) ?></span>
                        </label>
                        <?php endforeach; ?>
                    </div>
                    <div class="tip-pill" style="margin-top: 8px;"><i class="fa-solid fa-lightbulb"></i><span>Select one or more exam types to view their statistics</span></div>
                </div>

                <div class="step">
                    <div class="step-header">
                        <h4><i class="fa-solid fa-calendar-days" style="opacity:.9"></i>&nbsp; Choose Exam Date</h4>
                    </div>
                    <select id="examDateSelect" disabled>
                        <option value="">Choose exam type first</option>
                    </select>
                    <div class="tip-pill"><i class="fa-solid fa-lightbulb"></i><span>You can also click a year chip
                            below to view all dates in that year.</span></div>
                    <div id="yearChips" class="year-chips"></div>
                </div>

                <div class="filter-actions">
                    <button id="confirmBtn" class="btn-primary" disabled
                        title="Load charts for the selected type/date"><i class="fas fa-check-circle"></i> Confirm &
                        View</button>
                    <button id="clearBtn" class="btn-clear" title="Reset filters"><i class="fas fa-eraser"></i>
                        Clear</button>
                </div>

                <div style="margin-top:14px;">
                    <div class="selected-header" style="display:flex; gap:12px; flex-wrap:wrap;">
                        <div class="selected-pill" style="display:flex;align-items:center;gap:8px;"><i
                                class="fa-solid fa-layer-group"></i><span id="selectedType">Overall</span></div>
                        <div class="selected-pill" style="display:flex;align-items:center;gap:8px;"><i
                                class="fa-solid fa-calendar"></i><span id="selectedDate">Overall</span></div>
                    </div>
                </div>
            </div>

            <div class="visual-panel" style="display: block; visibility: visible;">
                <div class="stats-top">
                    <div style="font-weight:700;">Data Statistics</div>
                    <div class="muted">Interactive visualizations by group</div>
                </div>

                <div id="statsContainer" class="printable" style="position:relative; display:block; visibility:visible;">
                    ...
                    <div class="charts-grid" style="display:grid; visibility:visible;">
                        <!-- Group 1 Section -->
                        <div class="group-section group-1">
                            ...
                            <div class="chart-card small-kpi span-4">
                                <div class="kpi-top">
                        ...
                        <div class="group-section group-2">
                            ...
                            <div class="chart-card large-pie span-3">
                            ...
                            <div class="chart-card small-kpi span-3">
                                <div class="kpi-top">
                                    <div id="passTopPercent" class="kpi-percent">—</div>
                                </div>
                                <div class="donut-wrapper">
                        ...
                        <div id="deptPassingCard" class="chart-card" style="grid-column: 1 / -1;">
                            <div
                                ...
                                <h4 style="margin: 0;">Board Passing Rate in the College of Engineering Department
                                    (2019–2024)</h4>
                            ...
                            <div id="deptDescription" class="muted"
                                style="display: none; margin-bottom: 16px; font-size: 0.92rem; line-height: 1.5; padding: 14px; background: linear-gradient(145deg, #E2DFDA 0%, #CBDED3 100%); border-radius: 12px; border-left: 4px solid #8BA49A;">
                                <i class="fas fa-chart-line" style="color: #8BA49A; margin-right: 6px;"></i>
                                This visualization shows the overall board exam passing rate trend for the College of
                                Engineering department from 2019 to 2024.
                                The data reflects the percentage of students who successfully passed their respective
                                board examinations across all engineering disciplines during this period.
                            </div>
                            <canvas id="chartDeptPassingRate" height="350"></canvas>
                            <div class="print-caption">Board passing rate in the College of Engineering department,
                                2019–2024</div>
                        </div>
                        <div id="overallTrendsCard" class="chart-card" style="grid-column: 1 / -1;">
                            <div
                                ...
                                <button id="openTrendInfo" class="btn-secondary"
                                    style="padding: 8px 16px; border-radius: 10px; font-size: 0.9rem;">
                                    <i class="fas fa-info-circle"></i> View Details
                                </button>
                            </div>
                            <div id="trendDescription" class="muted"
                                style="display: none; margin-bottom: 16px; font-size: 0.92rem; line-height: 1.5; padding: 14px; background: linear-gradient(145deg, #E2DFDA 0%, #CBDED3 100%); border-radius: 12px; border-left: 4px solid #8BA49A;">
                                <i class="fas fa-chart-bar" style="color: #8BA49A; margin-right: 6px;"></i>
                                This bar chart displays the passing rate trends for different engineering board exam
                                types from 2019 to 2024. Each colored bar represents a specific board exam (EELE, EELE
                                Electronics, REELE, RME), making it easy to compare performance across different
                                disciplines and identify which exams have higher or lower passing rates over the years.
                                This visualization helps administrators quickly identify trends and areas that may need
                        ...
                        <div id="forecastTrendsCard" class="chart-card" style="grid-column: 1 / -1;">
                            ...
                            <div class="print-caption">Linear regression forecast of passing rates per board exam type
                                using latest history.</div>
                        </div>
                        <div id="overallStackedCard" class="chart-card" style="grid-column: 1 / -1;">
                            ...
                            <div id="compositionModeControls"
                                style="display:flex;align-items:center;gap:8px;margin:6px 0 8px 0;">
                                <span class="muted">View:</span>
                                <button id="compModePercent" class="btn-secondary"
                                    style="padding:6px 10px;border-radius:8px;line-height:1;">Percent</button>
                                <button id="compModeCounts" class="btn-secondary"
                                    style="padding:6px 10px;border-radius:8px;line-height:1;">Counts</button>
                            </div>
                            ...
                            <div class="print-caption">Share of passed examinees by board exam type (percent), 2019–2024
                            </div>
                        </div>
                        ...
                        <div id="overallTotalsCard" class="chart-card" style="grid-column: 1 / -1;">
                            ...
                            <div class="print-caption">Total examinees per year, stacked by board exam type, 2019–2024
                            </div>
                        </div>
                        ...
                        <div class="chart-card" style="grid-column: 1 / -1;">
                            ...
                            <div id="subjectsInfo" class="muted" style="margin-bottom:6px;">Select a board exam type and
                                date, then click Confirm to load subjects.</div>
                            <!-- View mode toggle -->
                            <div id="subjectsModeControls"
                                style="display:flex;align-items:center;gap:8px;margin:6px 0 8px 0;">
                                <span class="muted">View:</span>
                                <button id="modeCounts" class="btn-secondary"
                                    style="padding:6px 10px;border-radius:8px;line-height:1;">Counts</button>
                                <button id="modePercent" class="btn-secondary"
                                    style="padding:6px 10px;border-radius:8px;line-height:1;">Percent</button>
                            </div>
                            <!-- Overall subjects mode toggle (shown in overall view) -->
                            <div id="overallSubjectsModeControls"
                                style="display:none;align-items:center;gap:8px;margin:6px 0 8px 0;">
                                <span class="muted">Overall Subjects View:</span>
                                <button id="overallModeCounts" class="btn-secondary"
                                    style="padding:6px 10px;border-radius:8px;line-height:1;">Counts</button>
                                <button id="overallModePercent" class="btn-secondary"
                                    style="padding:6px 10px;border-radius:8px;line-height:1;">Percent</button>
                            </div>
                            <canvas id="chartSubjectsBar" height="260"></canvas>
                            <div class="print-caption">Subjects (Passed vs Failed) for the selected board exam type and
                                date</div>
                        ...
                        <div id="studentsSubjectsModal"
                            style="display:none; position:fixed; left:0; top:0; right:0; bottom:0; background:rgba(2,6,23,0.5); align-items:center; justify-content:center; z-index:21000;">
                            <div
                                style="width:94%; max-width:1100px; background:#fff; border-radius:12px; padding:16px; box-shadow:0 14px 40px rgba(2,6,23,0.12); position:relative;">
                                <button id="closeStudentsSubjects"
                                    style="position:absolute; right:12px; top:12px; background:transparent;border:none;font-size:20px;cursor:pointer;color:#374151">&times;</button>
                                <h3 id="studentsSubjectsTitle" style="margin:0 0 10px 0; font-size:1.1rem;">Student
                                    Subject Grades</h3>
                    ...
                    <div style="margin-top:18px;">
                        <div style="display:flex;gap:12px;flex-wrap:wrap;">
                            ...
                            <div class="chart-card" style="flex:1;min-width:220px;">
                                <h4>Notes</h4>
                                <div class="muted">Click chart slices or bars to highlight corresponding records in the
                                    table (future enhancement).</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="recordsModal"
                        style="display:none; position:fixed; left:0; top:0; right:0; bottom:0; background:rgba(2,6,23,0.5); align-items:center; justify-content:center; z-index:20000;">
                        <div
                                ...
                                <table id="recordsTable"
                                    ...
                                    <thead>
                                        <tr style="background:#f8fafc; position:sticky; top:0;">
                                            <th style="padding:8px;border-bottom:1px solid #e6eef8;text-align:left">Name
                                            </th>
    ...
    function maybeCreateLegendToggle(cardId, chart) {
        try {
            ...
            if (!row) {
                ...
                row.innerHTML =
                    '<button class="btn-secondary" style="padding:6px 10px;border-radius:8px;line-height:1;display:inline-flex;gap:8px;align-items:center;"><i class="fa-solid fa-list"></i> Legend</button><div class="mobile-legend-list" style="margin-top:6px;"></div>';
                card.appendChild(row);
            }
            const btn = row.querySelector('button');
            const list = row.querySelector('.mobile-legend-list');
            const ds = (chart && chart.config && chart.config.data && chart.config.data.datasets) ? chart.config.data
                .datasets : [];
    ...
    function lighten(hex, amt) {
        ...
        try {
            ...
            return '#' + r.toString(16).padStart(2, '0') + g.toString(16).padStart(2, '0') + b.toString(16).padStart(2,
                '0');
        } catch (_) {
            return hex;
        }
    ...
    function darken(hex, amt) {
        return lighten(hex, -Math.abs(amt));
    }
            ...
            tooltip: {
                ...
                callbacks: {
                    ...
                    label: function(context) {
                        ...
                        const value = (context.raw !== undefined) ? context.raw : (context.dataset && context
                            .dataset.data ? context.dataset.data[context.dataIndex] : '');
    ...
    function donutOptions(customCutout) {
        ...
        o.elements = Object.assign({}, o.elements || {}, {
            arc: {
                borderWidth: 2
            }
        });
    ...
    const PIE_LARGE_OPTIONS = (function() {
        const o = JSON.parse(JSON.stringify(COMMON_CHART_OPTIONS));
    ...
    const examTypeCheckboxes = document.querySelectorAll('.exam-type-checkbox');
    ...
    let selectedTypeIds = []; // Array for multiple selections
    let selectedDateId = '';
    ...
    examTypeCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            ...
            selectedTypeIds = Array.from(document.querySelectorAll('.exam-type-checkbox:checked')).map(cb => cb.value);
            ...
            if (window.__overallSubjectCharts && Array.isArray(window.__overallSubjectCharts)) {
                try {
                    window.__overallSubjectCharts.forEach(ch => ch && ch.destroy && ch.destroy());
                } catch (_) {}
            ...
            placeholder.value = '';
            placeholder.textContent = '-- Select exam date --';
            examDateSelect.appendChild(placeholder);
            let has = false;
            const yearsSet = new Set();
            boardExamDates.forEach(d => {
                if (selectedTypeIds.includes(String(d.exam_type_id))) {
                    const opt = document.createElement('option');
                    opt.value = d.id;
                    // Format date to show only Month Year (e.g., "July 2024")
                    let dateDisplay = d.exam_date;
                    try {
                        const dateObj = new Date(d.exam_date + 'T00:00:00');
                        const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 
                                          'July', 'August', 'September', 'October', 'November', 'December'];
                        dateDisplay = monthNames[dateObj.getMonth()] + ' ' + dateObj.getFullYear();
                    } catch (_) {}
                    opt.textContent = dateDisplay + (d.exam_description ? ' — ' + d.exam_description : '');
                    examDateSelect.appendChild(opt);
                    has = true;
                    // collect unique years
                    try {
                        const y = String(d.exam_date).slice(0, 4);
                        if (y) yearsSet.add(y);
                    } catch (_) {}
                }
            });
            ...
            (function renderYearChips() {
                ...
                years.forEach(y => {
                    ...
                    chip.className = 'chip';
                    chip.textContent = y;
                    chip.title = `Show all dates in ${y}`;
                    chip.addEventListener('click', () => {
                        // toggle active
                        wrap.querySelectorAll('.chip').forEach(c => c.classList.remove(
                            'active'));
                        chip.classList.add('active');
                        selectedYear = String(y);
                        selectedDateId = '';
                        examDateSelect.value = '';
                        confirmBtn.disabled = selectedTypeIds.length === 0 || (!selectedDateId && !
                            selectedYear);
                        const selectedTypes = boardExamTypes.filter(x => selectedTypeIds.includes(String(x.id)));
                        const typeNames = selectedTypes.map(t => t.exam_type_name).join(', ');
                        selectedTypeEl.textContent = typeNames || '—';
        ...
        // Use first selected type for now
        ...
        if (selectedTypeId) {
            ...
            fetch(`${base}?action=subjects&boardExamTypeId=${encodeURIComponent(selectedTypeId)}`)
                .then(r => r.json())
                .then(subj => {
                    if (subj && subj.success) {
                        renderSubjectsChart(subj.data || []);
                    } else {
                        renderSubjectsChart([]);
                    }
                })
    ...
    examDateSelect.addEventListener('change', (e) => {
        ...
        if (val && String(val).startsWith('Y:')) {
            selectedYear = String(val).split(':')[1] || '';
            selectedDateId = '';
        } else {
            selectedDateId = val;
            selectedYear = '';
        }
        confirmBtn.disabled = (selectedTypeIds.length === 0 || (!selectedDateId && !selectedYear));
    ...
    confirmBtn.addEventListener('click', () => {
        if (selectedTypeIds.length === 0 || (!selectedDateId && !selectedYear)) return;
        updateSubjectsHeader();
        // call stats endpoints (use first selected type for now)
        const selectedTypeId = selectedTypeIds[0];
        const dateObj = selectedDateId ? boardExamDates.find(x => String(x.id) === String(selectedDateId)) :
            null;
        ...
        if (selectedYear) {
            ...
            pKpis = fetch(
                `${base}?action=kpis&boardExamType=${encodeURIComponent(boardExamType)}&fromDate=${encodeURIComponent(from)}&toDate=${encodeURIComponent(to)}&boardExamTypeId=${encodeURIComponent(selectedTypeId)}`
            ).then(r => r.json());
            pGender = fetch(
                `${base}?action=gender&boardExamType=${encodeURIComponent(boardExamType)}&fromDate=${encodeURIComponent(from)}&toDate=${encodeURIComponent(to)}&boardExamTypeId=${encodeURIComponent(selectedTypeId)}`
            ).then(r => r.json());
            pComp = fetch(
                `${base}?action=composition&boardExamType=${encodeURIComponent(boardExamType)}&fromDate=${encodeURIComponent(from)}&toDate=${encodeURIComponent(to)}&boardExamTypeId=${encodeURIComponent(selectedTypeId)}`
            ).then(r => r.json());
            // subjects endpoint supports examYear for year-wide filtering
            pSubjects = fetch(
                `${base}?action=subjects&boardExamTypeId=${encodeURIComponent(selectedTypeId)}&examYear=${encodeURIComponent(selectedYear)}`
            ).then(r => r.json());
        } else {
            pKpis = fetch(
                `${base}?action=kpis&boardExamType=${encodeURIComponent(boardExamType)}&examDate=${encodeURIComponent(examDate)}&boardExamTypeId=${encodeURIComponent(selectedTypeId)}&examDateId=${encodeURIComponent(selectedDateId)}`
            ).then(r => r.json());
            pGender = fetch(
                `${base}?action=gender&boardExamType=${encodeURIComponent(boardExamType)}&examDate=${encodeURIComponent(examDate)}&boardExamTypeId=${encodeURIComponent(selectedTypeId)}&examDateId=${encodeURIComponent(selectedDateId)}`
            ).then(r => r.json());
            pComp = fetch(
                `${base}?action=composition&boardExamType=${encodeURIComponent(boardExamType)}&examDate=${encodeURIComponent(examDate)}&boardExamTypeId=${encodeURIComponent(selectedTypeId)}&examDateId=${encodeURIComponent(selectedDateId)}`
            ).then(r => r.json());
            pSubjects = fetch(
                `${base}?action=subjects&boardExamTypeId=${encodeURIComponent(selectedTypeId)}&examDateId=${encodeURIComponent(selectedDateId)}`
            ).then(r => r.json());
        }
        ...
        Promise.all([pKpis, pGender, pComp, pSubjects]).then(([kpis, gender, comp, subjects]) => {
            ...
            const deptCard = document.getElementById('deptPassingCard');
            if (deptCard) deptCard.style.display = '';
            const trendCard = document.getElementById('overallTrendsCard');
            if (trendCard) trendCard.style.display = '';
            const forecastCard = document.getElementById('forecastTrendsCard');
            if (forecastCard) forecastCard.style.display = '';
            const stackedCard = document.getElementById('overallStackedCard');
    ...
    function populatePrintHeader() {
        ...
        meta.innerHTML =
            `Type: ${escapeHtml(typeText)} &nbsp; | &nbsp; Date: ${escapeHtml(dateText)} &nbsp; | &nbsp; Generated: ${escapeHtml(ts)}`;
    }
    ...
    clearBtn.addEventListener('click', () => {
        // reset checkboxes
        document.querySelectorAll('.exam-type-checkbox').forEach(cb => cb.checked = false);
    ...
    function updateSubjectsHeader() {
        const hdr = document.getElementById('subjectsHeader');
        if (!hdr) return;
        const selectedTypeId = selectedTypeIds[0]; // Use first selected
        ...
        if (selectedTypeId && !selectedDateId && !selectedYear) {
            const selectedTypes = boardExamTypes.filter(x => selectedTypeIds.includes(String(x.id)));
            const typeNames = selectedTypes.map(t => t.exam_type_name).join(', ');
            hdr.textContent = `Subjects — ${typeNames}`;
            return;
        }
    ...
    let genderPie = null,
        compositionPie = null;
    let femaleDonut = null,
        maleDonut = null;
    let passDonut = null,
        failDonut = null,
        condDonut = null;
    let firstTimerDonut = null,
        repeaterDonut = null;
    let subjectsBar = null;
    let subjectsBarIds = [];
    ...
    function showLoading(show) {
        ...
        if (!overlay) {
            ...
            overlay.innerHTML =
                '<div style="padding:18px;background:#fff;border-radius:8px;box-shadow:0 6px 20px rgba(0,0,0,0.1);font-weight:700;">Loading statistics</div>';
    ...
    function renderGenderPie(arr) {
        ...
        if (!totalSum) {
            ...
            genderPie = updateOrCreateChart(genderPie, document.getElementById('chartGenderPie').getContext('2d'), {
                type: 'pie',
                data: {
                    labels: safeLabels,
                    datasets: [{
        ...
        genderPie = updateOrCreateChart(genderPie, ctx, {
            type: 'pie',
            data: {
                labels,
                datasets: [{
        ...
        ctx.canvas.onclick = function(evt) {
            const points = genderPie.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
            const idx = points[0].index;
            const label = genderPie.data.labels[idx];
    ...
    function renderGenderDonuts(arr) {
        ...
        const female = findBySex(arr, g => g === 'female' || g === 'f' || g.startsWith('f')) || {
            total: 0,
            passed: 0
        };
        const male = findBySex(arr, g => g === 'male' || g === 'm' || g.startsWith('m')) || {
            total: 0,
            passed: 0
        };

        let fTot = Number.parseInt(female.total || 0, 10) || 0,
            fPass = Number.parseInt(female.passed || 0, 10) || 0;
        let mTot = Number.parseInt(male.total || 0, 10) || 0,
            mPass = Number.parseInt(male.passed || 0, 10) || 0;

        // defensive clamp: passed should not exceed total; if it does, clamp and log for investigation
        if (fPass > fTot) {
            console.warn('Gender data inconsistency: female passed > female total', fPass, fTot);
            fPass = fTot;
        }
        if (mPass > mTot) {
            console.warn('Gender data inconsistency: male passed > male total', mPass, mTot);
            mPass = mTot;
        }
        ...
        femaleDonut = updateOrCreateChart(femaleDonut, fCtx, {
            type: 'doughnut',
            data: {
                labels: ['Passed', 'Not Passed'],
                datasets: [{
                    data: fData,
                    backgroundColor: [createVerticalGradient(fCtx, PALETTE[1]), '#f3f4f6'],
                    borderWidth: 2,
                    borderColor: [darken(PALETTE[1], 0.12), '#e5e7eb'],
                    hoverOffset: 8,
                    spacing: 2
                }]
            },
        ...
        (function() {
            ...
            leg.innerHTML =
                `<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:${PALETTE[1]};border-radius:2px;border:1px solid ${darken(PALETTE[1],0.12)};"></span>Passed</span>
    <span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:#f3f4f6;border-radius:2px;border:1px solid #e5e7eb;"></span>Not Passed</span>`;
        ...
        maleDonut = updateOrCreateChart(maleDonut, mCtx, {
            type: 'doughnut',
            data: {
                labels: ['Passed', 'Not Passed'],
                datasets: [{
                    data: mData,
                    backgroundColor: [createVerticalGradient(mCtx, PALETTE[0]), '#f3f4f6'],
                    borderWidth: 2,
                    borderColor: [darken(PALETTE[0], 0.12), '#e5e7eb'],
                    hoverOffset: 8,
                    spacing: 2
                }]
            },
        ...
        (function() {
            ...
            leg.innerHTML =
                `<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:${PALETTE[0]};border-radius:2px;border:1px solid ${darken(PALETTE[0],0.12)};"></span>Passed</span>
    <span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:#f3f4f6;border-radius:2px;border:1px solid #e5e7eb;"></span>Not Passed</span>`;
        })();

        // clicking female/male donut shows passed/not passed records
        fCtx.canvas.onclick = function(evt) {
            const points = femaleDonut.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
        ...
        mCtx.canvas.onclick = function(evt) {
            const points = maleDonut.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
            const i = points[0].index;
    ...
    function renderCompositionPie(arr) {
        ...
        ctx.canvas.onclick = function(evt) {
            const points = compositionPie.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (points.length) {
                const idx = points[0].index;
                const label = compositionPie.data.labels[idx];
                fetchRecords('result', label, `Result: ${label}`, {}, evt);
            }
        };
    }
    ...
    function renderPassFailCondDonuts(arr) {
        ...
        const p = parseInt(passed?.count || 0),
            f = parseInt(failed?.count || 0),
            c = parseInt(cond?.count || 0);
        const total = p + f + c || 0;
        ...
        (function() {
            const c = document.getElementById('chartPassDonut').getContext('2d');
            passDonut = updateOrCreateChart(passDonut, c, {
                type: 'doughnut',
                data: {
                    labels: ['Passed', 'Not Passed'],
                    datasets: [{
                        data: passData,
                        backgroundColor: [createVerticalGradient(c, PALETTE[3]), '#f3f4f6'],
                        borderWidth: 2,
                        borderColor: [darken(PALETTE[3], 0.12), '#e5e7eb'],
                        hoverOffset: 8,
                        spacing: 2,
                        borderRadius: 6
                    }]
                },
                options: donutOptions('66%')
            });
        })();
        (function() {
            const c = document.getElementById('chartFailDonut').getContext('2d');
            failDonut = updateOrCreateChart(failDonut, c, {
                type: 'doughnut',
                data: {
                    labels: ['Failed', 'Not Failed'],
                    datasets: [{
                        data: failData,
                        backgroundColor: [createVerticalGradient(c, PALETTE[5]), '#f3f4f6'],
                        borderWidth: 2,
                        borderColor: [darken(PALETTE[5], 0.12), '#e5e7eb'],
                        hoverOffset: 8,
                        spacing: 2,
                        borderRadius: 6
                    }]
        ...
        (function() {
            const c = document.getElementById('chartCondDonut').getContext('2d');
            condDonut = updateOrCreateChart(condDonut, c, {
                type: 'doughnut',
                data: {
                    labels: ['Conditional', 'Not Conditional'],
                    datasets: [{
                        data: condData,
                        backgroundColor: [createVerticalGradient(c, PALETTE[4]), '#f3f4f6'],
                        borderWidth: 2,
                        borderColor: [darken(PALETTE[4], 0.12), '#e5e7eb'],
                        hoverOffset: 8,
                        spacing: 2,
                        borderRadius: 6
                    }]
        ...
        (function() {
            const wrap = document.getElementById('chartPassDonut').parentElement;
            if (wrap) {
                ...
                leg.innerHTML =
                    `<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:${PALETTE[3]};border-radius:2px;border:1px solid ${darken(PALETTE[3],0.12)};"></span>Passed</span> <span style=\"display:inline-flex;align-items:center;gap:6px;\"><span style=\"display:inline-block;width:12px;height:12px;background:#f3f4f6;border-radius:2px;border:1px solid #e5e7eb;\"></span>Not Passed</span>`;
            }
            const wrapF = document.getElementById('chartFailDonut').parentElement;
            if (wrapF) {
                let leg = wrapF.nextElementSibling;
                if (!leg || !leg.classList || !leg.classList.contains('chart-legend')) {
                    leg = document.createElement('div');
                    leg.className = 'chart-legend';
                    leg.style.justifyContent = 'center';
                    leg.style.marginTop = '4px';
                    wrapF.parentElement.insertBefore(leg, wrapF.nextSibling);
                }
                leg.innerHTML =
                    `<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:${PALETTE[5]};border-radius:2px;border:1px solid ${darken(PALETTE[5],0.12)};"></span>Failed</span> <span style=\"display:inline-flex;align-items:center;gap:6px;\"><span style=\"display:inline-block;width:12px;height:12px;background:#f3f4f6;border-radius:2px;border:1px solid #e5e7eb;\"></span>Not Failed</span>`;
            }
            const wrapC = document.getElementById('chartCondDonut').parentElement;
            if (wrapC) {
                let leg = wrapC.nextElementSibling;
                if (!leg || !leg.classList || !leg.classList.contains('chart-legend')) {
                    leg = document.createElement('div');
                    leg.className = 'chart-legend';
                    leg.style.justifyContent = 'center';
                    leg.style.marginTop = '4px';
                    wrapC.parentElement.insertBefore(leg, wrapC.nextSibling);
                }
                leg.innerHTML =
                    `<span style="display:inline-flex;align-items:center;gap:6px;\"><span style=\"display:inline-block;width:12px;height:12px;background:${PALETTE[4]};border-radius:2px;border:1px solid ${darken(PALETTE[4],0.12)};\"></span>Conditional</span> <span style=\"display:inline-flex;align-items:center;gap:6px;\"><span style=\"display:inline-block;width:12px;height:12px;background:#f3f4f6;border-radius:2px;border:1px solid #e5e7eb;\"></span>Not Conditional</span>`;
            }
        })();

        // add click handlers
        document.getElementById('chartPassDonut').onclick = function(evt) {
            const pts = passDonut.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            ...
            if (String(label).toLowerCase() === 'passed') {
                ...
                fetchRecords('result', 'Other', 'Not Passed', {
                    notResult: 'Passed'
                }, evt);
            }
        };
        ...
        document.getElementById('chartFailDonut').onclick = function(evt) {
            const pts = failDonut.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!pts.length) return;
            const idx = pts[0].index;
            ...
            if (String(label).toLowerCase() === 'failed') {
                ...
                fetchRecords('result', 'Other', 'Not Failed', {
                    notResult: 'Failed'
                }, evt);
            }
        };
        ...
        document.getElementById('chartCondDonut').onclick = function(evt) {
            const pts = condDonut.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!pts.length) return;
            const idx = pts[0].index;
            ...
            if (String(label).toLowerCase().includes('cond')) {
                ...
                fetchRecords('result', 'Other', 'Not Conditional', {
                    notResult: 'Conditional'
                }, evt);
            }
        };
    }
    ...
    function renderFirstRepeaterDonuts(data) {
        ...
        (function() {
            const c = document.getElementById('chartFirstTimerDonut').getContext('2d');
            firstTimerDonut = updateOrCreateChart(firstTimerDonut, c, {
                type: 'doughnut',
                data: {
                    labels: ['Passed', 'Not Passed'],
                    datasets: [{
                        data: firstData,
                        backgroundColor: [createVerticalGradient(c, PALETTE[2]), '#f3f4f6'],
                        borderWidth: 2,
                        borderColor: [darken(PALETTE[2], 0.12), '#e5e7eb'],
                        hoverOffset: 8,
                        spacing: 2
                    }]
                },
        ...
        (function() {
            const c = document.getElementById('chartRepeaterDonut').getContext('2d');
            repeaterDonut = updateOrCreateChart(repeaterDonut, c, {
                type: 'doughnut',
                data: {
                    labels: ['Passed', 'Not Passed'],
                    datasets: [{
                        data: repData,
                        backgroundColor: [createVerticalGradient(c, PALETTE[6]), '#f3f4f6'],
                        borderWidth: 2,
                        borderColor: [darken(PALETTE[6], 0.12), '#e5e7eb'],
                        hoverOffset: 8,
                        spacing: 2
                    }]
                },
        ...
        (function() {
            const wf = document.getElementById('chartFirstTimerDonut').parentElement;
            if (wf) {
                let leg = wf.nextElementSibling;
                if (!leg || !leg.classList || !leg.classList.contains('chart-legend')) {
                    leg = document.createElement('div');
                    leg.className = 'chart-legend';
                    leg.style.justifyContent = 'center';
                    leg.style.marginTop = '4px';
                    wf.parentElement.insertBefore(leg, wf.nextSibling);
                }
                leg.innerHTML =
                    `<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:${PALETTE[2]};border-radius:2px;border:1px solid ${darken(PALETTE[2],0.12)};"></span>Passed</span> <span style=\"display:inline-flex;align-items:center;gap:6px;\"><span style=\"display:inline-block;width:12px;height:12px;background:#f3f4f6;border-radius:2px;border:1px solid #e5e7eb;\"></span>Not Passed</span>`;
            }
            const wr = document.getElementById('chartRepeaterDonut').parentElement;
            if (wr) {
                let leg = wr.nextElementSibling;
                if (!leg || !leg.classList || !leg.classList.contains('chart-legend')) {
                    leg = document.createElement('div');
                    leg.className = 'chart-legend';
                    leg.style.justifyContent = 'center';
                    leg.style.marginTop = '4px';
                    wr.parentElement.insertBefore(leg, wr.nextSibling);
                }
                leg.innerHTML =
                    `<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:12px;height:12px;background:${PALETTE[6]};border-radius:2px;border:1px solid ${darken(PALETTE[6],0.12)};"></span>Passed</span> <span style=\"display:inline-flex;align-items:center;gap:6px;\"><span style=\"display:inline-block;width:12px;height:12px;background:#f3f4f6;border-radius:2px;border:1px solid #e5e7eb;\"></span>Not Passed</span>`;
            }
        })();
        ...
        document.getElementById('chartFirstTimerDonut').onclick = function(evt) {
            const pts = firstTimerDonut.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!pts.length) return;
            const idx = pts[0].index;
        ...
        document.getElementById('chartRepeaterDonut').onclick = function(evt) {
            const pts = repeaterDonut.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!pts.length) return;
            const idx = pts[0].index;
    ...
    // Palette chosen to harmonize with primary sage green (#8BA49A) and magenta (#f472b6)
    // Pastel color palette
    const PALETTE = [
        '#3B6255', // dark sage green (Male)
        '#D2C49E', // light sage green (Female)
        '#8BA49A', // sage green (charts accent)
        '#3B6255', // dark sage green (Passed)
        '#f59e0b', // amber (Conditional)
        '#ef4444', // red (Failed)
        '#D2C49E', // very light sage (accent)
        '#94a3b8' // slate (muted)
    ];
    ...
    // Note: Initial chart rendering moved to loadOverallStats() which is called on DOMContentLoaded
    // This ensures overall data loads automatically instead of showing empty placeholders
    ...
    (function initSubjectsModeToggle() {
        ...
        function refresh() {
            ...
            btnCounts.style.background = isPercent ? '#fff' : '#8BA49A';
            btnCounts.style.color = isPercent ? '#8BA49A' : '#fff';
            btnPercent.style.background = isPercent ? '#8BA49A' : '#fff';
            btnPercent.style.color = isPercent ? '#fff' : '#8BA49A';
        }
        btnCounts.addEventListener('click', () => {
            window.__subjectsMode = 'count';
            refresh();
            renderSubjectsChart(subjectsRawRows);
        });
        btnPercent.addEventListener('click', () => {
            window.__subjectsMode = 'percent';
            refresh();
            renderSubjectsChart(subjectsRawRows);
        });
        refresh();
    ...
    function renderSubjectsChart(rows) {
        ...
        if (!hasData) {
            if (infoEl) {
                ...
                else if (selectedTypeId) infoEl.textContent =
            ...
            options: (function() {
                ...
                o.scales = {
                    x: {
                        ...
                        ticks: (mode === 'percent') ? {
                            precision: 0,
                            stepSize: 10,
                            callback: (v) => Number.isFinite(v) ? (Math.floor(v) + '%') : v
                        } : {
                            precision: 0,
                            stepSize: 1,
                            callback: (v) => Number.isFinite(v) ? Math.floor(v) : v
                        ...
                        title: {
                            display: true,
                            text: (mode === 'percent') ? 'Percentage of examinees' :
                                'Count of examinees'
                        }
                    },
                    y: {
                ...
                if (mode === 'percent') {
                    ...
                    o.plugins.tooltip.callbacks.label = function(context) {
                        ...
                        const cnt = (l === 'passed') ? (passed[idx] || 0) : (l === 'failed' ? (failed[
                            idx] || 0) : (unknown[idx] || 0));
        ...
        ctx.canvas.onclick = function(evt) {
            ...
            const points = subjectsBar.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
            const p = points[0];
            const rowIdx = p.index;
            const dsIdx = p.datasetIndex;
            const subjId = subjectsBarIds[rowIdx] || 0;
            const dsLabel = (subjectsBar.data.datasets[dsIdx] && subjectsBar.data.datasets[dsIdx].label) ? String(
                subjectsBar.data.datasets[dsIdx].label) : '';
            const subjName = labels[rowIdx] || 'Subject';
            if (!subjId || !selectedTypeId) return;
            const ds = dsLabel.toLowerCase();
            const res = ds.includes('pass') ? 'Passed' : (ds.includes('fail') ? 'Failed' : (ds.includes('unknown') ?
                'Unknown' : ''));
            if (res === 'Unknown') {
    ...
    function openStudentsSubjectsDetails(legendLabel) {
        ...
        const q = new URLSearchParams({
            action: 'students_subjects',
            boardExamTypeId: selectedTypeId
        });
    ...
    function renderStudentsSubjectsModal(legendLabel, payload) {
        const modal = document.getElementById('studentsSubjectsModal');
        if (!modal) return;
        ...
        const subHeader = subjects.map(s =>
                `<th style="padding:6px;border-bottom:1px solid #e6eef8;text-align:center">${escapeHtml(s.name)}</th>`)
            .join('');
        let html = '';
        if (!students.length) {
            ...
            students.forEach(st => {
                const subsMap = {};
                (st.subjects || []).forEach(su => {
                    subsMap[String(su.subject_id)] = su;
                });
                let cells = '';
                subjects.forEach(s => {
    ...
    (function() {
        const btn = document.getElementById('closeStudentsSubjects');
        if (btn) {
    ...
    function fetchSubjectRecords(subjectId, subjectName, subjectResult) {
        if (!selectedTypeId) {
            alert('Please select a board exam type first.');
            return;
        }
        const q = new URLSearchParams({
            boardExamTypeId: selectedTypeId,
        ...
        fetch('get_subject_records.php?' + q.toString())
            .then(async r => {
                ...
                try {
                    ...
                    console.error('Subject records non-JSON response:', text);
                    throw e;
                }
            })
            .then(resp => {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && recordsModal.style.display === 'flex') {
            recordsModal.style.display = 'none';
            document.body.style.overflow = (document.body.dataset.prevOverflow || '');
        }
    });
    ...
    function fetchRecords(filterType, filterValue, title, extraFilters = {}, evt) {
        ...
        for (const k in extraFilters) {
            if (Object.prototype.hasOwnProperty.call(extraFilters, k) && extraFilters[k] !== undefined && extraFilters[
                    k] !== null) {
    ...
    function showRecordsModal(title, rows, count, totalCount) {
        ...
        rows.forEach(r => {
            ...
            tr.innerHTML =
                `<td style="padding:8px;border-bottom:1px solid #eef2f7">${escapeHtml(r.full_name)}</td><td style="padding:8px;border-bottom:1px solid #eef2f7;text-align:center">${escapeHtml(r.sex)}</td><td style="padding:8px;border-bottom:1px solid #eef2f7">${escapeHtml(r.course)}</td><td style="padding:8px;border-bottom:1px solid #eef2f7;text-align:center">${escapeHtml(r.year_graduated)}</td><td style="padding:8px;border-bottom:1px solid #eef2f7;text-align:center">${escapeHtml(examDate)}</td><td style="padding:8px;border-bottom:1px solid #eef2f7;text-align:center">${escapeHtml(r.result)}</td>`;
    ...
    function showRecordsPopover(title, rows, count, totalCount, evt) {
        ...
        header.style.background = 'linear-gradient(135deg, #8BA49A 0%, #3B6255 100%)';
        ...
        cls.onmouseover = () => {
            cls.style.background = 'rgba(255, 255, 255, 0.3)';
        };
        cls.onmouseout = () => {
            cls.style.background = 'rgba(255, 255, 255, 0.2)';
        };
        ...
        info.style.color = '#3B6255';
        info.style.background = 'linear-gradient(135deg, #E2DFDA 0%, #CBDED3 100%)';
        info.style.borderBottom = '2px solid rgba(145, 179, 142, 0.1)';
        info.innerHTML =
        ...
        table.innerHTML =
            `<thead><tr style="background: linear-gradient(135deg, #8BA49A 0%, #3B6255 100%); position:sticky; top:0; color: white; box-shadow: 0 4px 15px rgba(145, 179, 142, 0.2);"><th style="padding: 14px 12px; font-weight: 700; text-align:left; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Name</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Sex</th><th style="padding: 14px 12px; font-weight: 700; text-align:left; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Course</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Year Graduated</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Exam Date</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Result</th></tr></thead><tbody></tbody>`;
        ...
        rows.forEach((r, idx) => {
            ...
            tr.innerHTML =
                `<td style="padding: 12px; border-bottom: 1px solid rgba(145, 179, 142, 0.1); font-weight: 600; color: #0f1724;">${escapeHtml(r.full_name)}</td><td style="padding: 12px; border-bottom: 1px solid rgba(145, 179, 142, 0.1); text-align:center; color: #64748b;">${escapeHtml(r.sex)}</td><td style="padding: 12px; border-bottom: 1px solid rgba(145, 179, 142, 0.1); color: #475569; font-size: 0.9rem;">${escapeHtml(r.course)}</td><td style="padding: 12px; border-bottom: 1px solid rgba(145, 179, 142, 0.1); text-align:center; font-weight: 600; color: #3B6255;">${escapeHtml(r.year_graduated)}</td><td style="padding: 12px; border-bottom: 1px solid rgba(145, 179, 142, 0.1); text-align:center; color: #475569;">${escapeHtml(examDate)}</td><td style="padding: 12px; border-bottom: 1px solid rgba(145, 179, 142, 0.1); text-align:center;"><span style="padding: 6px 14px; border-radius: 999px; background: ${resultBg}; color: ${resultColor}; font-weight: 700; font-size: 0.85rem; display: inline-block; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">${escapeHtml(r.result)}</span></td>`;
            tb.appendChild(tr);
        });
        ...
        const rectW = Math.min(pop.offsetWidth || 640, 900);
        const rectH = Math.min(pop.offsetHeight || 400, Math.floor(vh * 0.75));
    ...
    function loadOverallStats() {
        ...
        const pKpis = fetch(`${base}?action=kpis`).then(r => r.json()).catch(e => { console.error('KPIs fetch error:', e); return {success: false}; });
        const pGender = fetch(`${base}?action=gender`).then(r => r.json()).catch(e => { console.error('Gender fetch error:', e); return {success: false}; });
        const pComp = fetch(`${base}?action=composition`).then(r => r.json()).catch(e => { console.error('Composition fetch error:', e); return {success: false}; });
        const pAllSubjects = fetch(`${base}?action=subjects_all`).then(r => r.json()).catch(e => { console.error('Subjects fetch error:', e); return {success: false}; });
        const pDept = fetch(`${base}?action=dept_passing_rate&yearStart=2019&yearEnd=2024`).then(r => r.json()).catch(e => { console.error('Dept passing rate fetch error:', e); return {success: false}; });
        const pTrend = fetch(`${base}?action=trend_passing_rate&yearStart=2019&yearEnd=2024`).then(r => r.json()).catch(e => { console.error('Trend passing rate fetch error:', e); return {success: false}; });
        const pStacked = fetch(`${base}?action=stacked_passing_composition&yearStart=2019&yearEnd=2024`).then(r => r
            .json()).catch(e => { console.error('Stacked fetch error:', e); return {success: false}; });
        const pTotals = fetch(`${base}?action=stacked_totals_by_year&yearStart=2019&yearEnd=2024`).then(r => r.json()).catch(e => { console.error('Totals fetch error:', e); return {success: false}; });
        Promise.all([pKpis, pGender, pComp, pAllSubjects, pDept, pTrend, pStacked, pTotals]).then(([kpis, gender, comp,
            allSubjects, dept, trend, stacked, totals
        ]) => {
            console.log('All data loaded:', {kpis, gender, comp, allSubjects, dept, trend, stacked, totals});
            if (kpis && kpis.success) {
                document.getElementById('kpiTotal').textContent = kpis.data.total + ' total records';
                document.getElementById('kpiPassed').textContent = kpis.data.passed + ' passed';
                renderFirstRepeaterDonuts(kpis.data);
            } else {
                document.getElementById('kpiTotal').textContent = '0 total records';
                document.getElementById('kpiPassed').textContent = '0 passed';
                renderFirstRepeaterDonuts({total: 0, passed: 0, failed: 0, conditional: 0, first_timer: 0, repeater: 0});
            }
            if (gender && gender.success) {
            ...
            const stackedCard = document.getElementById('overallStackedCard');
            if (stackedCard) stackedCard.style.display = '';
            const totalsCard = document.getElementById('overallTotalsCard');
            if (totalsCard) totalsCard.style.display = '';
            if (dept && dept.success) {
                renderDeptPassingRate(dept.data || dept);
            } else {
                renderDeptPassingRate([]);
            }
            if (trend && trend.success) {
                renderPassingTrends(trend.data || trend);
            } else {
                renderPassingTrends([]);
            }
            // render forecasts in separate card using the same years/types from trend payload
            if (trend && trend.success) {
                renderPassingForecast((trend.data || trend));
            } else {
                renderPassingForecast([]);
            }
            if (stacked && stacked.success) {
                renderPassingStacked(stacked.data || stacked);
            } else {
                renderPassingStacked([]);
            }
            if (totals && totals.success) {
                renderTotalsStacked(totals.data || totals);
            } else {
                renderTotalsStacked([]);
            }
            ...
            return fetchSubjectsTotalsForType(g.exam_type_id).then(rows => {
                ...
                if (!rows || !rows.length) {
                    ...
                    rows.forEach(r => {
                        ...
                        segPass.className = 'seg-pass';
                        const segFail = document.createElement('div');
                        segFail.className = 'seg-fail';
                        // initial widths based on current mode (default percent)
                        const mode = (window.__overallSubjectsMode === 'counts') ? 'counts' :
                            'percent';
                        let pw = 0,
                            fw = 0;
                        if (mode === 'percent') {
                            pw = t ? Math.round((p / t) * 100) : 0;
                            fw = t ? Math.round((f / t) * 100) : 0;
                        } else {
                            const maxT = Math.max(1, maxTotal);
                            pw = Math.round((p / maxT) * 100);
                            fw = Math.round((f / maxT) * 100);
                        }
                        ...
                        row.addEventListener('click', (ev) => {
                            if (!sid) return;
                            openRecordsForTypeSubject(g.exam_type_id, sid, name,
                                undefined, ev);
                        });
                ...
                legend.innerHTML =
                    `<span style="display:inline-flex;align-items:center;gap:6px;"><span style="display:inline-block;width:16px;height:8px;background:#4ade80;border-radius:2px;"></span>Passed</span><span style="display:inline-flex;align-items:center;gap:6px;">
    ...
    (function initOverallSubjectsMode() {
        ...
        function applyButtons() {
            ...
            btnC.style.background = isCounts ? '#8BA49A' : '#fff';
            btnC.style.color = isCounts ? '#fff' : '#8BA49A';
            btnP.style.background = isCounts ? '#fff' : '#8BA49A';
            btnP.style.color = isCounts ? '#8BA49A' : '#fff';
        }
        ...
        btnC.addEventListener('click', () => {
            window.__overallSubjectsMode = 'counts';
            applyButtons();
            applyMode();
        });
        btnP.addEventListener('click', () => {
            window.__overallSubjectsMode = 'percent';
            applyButtons();
            applyMode();
        });
        applyButtons();
    ...
    function renderDeptPassingRate(payload) {
        ...
        const el = document.getElementById('chartDeptPassingRate');
        if (!el) return;
        ...
        let cfg = {
            ...
            options: (function() {
                ...
                o.plugins.legend = {
                    ...
                    labels: {
                        ...
                        filter: function(item) {
                            return !/\(Forecast\)$/.test(String(item.text || ''));
                        }
                    }
                };
                o.plugins.tooltip = o.plugins.tooltip || {};
                ...
                o.plugins.tooltip.callbacks.label = function(context) {
                    ...
                    const point = (s.points_details && s.points_details[String(year)]) || {
                        rate: context.raw,
                        passed: 0,
                        total: 0
                    };
        ...
        ctx.canvas.onclick = function(evt) {
            const points = deptPassingChart.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
            const p = points[0];
    ...
    function renderPassingTrends(payload) {
        ...
        const el = document.getElementById('chartPassingTrendLine');
        if (!el) return;
        const ctx = el.getContext('2d');
        const years = (payload && payload.years) ? payload.years : [];
        const series = (payload && payload.series) ? payload.series : [];
        ...
        ctx.canvas.onclick = function(evt) {
            const points = passingTrendChart.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
            const p = points[0];
            ...
            const dsLabel = (passingTrendChart.config.data.datasets[p.datasetIndex] && passingTrendChart.config.data
                .datasets[p.datasetIndex].label) || '';
            // Ignore forecast clicks or future years
            if ((typeof year === 'number' && maxActualYear !== null && year > maxActualYear) || /\(Forecast\)$/
                .test(String(dsLabel))) return;
    ...
    function renderPassingForecast(trendPayload) {
        try {
            const card = document.getElementById('forecastTrendsCard');
            if (card) card.style.display = '';
            const el = document.getElementById('chartPassingForecastLine');
            if (!el) return;
            const ctx = el.getContext('2d');
            ...
            if (!yearsActual.length || !series.length) {
                ...
                passingForecastChart = updateOrCreateChart(passingForecastChart, ctx, {
                    type: 'line',
                    data: {
                        labels: [],
                        datasets: []
                ...
                if (cont) cont.textContent = 'No forecast available.';
                return;
            }
            ...
            const fetches = series.map((s) => {
                ...
                const url =
                    `${base}?action=passing_rate_forecast&yearStart=${encodeURIComponent(firstYear)}&yearEnd=${encodeURIComponent(lastYear)}&horizon=${encodeURIComponent(horizon)}&boardExamTypeId=${encodeURIComponent(typeId)}`;
            ...
            Promise.all(fetches).then(results => {
                ...
                results.forEach((item, idx) => {
                    ...
                    const mapF = {};
                    fArr.forEach(f => {
                        mapF[String(f.year)] = (f && typeof f.rate !== 'undefined') ? Number(f
                            .rate) : null;
                    });
                    const vals = years.map(y => (mapF.hasOwnProperty(String(y)) ? mapF[String(y)] :
                        null));
                    ...
                    const r2 = (item.res.data.model && item.res.data.model.r2 != null) ? Number(item.res
                        .data.model.r2) : null;
                ...
                const cfg = {
                    ...
                    options: (function() {
                        ...
                        o.plugins.tooltip.callbacks = o.plugins.tooltip.callbacks || {};
                        o.plugins.tooltip.callbacks.label = function(context) {
                            const dsLabel = (context && context.dataset && context.dataset
                ...
                if (cont) {
                    if (!datasets.length) {
                        cont.textContent = 'No forecast available.';
                    } else {
                        const r2Avg = r2List.length ? Math.round((r2List.reduce((a, b) => a + b, 0) / r2List
                            .length) * 100) / 100 : null;
    ...
    function renderPassingStacked(payload) {
        ...
        const el = document.getElementById('chartPassingStackedArea');
        if (!el) return;
        const ctx = el.getContext('2d');
        const years = (payload && payload.years) ? payload.years : [];
        const series = (payload && payload.series) ? payload.series : [];
            ...
            options: (function() {
                ...
                if (mode === 'counts') {
                    ...
                    years.forEach(y => {
                        let sum = 0;
                        series.forEach(s => {
                            sum += (s.passed_by_year && s.passed_by_year[String(y)]) ?
                                Number(s.passed_by_year[String(y)]) : 0;
                        });
                ...
                o.plugins.tooltip.callbacks.label = function(context) {
                    ...
                    if (mode === 'counts') {
                        ...
                        let total = 0;
                        series.forEach(ss => {
                            total += (ss.passed_by_year && ss.passed_by_year[String(year)]) ?
                                Number(ss.passed_by_year[String(year)]) : 0;
                        });
        ...
        ctx.canvas.onclick = function(evt) {
            const points = passingStackedChart.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
            const p = points[0];
        ...
        (function() {
            const cont = document.getElementById('compositionSummary');
            if (!cont) return;
            ...
            let total = 0;
            series.forEach(s => {
                total += (s.passed_by_year && s.passed_by_year[latest]) ? Number(s.passed_by_year[latest]) :
                    0;
            });
            const parts = series.map((s, i) => {
                const cnt = (s.passed_by_year && s.passed_by_year[latest]) ? Number(s.passed_by_year[
                    latest]) : 0;
                const pct = (s.percent_by_year && s.percent_by_year[latest]) ? Math.round(Number(s
                    .percent_by_year[latest])) : (total ? Math.round((cnt / total) * 100) : 0);
                const color = PALETTE[i % PALETTE.length];
                const valText = (window.__compositionMode === 'counts') ? `${cnt} passed` :
                    `${pct}% (${cnt})`;
    ...
    function renderTotalsStacked(payload) {
        ...
        const el = document.getElementById('chartTotalsStackedBar');
        if (!el) return;
        const ctx = el.getContext('2d');
        const years = (payload && payload.years) ? payload.years : [];
        const series = (payload && payload.series) ? payload.series : [];
        ...
        const datasets = series.map((s, i) => ({
            label: s.label,
            data: years.map(y => (s.totals_by_year && s.totals_by_year[String(y)]) ? Number(s
                .totals_by_year[String(y)]) : 0),
            backgroundColor: PALETTE[i % PALETTE.length],
            borderColor: PALETTE[i % PALETTE.length],
            stack: 'stack1'
        }));
        ...
        years.forEach(y => {
            let sum = 0;
            series.forEach(s => {
                sum += (s.totals_by_year && s.totals_by_year[String(y)]) ? Number(s.totals_by_year[
                    String(y)]) : 0;
            });
            if (sum > maxSum) maxSum = sum;
        });
        const cfg = {
            ...
            options: (function() {
                ...
                o.plugins.tooltip.callbacks.label = function(context) {
                    const s = series[context.datasetIndex];
                    const year = years[context.dataIndex];
                    const cnt = (s.totals_by_year && s.totals_by_year[String(year)]) ? Number(s
                        .totals_by_year[String(year)]) : 0;
                    // compute share for the year
                    let total = 0;
                    series.forEach(ss => {
                        total += (ss.totals_by_year && ss.totals_by_year[String(year)]) ?
                            Number(ss.totals_by_year[String(year)]) : 0;
                    });
                    const share = total ? Math.round((cnt / total) * 100) : 0;
        ...
        ctx.canvas.onclick = function(evt) {
            const points = totalsStackedChart.getElementsAtEventForMode(evt, 'nearest', {
                intersect: true
            }, true);
            if (!points.length) return;
            const p = points[0];
        ...
        (function() {
            const cont = document.getElementById('totalsSummary');
            if (!cont) return;
            if (!years.length || !series.length) {
                cont.textContent = 'No data.';
                return;
            ...
            let total = 0;
            series.forEach(s => {
                total += (s.totals_by_year && s.totals_by_year[latest]) ? Number(s.totals_by_year[latest]) :
                    0;
            });
            const parts = series.map((s, i) => {
                const cnt = (s.totals_by_year && s.totals_by_year[latest]) ? Number(s.totals_by_year[
                    latest]) : 0;
                const share = total ? Math.round((cnt / total) * 100) : 0;
                const color = PALETTE[i % PALETTE.length];
            ...
            cont.innerHTML = parts.join('') + (total ?
                ` <span class="muted" style="margin-left:8px;white-space:nowrap;">Total in ${latest}: <strong>${total}</strong></span>` :
                '');
        })();
    }
    ...
    (function initCompositionToggle() {
        ...
        function applyButtons() {
            ...
            btnP.style.background = isPercent ? '#8BA49A' : '#fff';
            btnP.style.color = isPercent ? '#fff' : '#8BA49A';
            btnC.style.background = isPercent ? '#fff' : '#8BA49A';
            btnC.style.color = isPercent ? '#8BA49A' : '#fff';
        }
        btnP.addEventListener('click', () => {
        ...
        btnC.addEventListener('click', () => {
            window.__compositionMode = 'counts';
            applyButtons();
            if (lastStackedPayload) renderPassingStacked(lastStackedPayload);
        });
        applyButtons();
    ...
    // Ensure visual elements are visible on page load and show overall data automatically
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set initial "Overall" labels
        if (selectedTypeEl) selectedTypeEl.textContent = 'Overall';
        if (selectedDateEl) selectedDateEl.textContent = 'Overall';
        
        // Load overall data immediately on page load (no filters applied)
        loadOverallStats();
    });
