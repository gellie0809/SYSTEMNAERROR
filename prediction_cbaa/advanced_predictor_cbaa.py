"""
Advanced Board Exam Passing Rate Predictor for CBAA Department
...
class CBAABoardExamPredictor:
    def __init__(self):
        ...
        self.department = 'Business Administration and Accountancy'
    ...
    def fetch_cbaa_anonymous_data(self):
        ...
        try:
            query = """
                ...
                AND department = 'Business Administration and Accountancy'
            ...
            print(f"✅ Fetched {len(df)} CBAA anonymous records")
            return df
    ...
    def prepare_features(self, df):
        ...
        if 'exam_month' in df.columns and df['exam_month'].notna().any():
            ...
            df['exam_month_num'] = 5  # Default to May (typical CPALE month)
    ...
    def train_all_models(self):
        ...
        print("\n🤖 Starting CBAA Board Exam Prediction Training...")
        print("=" * 70)
        
        # Fetch data
        df = self.fetch_cbaa_anonymous_data()
        if df is None or len(df) == 0:
            print("❌ No CBAA data available for training")
            return False
    ...
    def generate_visualizations(self, X_test, y_test):
        ...
        colors = ['#AA4C0A' if m == self.best_model_name else '#E08600' for m in models]
        
        bars = ax.barh(models, r2_scores, color=colors, edgecolor='#763A12', linewidth=2)
        ax.set_xlabel('R² Score', fontsize=12, fontweight='bold')
        ax.set_title('CBAA Model Comparison - R² Scores', fontsize=14, fontweight='bold', pad=20)
        ax.set_xlim(0, 1.1)
        ...
        bars = ax.bar(models, accuracies, color='#E08600', edgecolor='#763A12', linewidth=2)
        ax.set_ylabel('Accuracy (%)', fontsize=12, fontweight='bold')
        ax.set_title('CBAA Model Accuracy Comparison', fontsize=14, fontweight='bold', pad=20)
        ax.set_ylim(0, 100)
        ...
        bars = ax.bar(models, mae_scores, color='#AA4C0A', edgecolor='#763A12', linewidth=2)
        ax.set_ylabel('Mean Absolute Error (%)', fontsize=12, fontweight='bold')
        ax.set_title('CBAA Model Error Comparison (Lower is Better)', fontsize=14, fontweight='bold', pad=20)
        plt.xticks(rotation=45, ha='right')
        ...
        ax.scatter(y_test, y_pred, alpha=0.6, s=100, color='#E08600', edgecolors='#763A12', linewidth=2)
        ...
        ax.set_title(f'CBAA Predictions vs Actual - {self.best_model_name}', 
        ...
        axes[0].hist(residuals, bins=15, color='#E08600', edgecolor='#763A12', linewidth=2, alpha=0.7)
        ...
        axes[1].scatter(y_pred, residuals, alpha=0.6, s=100, color='#AA4C0A', edgecolors='#763A12', linewidth=2)
        ...
        plt.suptitle(f'CBAA Residual Analysis - {self.best_model_name}', fontsize=14, fontweight='bold', y=1.02)
        plt.tight_layout()
        ...
        if self.training_data is not None and len(self.training_data) > 0:
            ...
            ax.set_title('CBAA Historical Passing Rate Trends', fontsize=14, fontweight='bold', pad=20)
