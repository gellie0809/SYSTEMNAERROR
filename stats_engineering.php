...
// Helper to dynamically bind params with references (required by mysqli)
function bindParams($stmt, $params) {
  ...
  // Build array of references: first the types string, then each value by reference
  $refs = [];
  $refs[] = &$types;
...
// accept authoritative ids from client (prefer these when present)
...
// If IDs were provided, translate them to canonical values and prefer those filters
if ($boardExamTypeId !== '') {
...
// debug flag to include SQL/params in responses when needed
$debug = isset($_GET['debug']) && ($_GET['debug'] === '1' || $_GET['debug'] === 'true');

try {
  // Lightweight signature endpoint for polling clients to detect DB changes quickly
  if ($action === 'signature') {
    // Compose a compact signature based on max row id, latest exam date and total count
  ...
  if ($action === 'trend') {
    // Monthly trend for last 12 months by default if no date filters
  ...
  if ($action === 'gender') {
    // Use sex column from DB but present it as 'gender' to the frontend
  ...
  if ($action === 'kpis') {
    ...
    // Gender breakdown (reuse gender query)
    $sql = "SELECT COALESCE(NULLIF(TRIM(sex), ''), 'Unknown') AS gender,
                   COUNT(*) AS total,
                   SUM(CASE WHEN result='Passed' THEN 1 ELSE 0 END) AS passed
            FROM board_passers
    ...
    // Repeater / First Timer breakdown
    $sql = "SELECT
  ...
  if ($action === 'subjects') {
    // Subject-level summary: counts per subject for the selected exam type/date.
    // Uses subject result (bps.result or bps.passed) rather than overall passer result.
  ...
  $examYearParam = isset($_GET['examYear']) ? intval($_GET['examYear']) : 0;
    ...
    // Look up canonical exam type name and date string so we can filter board_passers reliably (consistent with other endpoints)
    $typeName = '';
    $dateStr = '';
    $tstmt = $conn->prepare("SELECT exam_type_name FROM board_exam_types WHERE id=? LIMIT 1");
    ...
    // derive from/to dates when year provided
    $fromYear = '';
    $toYear = '';
    if ($examYearParam > 0 && $dateStr === '') { $fromYear = $examYearParam.'-01-01'; $toYear = $examYearParam.'-12-31'; }

    // Build SQL to aggregate subject results over the set of filtered passers.
    // Strategy: derive the filtered passers for the chosen type/date/year, then
    // LEFT JOIN subject results per subject. This lets us include examinees who
    // have no recorded subject row yet ("Unknown"). Use only subjects mapped to the
    // selected exam type for accuracy.
    // Detect available columns in board_passer_subjects so we can build portable expressions
    ...
    // Bind order: se.exam_type_id, then subquery filters
    $stmt = $conn->prepare($sql);
  ...
  if ($action === 'subjects_all') {
    // Overall view: list all subjects grouped by board exam type (Engineering department)
    $sql = "SELECT bet.id AS exam_type_id,
            ...
            LEFT JOIN subjects s ON s.id = se.subject_id AND s.department = 'Engineering'
            WHERE bet.department = 'Engineering'
  ...
  if ($action === 'students_subjects') {
    // Return per-student details and per-subject grades/percentages for the selected exam type and date/year.
    $typeId = isset($_GET['boardExamTypeId']) ? intval($_GET['boardExamTypeId']) : 0;
    $dateId = isset($_GET['examDateId']) ? intval($_GET['examDateId']) : 0;
    $examYearParam = isset($_GET['examYear']) ? intval($_GET['examYear']) : 0;
    $legendFilter = trim($_GET['legendFilter'] ?? ''); // 'Passed' | 'Failed' | 'Unknown' | ''
    ...
    // Fetch mapped subjects for this type
    $subjects = [];
    $subStmt = $conn->prepare("SELECT s.id, COALESCE(NULLIF(TRIM(s.subject_name),''), CONCAT('Subject ', s.id)) AS subject_name, COALESCE(s.total_items,50) AS total_items
                               ...
                               JOIN subjects s ON s.id = se.subject_id AND s.department='Engineering'
    ...
    // Pull passers and their subject rows in one query (limit reasonably large cohort)
    $inClause = '';
    if (!empty($subjectIds)) {
    ...
    $stmt = $conn->prepare($sql);
    if (!$stmt) { echo json_encode(['success'=>false,'error'=>'Prepare failed']); exit(); }
    // bind params: passers filters then subject ids
    ...
    // Compute summaries and apply legendFilter if any
  ...
  if ($action === 'stacked_passing_composition') {
    ...
    // For each type, get passed counts per year
    $passedByTypeYear = [];
    foreach ($types as $t) {
    ...
    // Compute total passed per year and percent stack per type
    $series = [];
    foreach ($types as $t) {
      $percentByYear = [];
      $passedByYear = [];
  ...
  if ($action === 'stacked_totals_by_year') {
    // New: Total examinees per year stacked by board exam type (all results)
    $yearStart = isset($_GET['yearStart']) ? intval($_GET['yearStart']) : 2019;
    $yearEnd = isset($_GET['yearEnd']) ? intval($_GET['yearEnd']) : 2024;
    if ($yearStart > $yearEnd) { $t = $yearStart; $yearStart = $yearEnd; $yearEnd = $t; }
    ...
    // For each type, get TOTAL examinees per year (not just passed)
    $totalsByTypeYear = [];
    foreach ($types as $t) {
  ...
  if ($action === 'dept_passing_rate') {
    // Department-level passing rate per year (all exam types), percentage
    $yearStart = isset($_GET['yearStart']) ? intval($_GET['yearStart']) : 2019;
    $yearEnd = isset($_GET['yearEnd']) ? intval($_GET['yearEnd']) : 2024;
    if ($yearStart > $yearEnd) { $t = $yearStart; $yearStart = $yearEnd; $yearEnd = $t; }
  ...
  if ($action === 'passing_rate_forecast') {
    // Linear regression forecast for board passing rate (department or per exam type)
    $yearStart = isset($_GET['yearStart']) ? intval($_GET['yearStart']) : 2019;
    $yearEnd   = isset($_GET['yearEnd'])   ? intval($_GET['yearEnd'])   : 2024;
    $horizon   = isset($_GET['horizon'])   ? max(1, min(5, intval($_GET['horizon']))) : 2; // predict next 1–5 years
    if ($yearStart > $yearEnd) { $t = $yearStart; $yearStart = $yearEnd; $yearEnd = $t; }
    ...
    // Linear regression on (index, rate)
    $n = count($knownYears);
    $x = []; $y = [];
    ...
    // Forecast next H years after the last known (or after yearEnd if none)
    $startYear = $n ? $knownYears[$n-1] : $yearEnd;
    $forecast = [];
    for ($h=1; $h <= $horizon; $h++) {
      $t = $n - 1 + $h; // extend index
