...
// Allow College of Arts and Sciences admin or ICTS admin
...
$passers = $conn->query("SELECT * FROM anonymous_board_passers WHERE department='Arts and Sciences' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY id DESC");
...
// Check for success/error messages from update/delete
$update_message = '';
$update_type    = '';
if (isset($_GET['success']) && $_GET['success'] === 'updated') {
    ...
    switch ($_GET['error']) {
        case 'missing_fields':
            $update_message = 'Error: Please fill in all required fields';
            break;
        case 'record_not_found':
            $update_message = 'Error: Record not found';
            break;
        case 'no_changes':
            $update_message = 'No changes were made';
            break;
        default:
            $update_message = 'An error occurred: ' . htmlspecialchars($_GET['error']);
    }
}

// Fetch CAS courses for dropdown
$courses_result = $conn->query("SELECT course_name FROM courses WHERE department='Arts and Sciences' ORDER BY course_name ASC");
$courses = [];
...
// Add default CAS programs if none exist
if (empty($courses)) {
    $courses = [
...
// Fetch board exam types for CAS (e.g., LET, Psychometrician, etc.)
...
// Add default CAS licensure exams if none exist
if (empty($board_exam_types)) {
    $board_exam_types = [
        ['id' => 1, 'name' => 'Licensure Examination for Teachers (LET) - Secondary'],
        ['id' => 2, 'name' => 'Psychometrician Licensure Examination'],
        ['id' => 3, 'name' => 'Real Estate Appraiser Licensure Exam'],
        ['id' => 4, 'name' => 'Chemist Licensure Examination'],
        ['id' => 5, 'name' => 'Chemical Technician Licensure Exam']
    ];
}

// Fetch board exam dates for CAS
$board_exam_dates_result = $conn->query("
    SELECT bed.id, bed.exam_date, bed.exam_description, bed.exam_type_id, 
           IFNULL(bet.exam_type_name, 'Unknown Exam') AS exam_type_name
    FROM board_exam_dates bed 
    LEFT JOIN board_exam_types bet ON bed.exam_type_id = bet.id 
    WHERE bed.department = 'Arts and Sciences' 
...
if ($board_exam_dates_result && $board_exam_dates_result->num_rows > 0) {
    while ($row = $board_exam_dates_result->fetch_assoc()) {
        $board_exam_dates[] = [
            'id'             => $row['id'],
            'date'           => $row['exam_date'],
            'description'    => $row['exam_description'] ?: 'Licensure Exam',
            'exam_type_name' => $row['exam_type_name'],
            'exam_type_id'   => $row['exam_type_id'] ? (int)$row['exam_type_id'] : null
        ];
    }
}
?>
...
<head>
    ...
    <title>CAS - BOARD PASSING RATE SYSTEM</title>
    ...
    * {
    ...
    .shortcuts-btn {
        background: rgba(255, 161, 195, 0.08);
        color: #4F0024;
        border: 2px solid rgba(131, 0, 52, 0.35);
        border-radius: 12px;
        padding: 10px 16px;
        font-size: 0.9rem;
        font-weight: 600;
    ...
    .shortcuts-btn:hover {
        background: rgba(255, 161, 195, 0.15);
        border-color: rgba(131, 0, 52, 0.55);
        transform: translateY(-2px) scale(1.02);
        box-shadow: 0 8px 25px rgba(79, 0, 36, 0.2);
    }
    ...
    .board-table th {
        ...
        top: 0;
        z-index: 10;
        box-shadow: 0 2px 8px rgba(255, 161, 195, 0.18);
    ...
    .add-first-btn:hover {
        ...
        background: linear-gradient(135deg, #2563eb, #FF99CC);
    }
    ...
    .filter-section {
        ...
        margin-bottom: 24px;
        overflow: visible;
        box-shadow: 0 4px 16px rgba(255, 153, 204, 0.15);
        position: relative;
        z-index: 10;
    }
    ...
    .filter-header {
        ...
        border-bottom: 1px solid rgba(131, 0, 52, 0.3);
        border-radius: 16px;
    }
    ...
    .toggle-btn {
        ...
        gap: 8px;
        transition: all 0.3s ease;
        box-shadow: 0 2px 8px rgba(131, 0, 52, 0.2);
    }
    ...
    .filter-input[multiple] option:checked {
        background: linear-gradient(135deg, #FF99CC 0%, #0891a8 100%);
        color: white;
        font-weight: 600;
    }
    ...
    .filter-input:focus {
        border-color: #FF99CC;
        box-shadow: 0 0 0 3px rgba(255, 153, 204, 0.25);
    }
    ...
    .search-input:focus {
        border-color: #FF99CC;
        box-shadow: 0 0 0 4px rgba(255, 153, 204, 0.18);
        transform: translateY(-1px);
    }
    ...
    .search-clear {
        ...
        width: 24px;
        height: 24px;
        background: linear-gradient(135deg, #FF99CC 0%, #FEB3DD 100%);
        border-radius: 50%;
        display: flex;
        align-items: center;
        justify-content: center;
    ...
    .search-clear:hover {
        background: linear-gradient(135deg, #FEB3DD 0%, #047a89 100%);
    ...
    .export-main-btn:hover {
        ...
        background: linear-gradient(135deg, #FF99CC 0%, #2563eb 100%);
    }
    ...
    .apply-btn:hover {
        background: linear-gradient(135deg, #4F0024 0%, #830034 100%);
        transform: translateY(-1px);
        box-shadow: 0 6px 16px rgba(131, 0, 52, 0.4);
    }

    .clear-btn {
        background: linear-gradient(135deg, #FFA1C3 0%, #FF6B9D 100%);
        /* Soft Pink */
        color: white;
        box-shadow: 0 4px 12px rgba(255, 161, 195, 0.3);
    }

    .clear-btn:hover {
        background: linear-gradient(135deg, #FF6B9D 0%, #FFA1C3 100%);
        transform: translateY(-1px);
        box-shadow: 0 6px 16px rgba(255, 161, 195, 0.4);
    }

    .export-btn {
        background: linear-gradient(135deg, #FFA1C3 0%, #830034 100%);
        color: white;
        box-shadow: 0 4px 12px rgba(255, 161, 195, 0.2);
        position: relative !important;
    ...
    .export-btn:hover {
        background: linear-gradient(135deg, #830034 0%, #4F0024 100%);
        transform: translateY(-1px);
        box-shadow: 0 6px 16px rgba(131, 0, 52, 0.3);
    }
    ...
    .export-option:hover {
        background: linear-gradient(90deg, #FFF0FC 0%, #FFA1C3 15%);
        transform: translateX(4px);
        box-shadow: inset 0 0 0 1px rgba(131, 0, 52, 0.1);
    }
    ...
    .export-option[data-format="csv"] i {
        color: #830034;
        background: rgba(131, 0, 52, 0.1);
    }

    .export-option[data-format="csv"]:hover i {
        background: rgba(131, 0, 52, 0.2);
    }
    ...
    .export-option[data-format="excel"] i {
        color: #4F0024;
        background: rgba(79, 0, 36, 0.1);
    }

    .export-option[data-format="excel"]:hover i {
        background: rgba(79, 0, 36, 0.2);
    }
    ...
    .export-option[data-format="pdf"] i {
        color: #FFA1C3;
        background: rgba(255, 161, 195, 0.1);
    }

    .export-option[data-format="pdf"]:hover i {
        background: rgba(255, 161, 195, 0.2);
    }
    ...
    #editStudentModal .modal-header {
        background: linear-gradient(135deg, #0ea5b1 0%, #FF99CC 45%, #60a5fa 100%);
        color: #fff;
    }
    ...
    #editStudentModal .tab-header .tab-icon {
        background: linear-gradient(135deg, #FF99CC 0%, #3b82f6 100%);
    ...
    #editStudentModal .form-group select:focus {
        border-color: #FF99CC;
        box-shadow: 0 0 0 4px rgba(255, 153, 204, 0.15);
        transform: translateY(-1px);
    }
    ...
    #editStudentModal .btn-primary {
        background: linear-gradient(135deg, #FF99CC 0%, #0ea5b1 100%);
        color: #fff;
        box-shadow: 0 10px 24px rgba(255, 153, 204, 0.35);
    }

    #editStudentModal .btn-primary:hover {
        background: linear-gradient(135deg, #0d95a5 0%, #0a8e9a 100%);
        box-shadow: 0 14px 30px rgba(255, 153, 204, 0.45);
        transform: translateY(-2px);
    }
    ...
    #editStudentModal .btn-secondary:hover {
        background: rgba(255, 153, 204, 0.08);
        color: #067b8c;
        border-color: #0ea5b1;
    }
    ...
    /* Logout Modal Styles - Beautiful CAS Pink/Maroon Theme Design */
    #logoutModal.modal {
    ...
    #logoutModal .modal-content {
        ...
        box-shadow:
            0 32px 64px -12px rgba(255, 153, 204, 0.25),
    ...
    #logoutModal .modal-content::before {
        ...
        background: linear-gradient(135deg, #FFA1C3 0%, #ec4899 25%, #db2777 50%, #ec4899 75%, #FFA1C3 100%) !important;
    ...
    #logoutModal .modal-header::before {
        ...
        background: linear-gradient(90deg, #FFA1C3 0%, #ec4899 50%, #db2777 100%) !important;
    ...
    #logoutModal .modal-icon {
        ...
        background: linear-gradient(135deg, #FFA1C3 0%, #ec4899 50%, #db2777 100%) !important;
        border-radius: 50% !important;
        display: flex !important;
        align-items: center !important;
        justify-content: center !important;
    ...
    @keyframes iconPulse {
        ...
        50% {
            box-shadow:
                0 25px 50px rgba(236, 72, 153, 0.6),
    ...
    #logoutModal .modal-title {
        ...
        background: linear-gradient(135deg, #ec4899 0%, #830034 100%) !important;
    ...
    #logoutModal .modal-btn.logout-confirm:hover {
        background: linear-gradient(135deg, #9f1239 0%, #ec4899 50%, #db2777 100%) !important;
    ...
    #logoutModal .modal-btn.logout-confirm:active {
        ...
        background: linear-gradient(135deg, #9f1239 0%, #be185d 50%, #ec4899 100%) !important;
    }
    ...
    @keyframes corner-lightning-tr {
        ...
        0%,
        100% {
            background: linear-gradient(45deg, #9f1239, #db2777, #ec4899);
            transform: scale(1);
            opacity: 0.8;
        }

        50% {
            background: linear-gradient(45deg, #FFA1C3, #ec4899, #db2777);
            transform: scale(1.1);
            opacity: 1;
        }
    }
    ...
    #logoutModal::before {
        ...
        background: linear-gradient(225deg, #db2777, #ec4899, #9f1239) !important;
    ...
    #logoutModal::after {
        ...
        width: 40px !important;
        height: 40px !important;
        background: linear-gradient(315deg, #9f1239, #db2777, #ec4899) !important;
    ...
    @keyframes corner-lightning-bl {
        ...
        0%,
        100% {
            background: linear-gradient(225deg, #db2777, #ec4899, #9f1239);
            transform: scale(1);
            opacity: 0.8;
        }

        50% {
            background: linear-gradient(225deg, #ec4899, #9f1239, #FFA1C3);
            transform: scale(1.1);
            opacity: 1;
        }
    }
    ...
    @keyframes corner-lightning-br {
        ...
        0%,
        100% {
            background: linear-gradient(315deg, #9f1239, #db2777, #ec4899);
            transform: scale(1);
            opacity: 0.8;
        }

        50% {
            background: linear-gradient(315deg, #FFA1C3, #ec4899, #db2777);
            transform: scale(1.1);
            opacity: 1;
        }
    }
    ...
    #logoutModal .modal-icon {
        ...
        box-shadow: 0 15px 35px rgba(159, 18, 57, 0.3) !important;
    }
    ...
    #logoutModal .modal-btn.logout-confirm {
        ...
        overflow: hidden !important;
        background: linear-gradient(135deg, #db2777 0%, #9f1239 100%) !important;
        box-shadow: 0 8px 25px rgba(219, 39, 119, 0.3) !important;
    }
    ...
    .export-option-card {
        ...
        position: relative;
        overflow: hidden;
        box-shadow: 0 4px 12px rgba(131, 0, 52, 0.06);
    }
    ...
    .export-option-card::before {
        ...
        width: 100%;
        height: 100%;
        background: linear-gradient(90deg, transparent, rgba(255, 161, 195, 0.08), transparent);
        transition: left 0.6s ease;
    }
    ...
    .export-option-card:hover {
        transform: translateY(-3px) scale(1.02);
        border-color: #FFA1C3;
        box-shadow: 0 12px 30px rgba(131, 0, 52, 0.2);
...
<body>
    ...
    <div class="topbar">
        ...
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <a href="dashboard_icts.php?dept=CAS" class="icts-back-btn" title="Back to ICTS Dashboard">
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #4F0024 0%, #830034 100%);">
          ...
          ?>
            ...
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #FFA1C3 0%, #FF6B9D 100%);">
          ...
          $passing_rate_query = $conn->query("SELECT 
            ...
            FROM anonymous_board_passers WHERE department='Arts and Sciences' AND (is_deleted IS NULL OR is_deleted = 0)");
          ...
          ?>
            ...
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #FF99CC 0%, #FFA1C3 100%);">
                    ...
                    <div class="filter-grid">
                        ...
                        <div class="filter-group">
                            ...
                            <select id="boardExamTypeFilter" class="filter-input" multiple size="4">
                                <?php 
                                $exam_types = $conn->query("SELECT DISTINCT board_exam_type FROM anonymous_board_passers WHERE department='Arts and Sciences' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY board_exam_type");
                        ...
                        <div class="filter-group">
                            ...
                            <select id="examDateFilter" class="filter-input" multiple size="4">
                                <?php 
                                $exam_dates = $conn->query("SELECT DISTINCT board_exam_date FROM anonymous_board_passers WHERE department='Arts and Sciences' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY board_exam_date DESC");
                                        ...
                                        async function loadSubjectsForPair(typeSelectorId, dateSelectorId, containerId,
                                            ...
                                            try {
                                                ...
                                                const resp = await fetch(
                                                    'fetch_subjects_department.php?exam_type_id=' +
        ...
        <div class="modal-content export-confirm-content">
            ...
            <div class="export-confirm-details">
                ...
                <div class="export-detail-item">
                    ...
                    <span id="exportFileName" class="export-detail-value">Cas_Board_Passers.csv</span>
                </div>
            </div>
    ...
    */
    ...
    function showEditConfirmationModal(originalData, newData, formData) {
      ...
      `;
        ...
        <div class="confirm-modal-content" style="
          ...
          border-radius: 24px;
          box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25);
          border: 2px solid rgba(255, 153, 204, 0.12);
          overflow: hidden;
          position: relative;
          max-width: 650px;
          width: 100%;
        ...
        ">
          ...
          <div class="confirm-modal-dark-bg" style="
            background: linear-gradient(135deg, #0ea5b1 0%, #FF99CC 100%);
          ...
          ">
            <div style="background: #f7fafc; padding: 24px; border-radius: 16px; border: 1px solid #e2e8f0; margin-bottom: 24px;">
              <h4 style="color: #2d3748; margin: 0 0 16px 0; font-weight: 700; font-size: 1.1rem;">
                <i class="fas fa-user-graduate" style="margin-right: 8px; color: #FF99CC;"></i>
              ...
              <div style="color: #0ea5b1; margin-bottom: 16px; font-weight: 800; border-top: 1px solid #e2e8f0; padding-top: 16px; letter-spacing: .2px;">
                <i class="fas fa-clipboard-list" style="margin-right: 8px; color: #FF99CC;"></i>
                Summary of Changes:
              </div>
        ...
        Object.keys(originalData).forEach(key => {
            ...
            ">
              ...
              <div style="display: flex; align-items: center; gap: 10px; font-size: 0.92rem;">
                ...
                <i class="fas fa-arrow-right" style="color: #FF99CC; font-size: 0.75rem;"></i>
    ...
    function handleAddFormSubmission() {
        ...
        // Send add request to the existing add_board_passer_CBAA.php for processing
        console.log('Sending AJAX request...');
        fetch('add_board_passer_cas.php', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
    ...
    function showDeleteConfirmationModal(rowData) {
      ...
      `;
        ...
        <div style="
          ...
          border-radius: 24px;
          box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25);
          border: 2px solid rgba(209, 77, 114, 0.1);
          overflow: hidden;
          position: relative;
          max-width: 600px;
          width: 100%;
        ...
        ">
          <!-- Sticky Header -->
          <div style="
            background: linear-gradient(135deg, #D14D72 0%, #A73659 100%);
            padding: 32px 40px 28px;
            color: white;
            position: relative;
            overflow: hidden;
          ...
          <div style="
            ...
            overflow-y: auto;
          ">
            <div style="background: #fce4ec; padding: 24px; border-radius: 16px; border-left: 4px solid #D14D72; margin-bottom: 24px;">
              <h4 style="color: #A73659; margin: 0 0 16px 0; font-weight: 700; font-size: 1.1rem;">
                <i class="fas fa-graduation-cap" style="margin-right: 8px;"></i>
                Record to be deleted:
              </h4>
              
              <div style="background: white; padding: 20px; border-radius: 12px; border: 1px solid #F8BBD0;">
                <div style="display: grid; grid-template-columns: auto 1fr; gap: 12px; align-items: center;">
                  ...
                  <strong style="color: #374151;">Board Exam Date:</strong>
            ...
            <div style="background: #fce4ec; border: 2px solid #D14D72; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
              <div style="display: flex; align-items: flex-start; color: #A73659;">
                ...
                <div>
                  ...
                  <div style="line-height: 1.5;">
                    You are about to permanently remove this board exam record. 
          ...
          ">
            <button id="confirmDeleteBtn" style="
              ...
              min-height: 50px;
            " onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 20px rgba(209, 77, 114, 0.4)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px rgba(209, 77, 114, 0.3)'">
    ...
    function performStudentDeletion(studentId, studentName) {
        ...
        fetch('/delete_board_passer.php', { // lagyan ng slash sa simula
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
    ...
    function performCSVExport(headers, data) {
        try {
            ...
            link.setAttribute('download', `board_passers_cas_${new Date().toISOString().slice(0,10)}.csv`);
    ...
    function performPDFExport(headers, data) {
        try {
          ...
          <html>
          <head>
            <title>Board Passers Report - CBAA</title>
            <style>
          ...
          <body>
            ...
            <div class="header-info">
              <p><strong>Department:</strong> College of CBAA</p>
            ...
            <div class="footer">
              <p>Board Passing Rate System - CBAA Department</p>
            </div>
          </body>
          </html>
        `);
    ...
    function initializeKeyboardShortcuts() {
        ...
        const filename = `Cas_Board_Passers_${timestamp}.${selectedFormat.extension}`;
    ...
    function performCSVExport() {
        ...
        const filename = `Cas_Board_Passers_${getCurrentDateString()}.csv`;
    ...
    function performExcelExport() {
        ...
        const filename = `Cas_Board_Passers_${getCurrentDateString()}.xls`;
    ...
    function confirmLogout(event) {
        ...
        if (modal) {
            ...
            if (yesBtn) {
                ...
                console.log('Yes button made visible');
            }
            ...
            if (noBtn) {
                ...
                console.log('No button made visible');
            }

            if (modalButtons) {
                modalButtons.style.display = 'flex';
                console.log('Modal buttons container set to flex');
            }
    ...
    function exportToCSV(data) {
        ...
        downloadFile(csvContent, 'board_passers_cas.csv', 'text/csv');
    ...
    function exportToExcel(data) {
        ...
        downloadFile(csvContent, 'board_passers_cas.xls', 'application/vnd.ms-excel');
    ...
    function exportToJSON(data) {
        ...
        downloadFile(jsonContent, 'board_passers_cas.json', 'application/json');
    ...
    function exportToPDF(data) {
        ...
        <head>
          <meta charset="UTF-8">
          <title>LSPU CBAA Board Passers Report</title>
          <style>
            @page {
              size: A4 landscape;
              margin: 15mm 20mm;
        ...
        <body>
          ...
          <div class="header">
            ...
            <div class="department">College of CBAA</div>
          ...
          <!-- Footer -->
          <div class="footer">
            <p><strong>Laguna State Polytechnic University - College of CBAA</strong></p>
            <p>Santa Cruz, Laguna | Tel: (049) 501-0010 | Email: CBAA@lspu.edu.ph</p>
    ...
    function exportAsCSV() {
        ...
        downloadFile(csvData, `Cas_Board_Passers_${getCurrentDateString()}.csv`, 'text/csv');
    ...
    function exportAsExcel() {
        ...
        downloadFile(excelData, `Cas_Board_Passers_${getCurrentDateString()}.xls`, 'application/vnd.ms-excel');
    ...
    function generateExcelData(rows) {
        ...
        <html>
          ...
          <body>
            <h2>CBAA Board Passers - ${getCurrentDateString()}</h2>
            <table>
              <thead>
                <tr>
                  <th>Name</th>
...
<html>
<head>
  <title>CBAA Board Passers Report</title>
  <meta charset="UTF-8">
  <style>
...
<body>
  ...
  <div class="footer">
    <strong>Laguna State Polytechnic University - College of CBAA</strong><br>
    Email: CBAA@lspu.edu.ph | Phone: (049) 536-6303 | Website: www.lspu.edu.ph<br>
    Address: Sta. Cruz, Laguna, Philippines
        ...
        const filename = `LSPU_Cas_Board_Passers_${getCurrentDateString()}.html`;
    ...
    ">
        ...
        background: linear-gradient(135deg, #FF99CC 0%, #3182ce 50%, #60a5fa 100%);
        border-bottom: none;
        position: relative;
        color: white;
        padding: 32px 40px 28px;
    ...
    <div id="logoutModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(15,23,42,0.85); backdrop-filter:blur(10px); -webkit-backdrop-filter:blur(10px); z-index:999999; justify-content:center; align-items:center;">
        <div style="background:linear-gradient(145deg,#1e293b,#0f172a); border-radius:20px; padding:40px; max-width:420px; width:90%; box-shadow:0 25px 60px rgba(0,0,0,0.5); text-align:center; border:1px solid rgba(255,255,255,0.1);">
            <div style="width:80px; height:80px; background:linear-gradient(135deg,#8B1538,#6B0F2A); border-radius:50%; margin:0 auto 20px; display:flex; align-items:center; justify-content:center;">
            ...
            <p style="color:#64748b; font-size:13px; margin-bottom:30px; line-height:1.6;">
                You are about to log out from the CAS Admin Dashboard. Any unsaved changes will be lost.
            </p>
            <div style="display:flex; gap:15px; justify-content:center;">
                ...
                <button onclick="window.location.href='logout.php';" style="padding:12px 30px; border:none; border-radius:10px; font-size:15px; font-weight:500; cursor:pointer; background:linear-gradient(135deg,#8B1538,#6B0F2A); color:#fff; display:flex; align-items:center; gap:8px; transition:transform 0.2s;">
