...
/* -------------------------------------------------
   1. SECURITY Ã¢â‚¬â€œ only CBAA admin can access
   ------------------------------------------------- */
...
function ensureDatabaseExists($conn) {
    ...
    if (!$tableCheck || $tableCheck->num_rows == 0) {
        ...
        CREATE TABLE board_passers (
            ...
            department VARCHAR(100) NOT NULL DEFAULT 'Business Administration and Accountancy',
    ...
    try {
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle)) !== FALSE) {
                ...
                if (count($data) < 8) {
                    $errors[] = "Row $rowNumber: Insufficient columns (expected at least 8)";
                    $stats['errors']++;
                    continue;
                }
                ...
                $sex           = strtoupper(trim($data[5]));
                $examType      = trim($data[7] ?? 'First Timer');

                // Optional: school in column 6
                $school = trim($data[6] ?? '');

                if (empty($firstName) || empty($lastName) || empty($course) || empty($boardExamDate)) {
                    $errors[] = "Row $rowNumber: Missing required fields (Name, Course, Date)";
                    $stats['errors']++;
                    continue;
                }
                ...
                if (!in_array($sex, ['MALE', 'FEMALE'])) {
                    $errors[] = "Row $rowNumber: Invalid sex '$sex' Ã¢â‚¬â€œ use MALE or FEMALE";
                    $stats['errors']++;
                    continue;
                }
                ...
                if ($year < 2019 || $year > 2025) {
                    $errors[] = "Row $rowNumber: Year must be 2019Ã¢â‚¬â€œ2025 (got $year)";
                    $stats['errors']++;
                    continue;
                }
                ...
                $recordKey = $fullName . '|' . $course . '|' . $boardExamDate . '|' . $examType;

                // Prevent duplicates in CSV
                if (isset($processedRecords[$recordKey])) {
                    $errors[] = "Row $rowNumber: Duplicate in CSV Ã¢â‚¬â€œ $fullName";
                    $stats['duplicates']++;
                    continue;
                }
                ...
                $checkStmt = $conn->prepare(
                    "SELECT id FROM board_passers WHERE name = ? AND course = ? AND board_exam_date = ? AND department = 'Business Administration and Accountancy'"
                );
                $checkStmt->bind_param("sss", $fullName, $course, $boardExamDate);
                $checkStmt->execute();
                $res = $checkStmt->get_result();
                if ($res->num_rows > 0) {
                    $errors[] = "Row $rowNumber: Already in DB Ã¢â‚¬â€œ $fullName";
                    $stats['duplicates']++;
                    $checkStmt->close();
                    continue;
                }
                ...
                $insertStmt = $conn->prepare(
                    "INSERT INTO board_passers
                     (name, first_name, last_name, middle_name, sex, course, year_graduated, board_exam_date, result, department, exam_type, board_exam_type)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'PASSED', 'Business Administration and Accountancy', ?, ?)"
                );

                $yearGraduated = $year;
                $boardExamType = 'CPA Licensure Examination'; // Default for CBAA

                $insertStmt->bind_param(
                    "ssssssisss",
                    $fullName, $firstName, $lastName, $middleName, $sex, $course, $yearGraduated, $boardExamDate, $examType, $boardExamType
                );

                if ($insertStmt->execute()) {
                    $stats['imported']++;
                } else {
                    $errors[] = "Row $rowNumber: DB insert failed Ã¢â‚¬â€œ " . $insertStmt->error;
                    $stats['errors']++;
                }
                $insertStmt->close();
            }
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES['csvFile'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if ($ext !== 'csv') {
            $error = "Invalid file type Ã¢â‚¬â€œ only CSV allowed.";
        } elseif ($file['size'] > 10_485_760) { // 10 MB
            $error = "File too large (max 10 MB).";
        } else {
            $uploadDir = 'uploads/';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);

            $newName = 'cbaa_import_' . time() . '.csv';
            ...
            if (move_uploaded_file($file['tmp_name'], $dest)) {
                ...
                // Count total CBAA records
                $vq = $conn->query("SELECT COUNT(*) AS cnt FROM board_passers WHERE department = 'Business Administration and Accountancy'");
                $actual = $vq ? $vq->fetch_assoc()['cnt'] : 0;

                if ($stats['imported'] > 0) {
                    $message = "<strong>Success!</strong> Imported <strong>{$stats['imported']}</strong> of <strong>{$stats['total']}</strong> records. Total CBAA records: <strong>$actual</strong>.";
                } else {
                    $error = "No records imported. Check CSV format and errors below.";
                }
...
<head>
    ...
    <title>Import Data - CBAA Dashboard</title>
    ...
    * {
    ...
    /* Button Styles - CBAA Golden Theme */
    .btn {
        width: 100%;
        height: 45px;
    ...
    .import-card {
        ...
        padding: 0;
        box-shadow:
            0 32px 64px rgba(217, 119, 6, 0.15),
    ...
    .import-card::before {
        ...
        right: 0;
        bottom: 0;
        background: linear-gradient(135deg, rgba(217, 119, 6, 0.02) 0%, rgba(245, 158, 11, 0.03) 100%);
        pointer-events: none;
        z-index: 1;
    }
    ...
    .import-card:hover {
        ...
        box-shadow:
            0 40px 80px rgba(217, 119, 6, 0.2),
    ...
    .upload-area:hover {
        border-color: #F59E0B;
        background: linear-gradient(145deg, rgba(255, 255, 255, 1) 0%, rgba(255, 251, 235, 0.95) 100%);
    ...
    .upload-area.dragover {
        border-color: #D97706;
        background: linear-gradient(145deg, rgba(255, 251, 235, 0.98) 0%, rgba(254, 243, 199, 0.95) 100%);
    ...
    /* Import Stats */
    .import-stats {
        background: linear-gradient(145deg, rgba(255, 255, 255, 0.9) 0%, rgba(255, 251, 235, 0.8) 100%);
        border-radius: 20px;
        padding: 30px;
        margin-top: 30px;
    ...
    #importConfirmModal.modal {
        ...
        background: linear-gradient(135deg, rgba(217, 119, 6, 0.95) 0%, rgba(49, 130, 206, 0.9) 100%) !important;
    ...
    #importConfirmModal .modal-content::before {
        ...
        background: linear-gradient(135deg, #F59E0B 0%, #D97706 25%, #B45309 50%, #D97706 75%, #F59E0B 100%) !important;
    ...
    #importConfirmModal .modal-header {
        ...
        border: 2px solid rgba(217, 119, 6, 0.15) !important;
        position: relative !important;
        overflow: hidden !important;
    ...
    #logoutModal .modal-content {
        ...
        border-radius: 28px !important;
        box-shadow:
            0 32px 64px -12px rgba(245, 158, 11, 0.3),
            inset 0 1px 0 rgba(255, 255, 255, 0.9) !important;
    ...
    #logoutModal .modal-content::before {
        ...
        right: -2px !important;
        bottom: -2px !important;
        background: linear-gradient(135deg, #1e40af 0%, #F59E0B 25%, #60a5fa 50%, #F59E0B 75%, #1e40af 100%) !important;
        border-radius: 30px !important;
        z-index: -1 !important;
    ...
    #logoutModal .modal-header::before {
        ...
        background: linear-gradient(90deg, #1e40af 0%, #F59E0B 50%, #60a5fa 100%) !important;
    ...
    #logoutModal .modal-header::after {
        ...
        background: linear-gradient(135deg, rgba(245, 158, 11, 0.15) 0%, rgba(34, 211, 238, 0.08) 100%) !important;
    ...
    #logoutModal .modal-icon {
        ...
        box-shadow:
            ...
            0 0 0 6px rgba(245, 158, 11, 0.25) !important;
        position: relative !important;
        z-index: 1 !important;
    ...
    @keyframes iconPulse {
        ...
        50% {
            box-shadow:
                0 25px 50px rgba(245, 158, 11, 0.65),
    ...
    #logoutModal .modal-icon::before {
        ...
        background: linear-gradient(135deg, #60a5fa, #F59E0B, #1e40af, #2563eb) !important;
        border-radius: 50% !important;
        z-index: -1 !important;
        opacity: 0.6 !important;
    ...
    #logoutModal .modal-btn.logout-confirm:active {
        ...
        background: linear-gradient(135deg, #B45309 0%, #92400E 50%, #D97706 100%) !important;
    }
...
<body>
    <?php include './includes/cbaa_nav.php'; ?>
        ...
        <div class="card">
            <div class="card-header">
                <div class="header-content">
                    ...
                    <div>
                        <h1 class="header-title">Import Board Passers - College of Business Administration and
                            Accountancy</h1>
                        <p class="header-subtitle">Upload CSV files (2019-2024) for College of Business Administration
                            and Accountancy</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-content">
                ...
                <div class="template-section">
                    ...
                    <p class="template-description">Use this format for College of Business Administration and
                        Accountancy board passer data.</p>
    ...
    function downloadTemplate(event) {
        ...
        a.href = url;
        a.download = 'cbaa_board_passers_template.csv';
