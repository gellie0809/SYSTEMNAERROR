...
// Ensure table exists - auto-create if missing (shared table)
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $filename = "CBAA_board_passers_" . date('Y-m-d_His') . ".csv";
        ...
        while ($row = $result->fetch_assoc()) {
            $formattedDate = $row['board_exam_date'] ? date('Y-m-d', strtotime($row['board_exam_date'])) : '';
            $rating = $row['rating'] ? number_format($row['rating'], 2) : '';
            ...
            fputcsv($output, [
                ...
                $row['board_exam_type'] ?? 'CPA Licensure Examination',
                $rating
            ]);
        }
        ...
        $error_message = "No CBAA board passer data found to export.";
    }
}
...
<head>
    ...
    <title>Export Data - CBAA Dashboard</title>
