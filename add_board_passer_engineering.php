...
// Load courses for dropdown (Engineering)
$courses = [];
$course_stmt = $conn->prepare("SELECT course_name FROM courses WHERE department='Engineering' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY course_name");
...
// Load board exam types (Engineering)
$board_exam_types = [];
...
// Load subjects for Engineering (used for per-subject grade entry)
$subjects = [];
// Ensure `total_items` column exists (for older databases where it's missing)
$colCheck = $conn->query("SHOW COLUMNS FROM subjects LIKE 'total_items'");
if ($colCheck && $colCheck->num_rows === 0) {
  // Try to add the column, ignore failure (schema management should ideally be done via migrations)
  $conn->query("ALTER TABLE subjects ADD COLUMN total_items INT NOT NULL DEFAULT 50 AFTER subject_name");
}

// Optional filter: selected exam type id passed via GET or POST (when choosing an exam type on the form)
$selected_exam_type = intval($_REQUEST['selected_exam_type'] ?? 0);
if ($selected_exam_type > 0) {
  // Fetch subjects that are global (no mapping) OR mapped to the selected exam type
...
if ($sub_q) {
  while ($r = $sub_q->fetch_assoc()) {
    $subjects[] = $r; // ['id'=>..., 'subject_name'=>..., 'total_items'=>...]
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // (Subjects inputs removed per user request)
  ...
  // Duplicate check for named students
  elseif (empty($errors) && $examinee_type === 'named') {
    // If board_exam_type was posted as name, try to resolve its id to filter subjects
    $posted_board_exam_type_name = $board_exam_type;
    $posted_board_exam_type_id = 0;
    if (!empty($posted_board_exam_type_name)) {
      $q = $conn->prepare("SELECT id FROM board_exam_types WHERE exam_type_name = ? AND department = 'Engineering' LIMIT 1");
      if ($q) {
    ...
    // Fetch subjects filtered by the resolved exam_type id (if available)
    $subjects = [];
    ...
    // Duplicate-name check intentionally removed to allow multiple records with the same name.
    // Admins may add multiple examinees with identical names (e.g., repeated attempts).
  }

  if (empty($errors)) {
    // Use a transaction so that board_passers and its related subject rows are atomic.
    $conn->begin_transaction();
    // Defensive: some older databases may not have a `result` column on board_passers.
    // Check and prepare the appropriate INSERT statement and bindings.
    ...
    if ($has_result_col) {
      $stmt = $conn->prepare("INSERT INTO board_passers (name, first_name, middle_name, last_name, sex, course, year_graduated, board_exam_date, result, exam_type, board_exam_type, department) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Engineering')");
      ...
      // Fallback: omit `result` column if it doesn't exist
      $stmt = $conn->prepare("INSERT INTO board_passers (name, first_name, middle_name, last_name, sex, course, year_graduated, board_exam_date, exam_type, board_exam_type, department) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Engineering')");
      $year_graduated_int = intval($year_graduated);
    ...
    if ($stmt->execute()) {
      ...
      // Persist per-subject grades/results (if subjects exist)
      if (!empty($subjects)) {
        // Ensure the schema contains subject_id (older DBs may lack this column)
        $colCheckBPS = $conn->query("SHOW COLUMNS FROM board_passer_subjects LIKE 'subject_id'");
        $ins_has_subject_id = false;
        if ($colCheckBPS && $colCheckBPS->num_rows > 0) {
          $ins_has_subject_id = true;
        } else {
          // Try to add the column; if this fails (permissions, older DB), we'll fall back to inserting without subject_id
          if ($conn->query("ALTER TABLE board_passer_subjects ADD COLUMN subject_id INT NULL AFTER board_passer_id")) {
            $ins_has_subject_id = true;
          } else {
            // Could not add column; proceed with fallback
            $ins_has_subject_id = false;
          }
        }

        // Detect whether the BPS table uses 'result' or 'passed' (some installs use 'passed')
  ...
  // Build appropriate INSERT SQL depending on whether subject_id and a result/passed column exist
        ...
        // Determine if the subject result column (if present) is numeric
        ...
        if ($subject_result_col) {
          ...
          if ($colInfoRes && ($colInfo = $colInfoRes->fetch_assoc())) {
            // type like 'tinyint(1)', 'int(11)', 'varchar(10)'
            if (preg_match('/^(tinyint|int|smallint)/i', $colInfo['Type'])) { $subject_result_is_numeric = true; }
          }
        }
        ...
        foreach ($subjects as $s) {
          ...
          // Validate required: grade is always required; result only required when grade is non-numeric and when the DB stores a result column
          if ($grade_field === '') {
            $subject_validation_errors[] = 'All subject grades are required.';
            break;
          }
          // Treat grades as whole numbers (integers) and respect subject's total_items
          ...
          // Determine result based on posted grade and subject's total_items: >=75% => Passed, else Failed
          $resv = null;
          ...
          // If DB expects numeric result, map 'Passed'/'Failed' to 1/0
          $bind_result_value = $resv;
          if ($subject_result_col && $subject_result_is_numeric) {
            $bind_result_value = ($resv === 'Passed') ? 1 : 0;
          }
          // Bind parameters according to prepared INSERT shape
          if ($ins_has_subject_id) {
            if ($subject_result_col) {
              ...
              // board_passer_id, subject_id, grade
              $ins->bind_param('iii', $insert_id, $sid, $g);
            }
          } else {
            if ($subject_result_col) {
              // board_passer_id, grade, result
              if ($subject_result_is_numeric) {
...
<head>
    ...
    <title>Add New Board Examinee - Engineering Dashboard</title>
    <link rel="stylesheet" href="style.css" />
    <link rel="stylesheet" href="css/sidebar.css" />
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
    /* Examinee Type Selection Styles */
    .examinee-type-label {
        position: relative;
    }
    
    .examinee-type-label input[type="radio"] {
        width: 18px;
        height: 18px;
        cursor: pointer;
        accent-color: #8BA49A;
    ...
    .examinee-type-label:hover {
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(139, 164, 154, 0.15) !important;
    }
    ...
    * {
    ...
    .form-container {
        ...
        padding: 0;
        box-shadow:
            0 32px 64px rgba(145, 179, 142, 0.15),
            0 0 0 1px rgba(255, 255, 255, 0.2),
            inset 0 1px 0 rgba(255, 255, 255, 0.8);
        border: 2px solid rgba(145, 179, 142, 0.1);
        overflow: hidden;
        width: 100%;
        max-width: none;
        margin: 0;
    ...
    .form-container::before {
        ...
        right: 0;
        bottom: 0;
        background: linear-gradient(135deg, rgba(139, 164, 154, 0.02) 0%, rgba(59, 98, 85, 0.03) 100%);
        pointer-events: none;
        z-index: 1;
    }
    ...
    .form-icon {
        ...
        font-size: 2.2rem;
        box-shadow:
            0 15px 35px rgba(145, 179, 142, 0.3),
    ...
    .form-icon:hover {
        ...
        box-shadow:
            0 20px 40px rgba(145, 179, 142, 0.4),
    ...
    .tab-btn.active {
        color: #8BA49A;
        background: linear-gradient(135deg, rgba(139, 164, 154, 0.12) 0%, rgba(59, 98, 85, 0.08) 100%);
        backdrop-filter: blur(10px);
    ...
    .tab-btn.active::after {
        ...
        right: 0;
        height: 3px;
        background: linear-gradient(90deg, #8BA49A 0%, #3B6255 100%);
        box-shadow: 0 2px 8px rgba(139, 164, 154, 0.3);
    }
    ...
    .tab-header {
        ...
        text-align: center;
        background: linear-gradient(135deg, rgba(139, 164, 154, 0.05) 0%, rgba(59, 98, 85, 0.03) 100%);
        position: relative;
        overflow: hidden;
        z-index: 2;
    }
    ...
    .tab-icon {
        ...
        font-size: 2rem;
        box-shadow:
            0 15px 35px rgba(139, 164, 154, 0.4),
    ...
    @keyframes iconPulse {
        ...
        100% {
            transform: scale(1);
            box-shadow: 0 15px 35px rgba(139, 164, 154, 0.4);
        }

        50% {
            transform: scale(1.05);
            box-shadow: 0 20px 40px rgba(139, 164, 154, 0.5);
        }
    }

    .tab-icon:hover {
        transform: scale(1.1) rotate(5deg);
        box-shadow: 0 20px 40px rgba(139, 164, 154, 0.6);
    }
    ...
    .form-group label i {
        color: #8BA49A;
        filter: drop-shadow(0 2px 4px rgba(139, 164, 154, 0.2));
        transition: all 0.3s ease;
        font-size: 1.1rem;
    }
    ...
    .form-group textarea:focus {
        ...
        border-color: #8BA49A;
        box-shadow:
            0 0 0 4px rgba(139, 164, 154, 0.15),
            0 8px 25px rgba(139, 164, 154, 0.2);
    ...
    .form-group textarea:hover {
        ...
        box-shadow:
            0 6px 12px rgba(139, 164, 154, 0.15),
            inset 0 1px 0 rgba(255, 255, 255, 0.9);
    }
    ...
    .btn-primary {
        ...
        color: white;
        box-shadow:
            0 8px 25px rgba(139, 164, 154, 0.4),
            inset 0 1px 0 rgba(255, 255, 255, 0.3);
        margin-left: auto;
    ...
    .btn-primary:hover {
        background: linear-gradient(135deg, #3B6255 0%, #2A4A3E 100%);
        transform: translateY(-3px) scale(1.02);
        box-shadow:
            0 12px 35px rgba(139, 164, 154, 0.5),
            inset 0 1px 0 rgba(255, 255, 255, 0.4);
    }
    ...
    .validation-modal .field-list::-webkit-scrollbar-thumb {
        background: linear-gradient(180deg, #8BA49A 0%, #3B6255 100%);
        border-radius: 10px;
    }

    .validation-modal .field-list::-webkit-scrollbar-thumb:hover {
        background: linear-gradient(180deg, #3B6255 0%, #3d5a40 100%);
    }
    ...
    .validation-modal .field-list h4 i {
        color: #8BA49A;
        font-size: 1.1rem;
    }
    ...
    .validation-modal .field-item:hover {
        border-color: #8BA49A;
        box-shadow:
            0 4px 16px rgba(145, 179, 142, 0.2),
            0 0 0 1px #8BA49A;
        transform: translateX(6px);
        background: linear-gradient(90deg, #ffffff 0%, #E2DFDA 100%);
    }
    ...
    .validation-modal .field-list .info-note {
        ...
        gap: 14px;
        box-shadow: 0 2px 8px rgba(145, 179, 142, 0.1);
    }
    ...
    .validation-modal .modal-btn.validation-ok {
        ...
        transition: all 0.3s ease;
        box-shadow:
            0 4px 14px rgba(145, 179, 142, 0.35),
            inset 0 1px 0 rgba(255, 255, 255, 0.3);
        display: flex;
        align-items: center;
        gap: 10px;
    ...
    .validation-modal .modal-btn.validation-ok:hover {
        background: linear-gradient(135deg, #3B6255 0%, #3d5a40 100%);
        transform: translateY(-2px);
        box-shadow:
            0 6px 20px rgba(145, 179, 142, 0.45),
            inset 0 1px 0 rgba(255, 255, 255, 0.4);
    }
    ...
    .receipt-container {
        padding: 18px 20px;
        background: linear-gradient(180deg, #ffffff, #E2DFDA);
    }
    ...
    .receipt-section {
        margin-top: 12px;
        padding-top: 12px;
        border-top: 1px dashed rgba(145, 179, 142, 0.15)
    }
    ...
    #logoutModal .modal-icon::before {
        ...
        background: linear-gradient(135deg, #D2C49E, #8BA49A, #3B6255, #2d5a2e) !important;
        border-radius: 50% !important;
        z-index: -1 !important;
    ...
    .modal-btn.validation-ok:hover {
        ...
        transform: translateY(-2px);
        box-shadow: 0 8px 25px rgba(145, 179, 142, 0.5);
    }

    /* Confirmation Modal Styles - Modern Premium Design */
    .confirmation-modal {
        max-width: 700px;
        width: 95%;
        max-height: calc(100vh - 40px);
        ...
        border-radius: 24px;
        box-shadow:
            0 25px 70px rgba(45, 90, 46, 0.2),
            0 10px 30px rgba(145, 179, 142, 0.15),
            0 0 0 1px rgba(145, 179, 142, 0.08);
    ...
    /* Modern Header */
    .modern-header {
        background: linear-gradient(135deg, #2d5a2e 0%, #4a7350 50%, #3B6255 100%);
        padding: 28px 36px;
        display: flex;
        align-items: center;
        justify-content: space-between;
    ...
    .info-item:hover {
        background: linear-gradient(90deg, rgba(145, 179, 142, 0.04) 0%, transparent 100%);
        margin: 0 -16px;
        padding-left: 20px;
        border-left: 3px solid #8BA49A;
    }
    ...
    .result-pill {
        ...
        padding: 8px 22px;
        background: linear-gradient(135deg, #8BA49A 0%, #6b9c6e 50%, #3B6255 100%);
        color: white;
        border-radius: 25px;
        font-size: 0.85rem;
        font-weight: 700;
    ...
    .btn-confirm {
        ...
        align-items: center;
        gap: 8px;
        box-shadow: 0 4px 16px rgba(145, 179, 142, 0.4);
        position: relative;
        overflow: hidden;
    }
    ...
    .btn-confirm:hover {
        background: linear-gradient(135deg, #7aa77d 0%, #3B6255 50%, #2d5a2e 100%);
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(145, 179, 142, 0.5);
    }
    ...
    .confirmation-modal::-webkit-scrollbar-thumb {
        background: linear-gradient(135deg, rgba(145, 179, 142, 0.5) 0%, rgba(90, 133, 95, 0.5) 100%);
        border-radius: 10px;
        border: 1px solid rgba(255, 255, 255, 0.5);
    }

    .confirmation-modal::-webkit-scrollbar-thumb:hover {
        background: linear-gradient(135deg, rgba(145, 179, 142, 0.7) 0%, rgba(90, 133, 95, 0.7) 100%);
    }
    ...
    .confirmation-modal .modal-header {
        ...
        text-align: center;
        border-bottom: 1px solid rgba(145, 179, 142, 0.15);
        background: linear-gradient(135deg, rgba(240, 249, 240, 0.9) 0%, rgba(248, 252, 248, 0.95) 100%);
    ...
    .receipt-footer {
        ...
        font-size: 1rem;
        color: #64748b;
        border-top: 2px solid rgba(145, 179, 142, 0.2);
    }
    ...
    .modal-btn.confirmation-confirm:hover {
        background: linear-gradient(135deg, #3B6255 0%, #2d5a2e 100%);
        transform: translateY(-2px);
        box-shadow: 0 6px 16px rgba(145, 179, 142, 0.4);
    }
...
<body>
            ...
            <form method="POST" action="">
                ...
                <div id="personalTab" class="tab-content active">
                    ...
                    <div class="tab-footer">
                        <a href="dashboard_engineering.php" class="btn btn-secondary">
                ...
                font-style: italic;
              ">
                                <i class="fas fa-info-circle" style="color: #8BA49A;"></i>
                        ...
                        <div class="form-group" id="customDateGroup" style="display: none;">
                            ...
                            <input type="month" id="custom_board_exam_date" name="custom_board_exam_date"
                                min="2019-01" max="2024-12"
              ...
              ">
                        ...
                        <div class="form-group named-only-field">
                            ...
                            <input type="number" id="rating" name="rating"
                                value="<?= isset($_POST['rating']) ? htmlspecialchars($_POST['rating']) : '' ?>" min="1"
                                max="100" step="0.01" placeholder="Enter percentage (1-100)" required 
    ...
    <div id="validationModal" class="modal" style="display: none;">
        <div class="modal-content validation-modal">
            <div class="modal-header">
                ...
                <p class="modal-subtitle">Required information is missing. Please review and complete the highlighted fields.</p>
            </div>

            <div class="field-list">
                <h4><i class="fas fa-exclamation-circle"></i> Missing Required Fields</h4>
    ...
    <div id="confirmationModal" class="modal" style="display: none; overflow-y: auto;">
        <div class="modal-content confirmation-modal">
            ...
            <div class="modern-header">
                <div class="header-left">
                    ...
                    <div class="header-text">
                        <h3>LSPU Engineering Department</h3>
                ...
                <div class="info-section">
                    ...
                    <div class="info-grid">
                        ...
                        <div class="info-item">
                            ...
                            <span class="info-value" id="confirmSex"></span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Course</span>
                            <span class="info-value" id="confirmCourse"></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="info-section">
                    ...
                    <div class="info-grid">
                        <div class="info-item">
                            ...
                            <span class="info-value" id="confirmYear"></span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Board Exam Date</span>
                            <span class="info-value" id="confirmExamDate"></span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Result</span>
                            <span class="info-value">
                                <span class="result-pill" id="confirmResult"></span>
                            </span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Take Attempts</span>
                            <span class="info-value" id="confirmExamType"></span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Board Exam Type</span>
                            <span class="info-value" id="confirmBoardExamType"></span>
                        </div>
                        <div class="info-item" id="ratingRow" style="display: none;">
                            <span class="info-label">Rating</span>
                            <span class="info-value rating-value" id="confirmRating"></span>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                <button id="confirmCancel" class="btn-cancel">
                    <i class="fas fa-times"></i>
                    Cancel
                </button>
                <button id="confirmAddStudent" class="btn-confirm">
                    <i class="fas fa-check-circle"></i>
    ...
    function updateAvailableDates() {
        ...
        // === CLEAR SUBJECTS UI (same as your original) ===
        ...
        // === FETCH AND RENDER SUBJECTS (exact same logic mo) ===
        async function fetchAndRenderSubjectsForSelectedType() {
            try {
                clearSubjectsUI();

                const typeOption = examTypeSelect.options[examTypeSelect.selectedIndex];
                ...
                const resp = await fetch('fetch_subjects_engineering.php?exam_type_id=' + encodeURIComponent(
                    typeId));
                ...
                subjects.forEach(s => {
                    ...
                    if (document.getElementById('subject_row_' + s.id)) return;
                    ...
                    row.innerHTML = `
                    <input type="hidden" name="subject_id_${s.id}" value="${s.id}">
                    <div style="flex:1;padding:12px;border-radius:10px;border:1px solid #e5e7eb;background:#f8fafc;font-weight:600;">
                        ${s.subject_name}
                    </div>
                    <input type="number" name="subject_grade_${s.id}" id="subject_grade_${s.id}" 
                           placeholder="Grade" min="0" max="${s.total_items || 100}" step="1" required
                           style="width:140px;padding:12px;border-radius:10px;border:1px solid #e5e7eb;">
                    <div style="width:160px;padding:10px;border-radius:10px;border:1px solid #e5e7eb;background:#fff;text-align:center;">
                        <div style="font-size:12px;color:#6b7280;margin-bottom:4px;">Remarks</div>
                        <div id="subject_result_display_${s.id}" style="font-weight:700;"></div>
                        <input type="hidden" name="subject_result_${s.id}" id="subject_result_${s.id}">
                    </div>
                `;

                    subjectsList.appendChild(row);

                    // Real-time Passed/Failed computation (same as yours)
                    const gradeInput = document.getElementById('subject_grade_' + s.id);
                    const maxItems = parseInt(s.total_items || 100, 10);

                    const updateRemark = () => {
                        const val = gradeInput.value;
                        const g = parseInt(val, 10);
                        const resultDisplay = document.getElementById('subject_result_display_' + s.id);
                        const resultHidden = document.getElementById('subject_result_' + s.id);
                        ...
                        if (g > maxItems) gradeInput.value = maxItems;
                        if (g < 0) gradeInput.value = 0;

                        const passed = (g / maxItems) * 100 >= 75;
                        const remark = passed ? 'Passed' : 'Failed';

                        resultDisplay.textContent = remark;
                        resultDisplay.style.color = passed ? '#10b981' : '#ef4444';
                        resultHidden.value = remark;
                    };

                    gradeInput.addEventListener('input', updateRemark);
                    gradeInput.addEventListener('change', updateRemark);
                });
        ...
        // ===================================================================
        // MAIN LOGIC: UPDATE DATES (pinaka-importanteng inayos ko dito)
        // ===================================================================

        // Prevent multiple binding (ang #1 cause ng pagkawala ng dates)
        if (updateAvailableDates._initialized) {
            // Kung nag-run na before, update dates lang, wag na mag-add ulit ng listeners
            // (this fixes the disappearing dates!)
        } else {
            // First time only: bind events ONCE
            examTypeSelect.addEventListener('change', () => {
                clearSubjectsUI();
                updateAvailableDates(); // safe na to, kasi may guard na
            });

            boardExamDateSelect.addEventListener('change', fetchAndRenderSubjectsForSelectedType);

            updateAvailableDates._initialized = true;
        }
        ...
        if (selectedOption && selectedOption.dataset.typeId) {
            ...
            if (availableDates.length > 0) {
                ...
                availableDates.forEach(examDate => {
                    ...
                    opt.value = examDate.date;
                    opt.textContent = new Date(examDate.date + 'T00:00:00').toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long'
                    }) + (examDate.description ? ' - ' + examDate.description : '');
                    boardExamDateSelect.appendChild(opt);
                });

                // Auto-select first date + auto-load subjects (same behavior mo)
    ...
    // Initial call pag load ng page
    document.addEventListener('DOMContentLoaded', updateAvailableDates);
