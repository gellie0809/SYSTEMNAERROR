...
$courses = [];
$course_stmt = $conn->prepare("SELECT course_name FROM courses WHERE department='Engineering' ORDER BY course_name");
...
$board_exam_types = [];
$type_stmt = $conn->prepare("SELECT id, exam_type_name FROM board_exam_types WHERE department='Engineering' ORDER BY exam_type_name");
...
<head>
  ...
  <style>
    .remark-pass { color: #065f46; background: #ecfdf5; padding: 4px 8px; border-radius: 6px; }
    .remark-fail { color: #991b1b; background: #fff1f2; padding: 4px 8px; border-radius: 6px; }
    * {
      margin: 0;
      padding: 0;
    ...
    body {
      background: linear-gradient(135deg, #ecfeff 0%, #cffafe 100%); /* Cyan gradient background */
      margin: 0;
    ...
    .form-header {
      background: linear-gradient(135deg, #1e40af 0%, #06b6d4 100%); /* Navy to Cyan */
      color: white;
      padding: 40px;
      text-align: center;
      position: relative;
    ...
    .tab-btn.active::after {
      ...
      right: 0;
      height: 3px;
      background: linear-gradient(90deg, #06b6d4 0%, #0891a8 100%); /* Cyan gradient */
    ...
    @media (min-width: 1600px) {
      .modal-content { max-width: 760px; width: 56%; }
    }
  ...
  .confirmation-modal { max-width:700px; border-radius:14px; overflow:hidden }
    /* Polished header for confirmation modal */
    .confirmation-modal .modal-header {
      background: linear-gradient(90deg,#1e40af,#06b6d4);
      color:white;
      padding:14px 18px;
      display:flex;align-items:center;gap:12px;border-top-left-radius:14px;border-top-right-radius:14px;box-shadow:0 10px 30px rgba(14, 46, 110, 0.08)
    }
    .confirmation-modal .modal-header .header-icon-badge { width:40px;height:40px;border-radius:10px;background:rgba(255,255,255,0.12);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.1rem;flex:0 0 40px }
    .confirmation-modal .modal-title { margin:0;font-weight:800;font-size:1.05rem }
  .receipt-container { padding: 18px 20px; background: linear-gradient(180deg,#fff,#f8fbff); }
  .receipt-header { display:flex;align-items:center;justify-content:space-between; gap:12px }
  .receipt-header h4 { margin:0;color:#1f2937 }
  .receipt-section { margin-top:12px;padding-top:12px;border-top:1px dashed rgba(44,90,160,0.06) }
  .receipt-row { display:flex;justify-content:space-between;padding:8px 0;align-items:center }
  .receipt-row .label { color:#6b7280;font-weight:700 }
  .receipt-row .value { color:#111827;font-weight:800 }
  .receipt-footer { margin-top:12px;padding-top:12px;border-top:1px solid rgba(0,0,0,0.02);color:#6b7280 }
  .confirmation-modal .modal-buttons { padding:14px 20px; display:flex; gap:12px; justify-content:flex-end }
  .modal-btn.confirmation-confirm { background: linear-gradient(90deg,#10b981,#059669); color:#fff; padding:10px 16px; border-radius:10px; border:none; font-weight:800 }
  .modal-btn.confirmation-cancel { background:transparent; border:1px solid rgba(44,90,160,0.12); padding:10px 16px; border-radius:10px }
    ...
    #logoutModal .modal-icon {
      ...
      background: linear-gradient(135deg, #f97316 0%, #ea580c 50%, #dc2626 100%) !important; /* Orange to Red */
      border-radius: 50% !important;
      display: flex !important;
      align-items: center !important;
      justify-content: center !important;
            ...
            <div class="form-group">
              ...
              <select id="course" name="course" required>
                ...
                <?php foreach($courses as $course): ?>
                  <option value="<?= htmlspecialchars($course) ?>" <?= (isset($_POST['course']) && $_POST['course'] == $course) ? 'selected' : '' ?>>
          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <select id="result" name="result" required>
                ...
                <option value="Passed" <?= (isset($_POST['result']) && $_POST['result'] == 'Passed') ? 'selected' : '' ?>>Passed</option>
                <option value="Failed" <?= (isset($_POST['result']) && $_POST['result'] == 'Failed') ? 'selected' : '' ?>>Failed</option>
                <option value="Conditional" <?= (isset($_POST['result']) && $_POST['result'] == 'Conditional') ? 'selected' : '' ?>>Conditional</option>
              </select>
            </div>
            ...
            <div class="form-group">
              ...
              <select id="exam_type" name="exam_type" required>
                ...
                <option value="First Timer" <?= (isset($_POST['exam_type']) && $_POST['exam_type'] == 'First Timer') ? 'selected' : '' ?>>First Timer</option>
                <option value="Repeater" <?= (isset($_POST['exam_type']) && $_POST['exam_type'] == 'Repeater') ? 'selected' : '' ?>>Repeater</option>
              </select>
            </div>
            ...
            <div class="form-group">
              ...
              <input type="number" id="rating" name="rating" 
                ...
                placeholder="Exam rating/score (optional)">
            </div>
            ...
            <div class="form-group">
              ...
              <select id="board_exam_type" name="board_exam_type" required onchange="updateAvailableDates()">
            ...
            <div id="subjectsContainer" class="form-group full-width" style="grid-column: 1 / -1; display: none;">
              <label>
            ...
            <div id="noSubjectsPlaceholder" class="form-group full-width" style="grid-column: 1 / -1; color: #6b7280; display: none;">
              No subjects available for the selected take attempts/date. Please add subjects in Manage Courses &gt; Subjects.
            </div>
          </div>
    ...
    <div class="modal-content validation-modal">
      ...
      <div class="field-list">
        ...
        <div class="info-note">
          ...
          <p>Fields marked with an asterisk (<strong>*</strong>) are mandatory. Kindly ensure all required information is provided before proceeding to the next step.</p>
        </div>
      </div>
    ...
    <div class="modal-content confirmation-modal">
      ...
      <div class="receipt-container">
        <div class="receipt-header">
          <h4><i class="fas fa-university"></i> LSPU Engineering Department</h4>
      ...
      <div class="modal-buttons" style="padding: 20px 28px; margin-top: 0; background: rgba(248, 250, 252, 0.8); border-top: 2px solid rgba(44, 90, 160, 0.1); border-radius: 0 0 20px 20px;">
    ...
    function updateAvailableDates() {
      ...
      async function fetchAndRenderSubjectsForSelectedType() {
        try {
          ...
          console.debug('fetchAndRenderSubjectsForSelectedType: received', subjects.length, 'subjects for type', typeId);
          ...
          subjects.forEach(s => {
            ...
            title.style.cssText = 'flex:1;padding:12px;border-radius:10px;border:1px solid #e5e7eb;background:#f8fafc;font-weight:600;';
            ...
            grade.step = '1';
            grade.required = true;
            grade.style.cssText = 'width:140px;padding:12px;border-radius:10px;border:1px solid #e5e7eb;';

            // Show computed result as readonly remarks (label + value) and include a hidden input so server receives the value
            const resultWrapper = document.createElement('div');
            resultWrapper.style.cssText = 'width:160px;padding:6px;border-radius:10px;border:1px solid #e5e7eb;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;font-weight:600;';
            ...
            (function(ginput, smax){
              ginput.addEventListener('input', function() {
                ...
                try {
                  ...
                  const disp = document.getElementById('subject_result_display_' + sid);
                  if (hidden && disp) {
                    ...
                    if (!isNaN(gg)) {
                      ...
                      if (rr === 'Passed') disp.classList.add('remark-pass'); else disp.classList.add('remark-fail');
                    }
                  }
                } catch (e) { console.warn('remark compute failed', e); }
              });
        ...
        if (availableDates.length > 0) {
          ...
          if (boardExamDateSelect.options.length > 1) {
            ...
            try {
              ...
              try { fetchAndRenderSubjectsForSelectedType(); } catch (ex) { console.warn('auto-fetch subjects failed', ex); }
            }
          }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      try {
        ...
        if (typeSelect) {
          ...
          const prevDate = '<?= isset($_POST['board_exam_date']) ? htmlspecialchars($_POST['board_exam_date']) : '' ?>';
    ...
    function addStudentDirect() {
      ...
      for (let field of requiredFields) {
        ...
        if (isEmpty) {
          ...
          const label = field.closest('.form-group')?.querySelector('label')?.textContent?.replace('*', '').trim() || field.name || field.id;
          const elementKey = field.name || field.id || '';
          missingFields.push({ name: label, element: elementKey, icon: getFieldIcon(elementKey) });
      ...
      if (boardExamDateSelect && boardExamDateSelect.value) {
        if (boardExamDateSelect.value === 'other') {
          ...
          if (customDateInput && customDateInput.value) {
            ...
            showValidationModal([{ name: 'Board Exam Date', element: 'board_exam_date', icon: 'fas fa-calendar' }], customDateInput);
            return;
          }
          } else {
            // Use predefined date
        ...
        if (year < 2019 || year > 2024) {
          showValidationModal([{ name: 'Board Exam Date (2019-2024)', element: 'board_exam_date', icon: 'fas fa-calendar' }], boardExamDateSelect.value === 'other' ? customDateInput : boardExamDateSelect);
          return;
        }
      }
      ...
      try {
        ...
        gradeInputs.forEach(input => {
          ...
          input.addEventListener('change', function() {
            if (this.value === '') return;
            const parsed = parseInt(this.value.replace(/[^0-9-]/g, ''), 10);
            if (isNaN(parsed)) { this.value = ''; return; }
            ...
            try {
              ...
              const resultSpan = document.getElementById('subject_result_display_' + sid);
              if (resultHidden && resultSpan) {
                ...
                if (!isNaN(g)) {
                  ...
                  else if (res === 'Failed') resultSpan.classList.add('remark-fail');
                }
              }
            } catch (e) { console.warn('Failed to auto-set subject result', e); }
          });
        });
      } catch (err) {
