...
function callPredictionAPI($endpoint, $method = 'GET', $data = null) {
    // Use port 5001 for CCJE to avoid conflicts with Engineering (port 5000)
...
<head>
    ...
    <title>AI Board Exam Predictions - CCJE Anonymous Data</title>
    ...
    <style>
        * {
        ...
        /* CCJE Red colors */
        body .sidebar .logo, html body .sidebar .logo { color: #D32F2F !important; }
        body .sidebar-nav a, html body .sidebar-nav a { color: #800020 !important; }
        body .sidebar-nav i, html body .sidebar-nav i { color: #D32F2F !important; }
...
<body>
    ...
    <div class="topbar">
        <div class="dashboard-title">
            ...
            AI Board Exam Predictions - CCJE Anonymous Data
        </div>
        <a href="anonymous_dashboard_ccje.php" class="btn" style="background: rgba(255,255,255,0.2);">
    ...
    <div class="main">
        <?php if ($predictionsData && $predictionsData['success']): ?>
            ...
            <div class="prediction-card">
                ...
                <div class="btn-group">
                    ...
                    <button onclick="downloadTrainingReport()" class="btn" style="background: linear-gradient(135deg, #800020 0%, #D32F2F 100%);">
                    ...
                    <?php foreach ($predictionsData['data']['predictions'] as $pred): ?>
                        ...
                        <div class="prediction-item">
                            ...
                            <div class="rate-comparison">
                                ...
                                <div>
                                    <i class="fas fa-arrow-right" style="color: #D32F2F; font-size: 1.5rem;"></i>
                                </div>
            ...
            <?php if ($modelInfo && $modelInfo['success']): ?>
                <div class="model-info-card">
                    ...
                    <p style="opacity: 0.95;">
                        ...
                        Training Data: <?php echo $modelInfo['data']['training_records']; ?> anonymous records
                    </p>
                    
                    <div class="model-metrics">
                        <div class="metric">
                            <div class="metric-label">Accuracy (R²)</div>
                            <div class="metric-value"><?php echo number_format($modelInfo['data']['best_model_metrics']['test_r2'], 4); ?></div>
                        </div>
                        <div class="metric">
                            <div class="metric-label">Avg Error</div>
                            <div class="metric-value"><?php echo number_format($modelInfo['data']['best_model_metrics']['test_mae'], 2); ?>%</div>
                        </div>
                        <div class="metric">
                            <div class="metric-label">CV Score</div>
                            <div class="metric-value"><?php echo $modelInfo['data']['best_model_metrics']['cv_mean'] !== null ? number_format($modelInfo['data']['best_model_metrics']['cv_mean'], 3) : 'N/A'; ?></div>
                        </div>
                    </div>
                    
                    <!-- Algorithm Comparison -->
                    <div class="algorithm-comparison">
                        <h4 style="color: #800020; margin-bottom: 12px;">
                ...
                <div class="model-performance">
                    ...
                    <div id="backtestResults" style="margin-top: 20px;">
                        <div style="text-align: center; padding: 20px;">
                            <i class="fas fa-spinner fa-spin" style="color: #800020; font-size: 24px;"></i>
                ...
                <div class="prediction-card">
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #800020; margin: 0;">Model R² Score Comparison</h3>
                            <button onclick="showChartInfo('r2comparison')" style="background: linear-gradient(135deg, #D32F2F 0%, #800020 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(211,47,47,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5001/api/graphs/model_comparison" alt="Model Comparison" style="max-width: 100%; height: auto;"
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #800020; margin: 0;">Performance Metrics</h3>
                            <button onclick="showChartInfo('performance')" style="background: linear-gradient(135deg, #D32F2F 0%, #800020 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(211,47,47,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5001/api/graphs/performance_metrics" alt="Performance Metrics" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                            <i class="fas fa-chart-bar"></i>
                            <p>Graph not available. Please retrain the models.</p>
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #800020; margin: 0;">Actual vs Predicted</h3>
                            <button onclick="showChartInfo('actualvspredicted')" style="background: linear-gradient(135deg, #D32F2F 0%, #800020 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(211,47,47,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5001/api/graphs/actual_vs_predicted" alt="Actual vs Predicted" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #800020; margin: 0;">Residual Analysis</h3>
                            <button onclick="showChartInfo('residuals')" style="background: linear-gradient(135deg, #D32F2F 0%, #800020 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(211,47,47,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5001/api/graphs/residuals" alt="Residuals" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                    ...
                    <div class="graph-container">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
                            <h3 style="color: #800020; margin: 0;">Historical Trend</h3>
                            <button onclick="showChartInfo('historical')" style="background: linear-gradient(135deg, #D32F2F 0%, #800020 100%); color: white; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 6px; transition: all 0.3s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(211,47,47,0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
                                <i class="fas fa-info-circle"></i> What is this?
                            </button>
                        </div>
                        <img src="http://localhost:5001/api/graphs/historical_trend" alt="Historical Trend" style="max-width: 100%; height: auto;"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                        <div class="loading" style="display: none;">
                            <i class="fas fa-chart-line"></i>
                            <p>Graph not available. Please retrain the models.</p>
        ...
        <?php else: ?>
            <div class="error-message">
                ...
                <p style="margin: 8px 0;">The CCJE prediction service is not running or the model needs to be trained.</p>
                <p style="margin: 8px 0;"><strong>Setup Instructions:</strong></p>
                <ol style="margin-left: 20px; margin-top: 8px;">
                    <li>Open a terminal/command prompt</li>
                    <li>Navigate to: <code>C:\laragon\www\SYSTEMNAERROR-3\FINALSYSTEMNAERROR\prediction_ccje\</code></li>
                    <li>Run: <code>setup.bat</code> (first time only)</li>
                    <li>Run: <code>train.bat</code> to train the models on CCJE anonymous data</li>
                    <li>Run: <code>start_api.bat</code> to start the API server on port 5001</li>
                    <li>Refresh this page</li>
                </ol>
                <p style="margin: 8px 0; padding: 12px; background: #fef3c7; border-left: 4px solid #f59e0b; border-radius: 8px;">
                    <strong>Note:</strong> This prediction system uses anonymous board passer data from CCJE and runs on a separate port (5001) to avoid conflicts with the Engineering prediction system (port 5000).
                </p>
            </div>
        <?php endif; ?>
    </div>
    ...
    <script>
        ...
        async function loadBacktestResults() {
            try {
                const response = await fetch('http://localhost:5001/api/backtest?test_year=2023&train_until=2022');
                const data = await response.json();
                
                if (data.success && data.backtest) {
                    const results = data.backtest;
                    const accuracy = results.accuracy || 0;
                    const mae = results.mae || 0;
                    const predictions = results.predictions || [];
                    
                    let html = '<div style="background: linear-gradient(135deg, #FFF5F5 0%, #FFE8E8 100%); border: 3px solid #D32F2F; border-radius: 16px; padding: 25px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(211, 47, 47, 0.15);">';
                    html += `<div style="text-align: center; margin-bottom: 20px;">`;
                    html += `<div style="display: inline-block; background: linear-gradient(135deg, #800020 0%, #D32F2F 100%); color: white; padding: 15px 30px; border-radius: 50px; margin-bottom: 15px; box-shadow: 0 4px 12px rgba(128, 0, 32, 0.3);">`;
                    html += `<i class="fas fa-check-circle" style="margin-right: 10px; font-size: 24px; vertical-align: middle;"></i>`;
                    html += `<span style="font-size: 52px; font-weight: bold; vertical-align: middle;">${accuracy.toFixed(1)}%</span>`;
                    html += `</div>`;
                    html += `<div style="color: #800020; font-size: 16px; font-weight: 600; letter-spacing: 0.5px;">Average Prediction Accuracy</div>`;
                    html += `</div>`;
                    ...
                    if (predictions.length > 0) {
                        ...
                        html += `<tr style="border-bottom: 3px solid #D32F2F; background: linear-gradient(to right, #800020, #D32F2F);">`;
                        ...
                        predictions.forEach(pred => {
                            ...
                            html += `<i class="fas fa-chart-line" style="color: #D32F2F; margin-right: 8px;"></i>`;
                            html += `${pred.exam_type || 'Board Exam'}</td>`;
                            html += `<td style="padding: 14px; text-align: center; font-weight: 700; font-size: 16px; color: #800020;">${(pred.actual || 0).toFixed(2)}%</td>`;
                            html += `<td style="padding: 14px; text-align: center; font-weight: 700; font-size: 16px; color: #1565C0;">${(pred.predicted || 0).toFixed(2)}%</td>`;
                            html += `<td style="padding: 14px; text-align: center; font-weight: 600; font-size: 14px; color: #E08600;">${(pred.error || 0).toFixed(2)}%</td>`;
                            html += `</tr>`;
                        });
                        
                        // Summary row
                        html += `<tr style="background: linear-gradient(135deg, #FFF9E6 0%, #FFE8CC 100%); border-top: 2px solid #D32F2F;">`;
                        html += `<td style="padding: 14px; color: #800020; font-weight: 700; font-size: 14px;">`;
                        html += `<i class="fas fa-calculator" style="margin-right: 8px;"></i>Average Results</td>`;
                        html += `<td style="padding: 14px; text-align: center; font-weight: 700; font-size: 14px; color: #800020;">-</td>`;
                        html += `<td style="padding: 14px; text-align: center; font-weight: 700; font-size: 14px; color: #1565C0;">-</td>`;
                        html += `<td style="padding: 14px; text-align: center; font-weight: 800; font-size: 16px; color: #2E7D32;">${mae.toFixed(2)}% MAE</td>`;
                        html += `</tr>`;
                    ...
                    html += `<div style="margin-top: 20px; padding: 18px; background: linear-gradient(135deg, #FFF9E6 0%, #FFE8CC 100%); border-radius: 12px; border-left: 5px solid #FAD6A5; box-shadow: 0 2px 8px rgba(250, 214, 165, 0.3);">`;
                    html += `<div style="display: flex; align-items: start; gap: 12px;">`;
                    html += `<div style="flex-shrink: 0; width: 40px; height: 40px; background: linear-gradient(135deg, #800020, #D32F2F); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 6px rgba(128, 0, 32, 0.3);">`;
                    html += `<i class="fas fa-lightbulb" style="color: #FAD6A5; font-size: 18px;"></i>`;
                    html += `</div>`;
                    html += `<div style="flex: 1;">`;
                    html += `<p style="margin: 0; color: #5D4037; font-size: 14px; line-height: 1.7; font-weight: 500;">`;
                    html += `<strong style="color: #800020; font-size: 15px;">What this means:</strong><br>`;
                    html += `Our AI model achieved <strong style="color: #800020;">${accuracy.toFixed(1)}%</strong> accuracy when predicting ${data.test_year || 2023} results using only ${data.train_until || 2022} and earlier training data. `;
                    html += `The average prediction error was only <strong style="color: #E08600;">${mae.toFixed(2)}%</strong>. `;
                    html += `This validates that our machine learning model can reliably predict future CCJE board exam passing rates.`;
                    html += `</p>`;
                    html += `</div>`;
                    html += `</div>`;
                    html += `</div>`;
                    ...
                    document.getElementById('backtestResults').innerHTML = 
                        `<div style="padding: 25px; text-align: center; color: #999; background: #FFF5F5; border-radius: 12px; border: 2px dashed #D32F2F;">
                            <i class="fas fa-database" style="font-size: 40px; color: #D32F2F; margin-bottom: 10px;"></i>
                            <p style="margin: 0; font-size: 14px;">Validation data not available. Train the models first.</p>
                        </div>`;
                }
            } catch (error) {
                document.getElementById('backtestResults').innerHTML = 
                    `<div style="padding: 25px; text-align: center; color: #999; background: #FFF5F5; border-radius: 12px; border: 2px dashed #D32F2F;">
                        <i class="fas fa-exclamation-circle" style="font-size: 40px; color: #D32F2F; margin-bottom: 10px;"></i>
                        <p style="margin: 0; font-size: 14px;">Unable to load validation results. API may not be running.</p>
                    </div>`;
            }
        }
        ...
        async function exportToPDF() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    a.href = url;
                    a.download = `LSPU_CCJE_Predictions_${predictions[0].prediction_year}_${new Date().getTime()}.pdf`;
        ...
        async function downloadTrainingReport() {
            ...
            try {
                const response = await fetch('http://localhost:5001/api/ccje/export/training-report', {
                    method: 'GET'
                });
                ...
                if (response.ok) {
                    ...
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = `LSPU_CCJE_Training_Report_${new Date().getTime()}.pdf`;
                    document.body.appendChild(a);
                    a.click();
                    window.URL.revokeObjectURL(url);
                    document.body.removeChild(a);
                    
                    alert('✅ Training Report downloaded successfully!\n\nThis report includes:\n• Complete CCJE anonymous training records\n• Model training process\n• Algorithm comparison\n• Validation results\n• Accuracy metrics\n• Historical validation');
                } else {
                    const error = await response.json();
                    alert('Error: ' + (error.error || 'Failed to generate training report'));
                }
            } catch (error) {
                alert('Error: ' + error.message + '\n\nMake sure the CCJE Python API is running on port 5001.');
            } finally {
                btn.disabled = false;
                btn.innerHTML = originalText;
            }
        ...
        async function retrainModel() {
            if (!confirm('This will retrain all 7 prediction algorithms with the latest CCJE anonymous data. This may take a few minutes. Continue?')) {
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    const bestModel = data.results?.best_model || {};
                    const r2Score = bestModel.r2_score || 'N/A';
                    const scoreText = typeof r2Score === 'number' ? r2Score.toFixed(4) : r2Score;
                    alert('✅ All models retrained successfully!\n\nBest Model: ' + (bestModel.name || 'Unknown') + '\nR² Score: ' + scoreText);
                    location.reload();
                } else {
        ...
        function showChartInfo(chartType) {
            const chartInfoData = {
                'r2comparison': {
                    ...
                    useCase: 'Use this to identify which AI algorithm is most reliable for CCJE board exam predictions.',
                    icon: 'fa-chart-bar'
                },
                'performance': {
                ...
                'historical': {
                    ...
                    explanation: 'This line chart displays CCJE board exam passing rates over multiple years, showing actual historical data and the trend line. This helps identify if performance is improving, declining, or staying stable over time.',
                    howToRead: [
                ...
                <div style="background: white; border-radius: 20px; padding: 40px; max-width: 650px; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 80px rgba(0,0,0,0.5); animation: slideUp 0.3s ease; position: relative;">
                    ...
                    <div style="margin-bottom: 24px;">
                        <h3 style="margin: 0 0 12px 0; color: #0f1724; font-size: 1.1rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-align-left" style="color: #D32F2F;"></i> What This Chart Shows
                        </h3>
                    ...
                    <div style="margin-bottom: 24px;">
                        <h3 style="margin: 0 0 16px 0; color: #0f1724; font-size: 1.1rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-book-reader" style="color: #D32F2F;"></i> How to Read This Chart
                        </h3>
                        <ul style="margin: 0; padding-left: 0; list-style: none;">
                            ${info.howToRead.map(tip => `
                                <li style="margin-bottom: 10px; padding: 12px 16px; background: #f8fafc; border-radius: 8px; border-left: 3px solid #D32F2F; color: #334155; font-size: 0.95rem;">
                                    <i class="fas fa-check-circle" style="color: #D32F2F; margin-right: 8px;"></i>${tip}
                                </li>
                            `).join('')}
                        </ul>
                    </div>
