...
/* -------------------------------------------------
   1. SECURITY – only CAS admin can access
   ------------------------------------------------- */
...
function ensureDatabaseExists($conn) {
    ...
    if (!$tableCheck || $tableCheck->num_rows == 0) {
        ...
        CREATE TABLE board_passers (
            ...
            department VARCHAR(100) NOT NULL DEFAULT 'Arts and Sciences',
...
/* -------------------------------------------------
   3. CSV PROCESSING (identical to Engineering)
   ------------------------------------------------- */
        ...
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle)) !== FALSE) {
                ...
                $checkStmt = $conn->prepare(
                    "SELECT id FROM board_passers WHERE name = ? AND course = ? AND board_exam_date = ?"
                );
    ...
    if ($file['error'] === 0) {
        ...
        if ($ext !== 'csv') {
            ...
            if (move_uploaded_file($file['tmp_name'], $dest)) {
                ...
                $vq = $conn->query("SELECT COUNT(*) AS cnt FROM board_passers WHERE department = 'Arts and Sciences'");
                $actual = $vq ? $vq->fetch_assoc()['cnt'] : 0;

                if ($stats['imported'] > 0) {
                    $message = "Imported {$stats['imported']} / {$stats['total']} records. DB now has $actual CAS records.";
                } else {
...
<head>
    ...
    <title>Import Data - CAS Dashboard</title>
...
<body>
    <?php include './includes/cas_nav.php'; ?>
        ...
        <div class="card">
            <div class="card-header">
                <div class="header-content">
                    ...
                    <div>
                        <h1 class="header-title">Import Board Passers – College of Arts and Science</h1>
                        <p class="header-subtitle">Upload CSV files (2019-2024) for Arts & Sciences passers</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-content">
                ...
                <div class="template-section">
                    ...
                    <p class="template-description">Use this format for College of Arts and Science board passer data.
                    </p>
    ...
    function downloadTemplate(event) {
        ...
        a.href = url;
        a.download = 'cas_board_passers_template.csv';
