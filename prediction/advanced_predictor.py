...
class AdvancedBoardExamPredictor:
    ...
    def train_and_compare_models(self):
        ...
        print("=" * 70)
        print("ADVANCED BOARD EXAM PREDICTION SYSTEM")
    ...
    def _create_comparison_visualizations(self, results_df, y_test, y_pred):
        ...
        # R2 Score comparison
        axes[0, 0].barh(results_df['model'], results_df['test_r2'], color='#8BA49A')
        ...
        # MAE comparison
        axes[0, 1].barh(results_df['model'], results_df['test_mae'], color='#3B6255')
        axes[0, 1].set_xlabel('Mean Absolute Error (%)', fontweight='bold')
        axes[0, 1].set_title('Model Comparison - MAE', fontweight='bold', fontsize=14)
        axes[0, 1].invert_xaxis()
        
        # Cross-validation scores with error bars
        axes[1, 0].barh(results_df['model'], results_df['cv_mean'], 
                       xerr=results_df['cv_std'], color='#CBDED3', edgecolor='#3B6255')
        axes[1, 0].set_xlabel('Cross-Validation Score', fontweight='bold')
        axes[1, 0].set_title('Cross-Validation Performance', fontweight='bold', fontsize=14)
        
        # Actual vs Predicted
        axes[1, 1].scatter(y_test, y_pred, alpha=0.6, color='#8BA49A', s=100, edgecolor='#3B6255')
        ...
        axes[1, 1].set_title(f'{self.best_model_name} - Actual vs Predicted', 
        ...
        # Residual scatter
        axes[0].scatter(y_pred, residuals, alpha=0.6, color='#8BA49A', s=100, edgecolor='#3B6255')
        ...
        axes[1].hist(residuals, bins=15, color='#8BA49A', edgecolor='#3B6255', alpha=0.7)
