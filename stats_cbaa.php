...
if ($boardExamTypeId > 0) {
  $tstmt = $conn->prepare("SELECT exam_type_name FROM board_exam_types WHERE id = ? AND department = 'Business Administration and Accountancy' LIMIT 1");
  if ($tstmt) {
...
if ($examDateId > 0) {
  $dstmt = $conn->prepare("SELECT exam_date FROM board_exam_dates WHERE id = ? AND department = 'Business Administration and Accountancy' LIMIT 1");
  if ($dstmt) {
...
try {
  ...
  if ($action === 'subjects') {
    ...
    $passerWhere = "bp.department = 'Business Administration and Accountancy'";
    $subParams = []; $subTypes = '';
    ...
    $sql = "SELECT s.id AS subject_id,
            ...
            WHERE s.department = 'Business Administration and Accountancy'
  ...
  if ($action === 'subjects_all') {
    $sql = "SELECT bet.id AS exam_type_id, bet.exam_type_name,
            ...
            LEFT JOIN subjects s ON s.id = se.subject_id AND s.department = 'Business Administration and Accountancy'
            WHERE bet.department = 'Business Administration and Accountancy'
  ...
  if (in_array($action, ['trend_passing_rate', 'dept_passing_rate', 'passing_rate_forecast'])) {
    ...
    $filter = "department = 'Business Administration and Accountancy' AND board_exam_date BETWEEN ? AND ?";
