...
// Allow College of Criminal Justice Education admin or ICTS admin
...
$passers = $conn->query("SELECT * FROM anonymous_board_passers WHERE department='Criminal Justice Education' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY id DESC");
...
$courses_result = $conn->query("
    SELECT course_name 
    FROM courses 
    WHERE department='Criminal Justice Education' 
...
// Add default course if none exist in database
if (empty($courses)) {
    $courses = [
...
$board_exam_types_result = $conn->query("
...
// Add default board exam type if none exist in database
if (empty($board_exam_types)) {
    $board_exam_types = [
        ['id' => 17, 'name' => 'Criminologist Licensure Examination (CLE)']
    ];
}
...
$board_exam_dates_result = $conn->query(
    "SELECT 
        bed.id, 
        bed.exam_date, 
        bed.exam_description, 
        bed.exam_type_id, 
        IFNULL(bet.exam_type_name, '') AS exam_type_name 
     FROM board_exam_dates bed 
     LEFT JOIN board_exam_types bet ON bed.exam_type_id = bet.id 
     WHERE bed.department='Criminal Justice Education' 
...
// Add default exam dates if none exist
if (empty($board_exam_dates)) {
    $board_exam_dates = [
        [
            'id' => 50,
            'date' => '2024-10-15',
            'description' => 'CLE October 2024',
            'exam_type_name' => 'Criminologist Licensure Examination (CLE)',
            'exam_type_id' => 17
        ],
        [
            'id' => 51,
            'date' => '2024-04-20',
            'description' => 'CLE April 2024',
            'exam_type_name' => 'Criminologist Licensure Examination (CLE)',
            'exam_type_id' => 17
        ],
        [
...
<head>
    ...
    <title>CCJE Dashboard - BOARD PASSING RATE SYSTEM</title>
    ...
    * {
    ...
    .card h2 {
        ...
        padding: 24px 40px;
        border-radius: 16px;
        box-shadow: 0 12px 40px rgba(211, 47, 47, 0.22);
        text-align: center;
    ...
    .board-table tbody tr:hover {
        background: linear-gradient(90deg, #FDF3E7 0%, #FAD6A5 100%);
        /* Warm cream-peach tint */
    ...
    .status-failed {
        ...
        color: #ffffff;
        border-color: #1D1D1D;
        box-shadow: 0 2px 8px rgba(29, 29, 29, 0.3);
    }

    .status-cond {
        background: linear-gradient(135deg, #FAD6A5 0%, #E6C794 100%);
        /* Peach */
        color: #1D1D1D;
        border-color: #FAD6A5;
        box-shadow: 0 2px 8px rgba(250, 214, 165, 0.3);
    }
    ...
    .exam-repeater {
        background: linear-gradient(135deg, #4A4A4A 0%, #1D1D1D 100%);
        /* Dark Gray */
        color: #ffffff;
        border-color: #4A4A4A;
    ...
    .delete-btn {
        ...
        /* CCJE Dark Red/Maroon */
        color: white !important;
    }

    .delete-btn:hover {
        background: linear-gradient(135deg, #991B1B 0%, #5F0A0A 100%) !important;
        /* Darker CCJE Red */
    ...
    .add-first-btn {
        background: linear-gradient(135deg, #D32F2F, #800020);
        color: white;
        padding: 12px 24px;
        border: none;
        border-radius: 8px;
    ...
    .add-first-btn:hover {
        transform: translateY(-2px);
        box-shadow: 0 8px 15px -3px rgba(211, 47, 47, 0.4);
    ...
    .board-table tbody tr:hover {
        background: linear-gradient(90deg, #FDF3E7 0%, #FAD6A5 100%) !important;
    ...
    @keyframes rowUpdateFlash {
        0% {
            background: linear-gradient(90deg, #FAD6A5 0%, #FDF3E7 100%) !important;
        }
    ...
    .btn-primary:hover {
        background: linear-gradient(135deg, #800020 0%, #5D0016 100%);
        transform: translateY(-2px);
    ...
    .toggle-btn:hover {
        background: linear-gradient(135deg, #5D0016 0%, #4A0011 100%);
        transform: translateY(-1px);
        box-shadow: 0 4px 12px rgba(211, 47, 47, 0.3);
    }
    ...
    .export-main-btn:hover {
        ...
        background: linear-gradient(135deg, #D32F2F 0%, #C1121F 100%);
    }
    ...
    .clear-btn {
        background: linear-gradient(135deg, #1D1D1D 0%, #800020 100%);
        /* Dark to Burgundy */
        color: white;
    ...
    .filter-count {
        margin-left: auto;
        padding: 8px 16px;
        background: linear-gradient(135deg, #FAD6A5 0%, #FDF3E7 100%);
    ...
    .modal-header {
        ...
        text-align: center;
        position: relative;
        background: linear-gradient(135deg, #D32F2F 0%, #800020 50%, #C62828 100%);
        color: white;
    }
    ...
    #logoutModal .modal-header::before {
        ...
        background: linear-gradient(90deg, #800020 0%, #D32F2F 50%, #C62828 100%) !important;
    ...
    #logoutModal .modal-icon::before {
        ...
        background: linear-gradient(135deg, #FAD6A5, #D32F2F, #800020, #C62828) !important;
        border-radius: 50% !important;
        z-index: -1 !important;
    ...
    #logoutModal .modal-btn.logout-confirm:hover {
        background: linear-gradient(135deg, #5D0016 0%, #800020 50%, #A31C1C 100%) !important;
    ...
    #logoutModal .modal-btn.logout-confirm:active {
        ...
        background: linear-gradient(135deg, #5D0016 0%, #A31C1C 50%, #800020 100%) !important;
    }
    ...
    @keyframes corner-lightning-tr {
        ...
        0%,
        100% {
            background: linear-gradient(45deg, #800020, #D32F2F, #FAD6A5);
            transform: scale(1);
            opacity: 0.8;
        }

        50% {
            background: linear-gradient(45deg, #5D0016, #FAD6A5, #D32F2F);
            transform: scale(1.1);
            opacity: 1;
        }
    }
    ...
    #logoutModal::before {
        ...
        background: linear-gradient(225deg, #D32F2F, #FAD6A5, #800020) !important;
    ...
    #logoutModal::after {
        ...
        width: 40px !important;
        height: 40px !important;
        background: linear-gradient(315deg, #800020, #D32F2F, #FAD6A5) !important;
    ...
    @keyframes corner-lightning-bl {
        ...
        0%,
        100% {
            background: linear-gradient(225deg, #D32F2F, #FAD6A5, #800020);
            transform: scale(1);
            opacity: 0.8;
        }

        50% {
            background: linear-gradient(225deg, #FAD6A5, #800020, #5D0016);
            transform: scale(1.1);
            opacity: 1;
        }
    }
    ...
    @keyframes corner-lightning-br {
        ...
        0%,
        100% {
            background: linear-gradient(315deg, #800020, #D32F2F, #FAD6A5);
            transform: scale(1);
            opacity: 0.8;
        }

        50% {
            background: linear-gradient(315deg, #5D0016, #FAD6A5, #D32F2F);
            transform: scale(1.1);
            opacity: 1;
        }
    }
    ...
    #exportModal .excel-btn:hover {
        background: linear-gradient(135deg, #5D0016 0%, #4A0011 100%) !important;
    ...
    #exportModal .pdf-btn:hover {
        background: linear-gradient(135deg, #C1121F 0%, #9D0208 100%) !important;
    ...
    #logoutModal .modal-btn.logout-confirm {
        position: relative !important;
        overflow: hidden !important;
        background: linear-gradient(135deg, #D32F2F 0%, #800020 100%) !important;
...
<body>
    ...
    <div class="topbar">
        <h1 class="dashboard-title">College of Criminal Justice Education Admin Dashboard</h1>
        <div style="display: flex; align-items: center; gap: 12px;">
            <?php if ($is_icts_admin): ?>
            <a href="dashboard_icts.php?dept=CCJE" class="icts-back-btn" title="Back to ICTS Dashboard">
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #D32F2F 0%, #800020 100%);">
            ...
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #C62828 0%, #8B0000 100%);">
                ...
                $passing_rate_query = $conn->query("SELECT
                  ...
                  FROM anonymous_board_passers WHERE department='Criminal Justice Education' AND (is_deleted IS NULL OR is_deleted = 0)");
            ...
            <div class="stat-card">
                ...
                if ($failed_data && $failed_data['total_count'] > 0) {
                  $failed_count = $failed_data['failed_count'];
                ...
                ?>
                    ...
                    <p>Failed Rate</p>
                    <span class="stat-change neutral">
                        <i class="fas fa-user-xmark"></i> <?= $failed_count ?> failed
                    </span>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background: linear-gradient(135deg, #E63946 0%, #C1121F 100%);">
                ...
                $best_course_query = $conn->query("SELECT board_exam_type,
                  ...
                  ORDER BY pass_rate DESC, total_count DESC
                  LIMIT 1");
                ...
                if ($best_course) {
                  $course_rate = round($best_course['pass_rate'], 1);
                  $course_name = $best_course['board_exam_type'];
                }
                ?>
                    <h3><?= $course_rate ?>%</h3>
    ...
    <div style="display:none">
                    ...
                    <div class="filter-grid">
                        ...
                        <div class="filter-group">
                            ...
                            <select id="boardExamTypeFilter" class="filter-input" multiple size="4">
                                <?php 
                                $exam_types = $conn->query("SELECT DISTINCT board_exam_type FROM anonymous_board_passers WHERE department='Criminal Justice Education' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY board_exam_type");
                        ...
                        <div class="filter-group">
                            ...
                            <select id="examDateFilter" class="filter-input" multiple size="4">
                                <?php 
                                $exam_dates = $conn->query("SELECT DISTINCT board_exam_date FROM anonymous_board_passers WHERE department='Criminal Justice Education' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY board_exam_date DESC");
            ...
            <div class="table-container">
                <script>
                // Map board exam type IDs to their display names
                            ...
                            <td colspan="6"
                                ...
                                <div style="display: flex; flex-direction: column; align-items: center; gap: 20px;">
                                    ...
                                    <div>
                                        ...
                                        <p style="color: #6b7280; margin: 0;">Start adding Criminology board passers to
                                            see them here.</p>
                                    </div>
                                </div>
                            </td>
    ...
    <div id="editStudentModal" class="custom-modal" style="display: none;">
        <div class="modal-content add-modal-content">
            <div class="modal-header add-modal-header">
                ...
                <h3>Edit Criminology Board Passer</h3>
            ...
            <form id="editStudentForm">
                ...
                <div class="form-grid" style="padding: 0 24px;">
                    ...
                    <div class="form-group" style="grid-column: 1 / -1;">
                        ...
                        <select id="editBoardExamType" name="board_exam_type" required>
                            ...
                            <option value="<?= (int)$exam_type['id'] ?>"
                                ...
                                data-type-id="<?= (int)$exam_type['id'] ?>">
                                <?= htmlspecialchars($exam_type['name']) ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="editExamDate" name="board_exam_date">
                            ...
                            <?php endforeach; ?>
                            <option value="other">Other Date (Not Listed)</option>
                        </select>
                        <div id="editExamDateHint" class="small-hint" style="display:none;">Select Board Exam Type first
                        </div>
                    </div>
                ...
                <div style="padding: 0 24px 16px; margin-top: 8px;">
                    ...
                    <div id="editNoSubjectsPlaceholder"
                        style="display:none; color:#6b7280; font-size:0.95rem; padding:10px;">
                        No subjects available for selected CLE exam.
                    </div>
                </div>
    ...
    <div id="addStudentModal" class="custom-modal">
        <div class="modal-content add-modal-content">
            <div class="modal-header add-modal-header">
                ...
                <h3>Add New Criminology Board Passer</h3>
            ...
            <div class="tab-navigation">
                ...
                <button class="tab-btn" data-tab="exam" onclick="switchTab('exam')">
                    <i class="fas fa-graduation-cap"></i> Exam Info
                </button>
            </div>
            ...
            <form id="addStudentForm">
                ...
                <div id="personalTab" class="tab-content active">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            <label for="addLastName"><i class="fas fa-user"></i>Last Name *</label>
                            <input type="text" id="addLastName" name="last_name" required placeholder="Dela Cruz">
                        </div>
                        <div class="form-group">
                            <label for="addFirstName"><i class="fas fa-user"></i>First Name *</label>
                            <input type="text" id="addFirstName" name="first_name" required placeholder="Juan">
                        </div>
                        <div class="form-group">
                            <label for="addMiddleName"><i class="fas fa-user"></i>Middle Name</label>
                            <input type="text" id="addMiddleName" name="middle_name" placeholder="Santos">
                        </div>
                        <div class="form-group">
                            <label for="addSuffix"><i class="fas fa-award"></i>Suffix</label>
                            <input type="text" id="addSuffix" name="suffix" placeholder="Jr., III">
                        </div>
                        <div class="form-group">
                            <label for="addSex"><i class="fas fa-venus-mars"></i>Sex *</label>
                        ...
                        <div class="form-group">
                            <label for="addCourse"><i class="fas fa-graduation-cap"></i>Course *</label>
                    ...
                    <div class="tab-footer">
                        <button type="button" onclick="closeAddModal()" class="btn-secondary"><i
                                class="fas fa-times"></i> Cancel</button>
                        <button type="button" onclick="nextTab()" class="btn-primary">Next <i
                ...
                <div id="examTab" class="tab-content">
                    <div class="tab-header">
                        ...
                        <p>Enter CLE board exam details</p>
                    </div>
                    <div class="form-grid">
                        <div class="form-group">
                            <label for="addYear"><i class="fas fa-calendar"></i>Year Graduated *</label>
                            <select id="addYear" name="year_graduated" required>
                                <option value="">Select Year</option>
                                <?php for ($year = $current_year; $year >= 1950; $year--) { echo "<option value=\"$year\">$year</option>"; } ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="addExamDate"><i class="fas fa-calendar-check"></i>Board Exam Date *</label>
                            ...
                            <div id="addExamDateHint" class="small-hint" style="display:none;">Select Board Exam Type
                                first</div>
                        </div>
                        <div class="form-group" id="customDateGroup" style="display: none;">
                            <label for="addCustomDate"><i class="fas fa-calendar"></i>Custom Exam Date *</label>
                            <input type="date" id="addCustomDate" name="custom_exam_date">
                        </div>
                        <div class="form-group">
                            <label for="addResult"><i class="fas fa-award"></i>Result *</label>
                        ...
                        <div class="form-group">
                            <label for="addExamType"><i class="fas fa-redo"></i>Exam Type *</label>
                        ...
                        <div class="form-group" style="grid-column: 1 / -1;">
                            <label for="addBoardExamType"><i class="fas fa-certificate"></i>Board Exam Type *</label>
                    ...
                    <div class="tab-footer">
                        <button type="button" onclick="prevTab()" class="btn-secondary"><i
    ...
    <!-- Export Options & Confirmation Modals (Same as Engineering, but updated filename) -->
    <div id="exportOptionsModal" class="modal" style="display: none;">
        <div class="modal-content export-modal-content">
            <div class="modal-header export-modal-header">
                <div class="modal-icon export-modal-icon"><i class="fas fa-download"></i></div>
                <h3>Export Data</h3>
                <p>Choose your preferred export format</p>
                <button class="modal-close" onclick="closeExportOptionsModal()"><i class="fas fa-times"></i></button>
            </div>
            <div class="export-options-container">
                <div class="export-option-card" onclick="handleExport('csv')">
                    <div class="export-option-icon csv-icon"><i class="fas fa-file-csv"></i></div>
                    <div class="export-option-content">
                        <h4>Export as CSV</h4>
                        <p>Comma-separated values for spreadsheet applications</p>
                    </div>
                    <div class="export-option-arrow"><i class="fas fa-chevron-right"></i></div>
                </div>
                <div class="export-option-card" onclick="handleExport('excel')">
                    <div class="export-option-icon excel-icon"><i class="fas fa-file-excel"></i></div>
                    <div class="export-option-content">
                ...
                <div class="export-option-card" onclick="handleExport('pdf')">
                    <div class="export-option-icon pdf-icon"><i class="fas fa-file-pdf"></i></div>
                    <div class="export-option-content">
                        <h4>Export as PDF</h4>
                ...
                <div class="export-option-card" onclick="handleExport('json')">
                    <div class="export-option-icon json-icon"><i class="fas fa-file-code"></i></div>
                    <div class="export-option-content">
    ...
    <div id="exportConfirmModal" class="modal" style="display: none;">
        <div class="modal-content export-confirm-content">
            <div class="modal-header export-confirm-header">
                <div class="modal-icon export-confirm-icon"><i class="fas fa-question-circle"></i></div>
            ...
            <div class="export-confirm-details">
                ...
                <div class="export-detail-item">
                    ...
                    <span id="exportFileName" class="export-detail-value">CCJE_Board_Passers.csv</span>
                </div>
            </div>
            <div class="modal-buttons export-confirm-buttons">
                <button id="cancelExport" class="btn-secondary export-cancel-btn"><i class="fas fa-times"></i>
                    Cancel</button>
                <button id="confirmExport" class="btn-primary export-confirm-btn"><i class="fas fa-download"></i> <span
                        id="confirmExportText">Export CSV</span></button>
            </div>
        </div>
    </div>

    <!-- Update & Delete Modals (Same structure, updated text) -->
    <div id="updateModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-buttons">
                <button id="cancelUpdate" class="btn-secondary"><i class="fas fa-times"></i> Cancel</button>
                <button id="confirmUpdate" class="btn-primary"><i class="fas fa-check"></i> Confirm Update</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-content">
            <div class="modal-header"
                ...
                <div class="modal-icon"
                    style="background: rgba(255,255,255,0.2); width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px;">
                    <i class="fas fa-exclamation-triangle" style="font-size: 24px; color: #fff;"></i>
                </div>
                <h3 style="margin: 0 0 8px 0; text-align: center;">Confirm Deletion</h3>
                <p style="margin: 0; opacity: 0.9; text-align: center; font-size: 0.9rem;">This action cannot be undone!
                </p>
            </div>
            <div class="modal-body" style="padding: 32px 24px; text-align: center; background: #f8fafc;">
                <div
                    style="background: white; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-bottom: 24px;">
                    ...
                    <p style="color: #374151; font-size: 1.1rem; line-height: 1.6; margin: 0;">
                        Are you sure you want to delete this Criminology student's record?<br>
                        <small style="color: #6b7280; font-size: 0.9rem;">This action will permanently remove all
                            data.</small>
                    </p>
                </div>
            </div>
            <div class="modal-buttons"
                ...
                <button id="cancelDelete" class="btn-secondary"><i class="fas fa-times"></i> Cancel</button>
                <button id="confirmDelete" class="btn-danger"><i class="fas fa-trash-alt"></i> Yes, Delete</button>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const originalHandleExport = window.handleExport;
        window.handleExport = function(format) {
            const fileNames = {
                'csv': 'CCJE_Board_Passers.csv',
                'excel': 'CCJE_Board_Passers.xlsx',
                'pdf': 'CCJE_Board_Passers.pdf',
                'json': 'CCJE_Board_Passers.json'
            };
            document.getElementById('exportFileName').textContent = fileNames[format] ||
                'CCJE_Board_Passers.csv';
            if (originalHandleExport) originalHandleExport(format);
        };
    });
    </script>
    ...
    */
    ...
    function handleAddFormSubmission() {
        ...
        // Send add request to the existing add_boaccje for processing
        console.log('Sending AJAX request...');
        fetch('add_board_passer_ccje.php', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
    ...
    function applyFilters() {
        ...
        allRows.forEach(row => {
            ...
            // Get row data - New structure: ID, Course, Board Exam Type, Board Exam Date, Exam Type, Result, Actions
            const cells = row.querySelectorAll('td');
            const rowData = {
                course: cells[1].textContent.toLowerCase().trim(),
                boardExamType: cells[2].textContent.toLowerCase().trim(),
                examDate: cells[3].textContent.trim(),
                examType: cells[4].textContent.toLowerCase().trim(),
                result: cells[5].textContent.toLowerCase().trim()
            };
  ...
  <div class="footer">
    ...
    <div id="logoutModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(15,23,42,0.85); backdrop-filter:blur(10px); -webkit-backdrop-filter:blur(10px); z-index:999999; justify-content:center; align-items:center;">
        <div style="background:linear-gradient(145deg,#1e293b,#0f172a); border-radius:20px; padding:40px; max-width:420px; width:90%; box-shadow:0 25px 60px rgba(0,0,0,0.5); text-align:center; border:1px solid rgba(255,255,255,0.1);">
            <div style="width:80px; height:80px; background:linear-gradient(135deg,#1e3a5f,#0f1f3d); border-radius:50%; margin:0 auto 20px; display:flex; align-items:center; justify-content:center;">
            ...
            <p style="color:#64748b; font-size:13px; margin-bottom:30px; line-height:1.6;">
                You are about to log out from the CCJE Admin Dashboard. Any unsaved changes will be lost.
            </p>
            <div style="display:flex; gap:15px; justify-content:center;">
                ...
                <button onclick="window.location.href='logout.php';" style="padding:12px 30px; border:none; border-radius:10px; font-size:15px; font-weight:500; cursor:pointer; background:linear-gradient(135deg,#1e3a5f,#0f1f3d); color:#fff; display:flex; align-items:center; gap:8px; transition:transform 0.2s;">
