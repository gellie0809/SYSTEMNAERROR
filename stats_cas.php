...
try {
  ...
  if ($action === 'subjects') {
    ...
    $passerWhere = "bp.department = 'Arts and Sciences'";
    $subParams = [];
    $subTypes = '';
    if ($typeName !== '') {
    ...
    $sql = "SELECT s.id AS subject_id,
       ...
       WHERE s.department = 'Arts and Sciences'
  ...
  if ($action === 'subjects_all') {
    $sql = "SELECT bet.id AS exam_type_id,
            ...
            LEFT JOIN subjects s ON s.id = se.subject_id AND s.department = 'Arts and Sciences'
            WHERE bet.department = 'Arts and Sciences'
    ...
    $res2 = $conn->query("SELECT id, exam_type_name FROM board_exam_types WHERE department='Arts and Sciences' ORDER BY exam_type_name ASC");
    if ($res2) {
  ...
  if ($action === 'students_subjects') {
    ...
    $subStmt = $conn->prepare("SELECT s.id, COALESCE(NULLIF(TRIM(s.subject_name),''), CONCAT('Subject ', s.id)) AS subject_name, COALESCE(s.total_items,50) AS total_items
                               ...
                               JOIN subjects s ON s.id = se.subject_id AND s.department='Arts and Sciences'
    ...
    $pw = "bp.department='Arts and Sciences'"; $pp=[]; $pt='';
  ...
  if ($action === 'trend_passing_rate') {
    ...
    $types = [];
    $res = $conn->query("SELECT id, exam_type_name FROM board_exam_types WHERE department='Arts and Sciences' ORDER BY exam_type_name ASC");
    ...
    foreach ($types as $t) {
      $sql = "SELECT YEAR(board_exam_date) AS y, COUNT(*) AS total, SUM(CASE WHEN result='Passed' THEN 1 ELSE 0 END) AS passed
              FROM board_passers
              WHERE department='Arts and Sciences' AND board_exam_date BETWEEN ? AND ?
  ...
  if ($action === 'passing_rate_forecast') {
    ...
    $filterSql = "WHERE department='Arts and Sciences' AND board_exam_date BETWEEN ? AND ?";
    $bindTypes = 'ss';
    $bindVals  = [ $from, $to ];
    if ($typeName !== '') {
