...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["add_exam_type"])) {
  ...
  if ($name === '') {
    header("Location: manage_data_cbaa.php?error=empty_exam_type"); exit();
  }
  $chk = $conn->prepare("SELECT id FROM board_exam_types WHERE LOWER(TRIM(exam_type_name)) = LOWER(TRIM(?)) AND department='Business Administration and Accountancy'");
  $chk->bind_param('s', $name); $chk->execute(); $chk->store_result();
  if ($chk->num_rows > 0) { $chk->close(); header("Location: manage_data_cbaa.php?error=exam_type_exists"); exit(); }
  $chk->close();
  $ins = $conn->prepare("INSERT INTO board_exam_types (exam_type_name, department) VALUES (?, 'Business Administration and Accountancy')");
  $ins->bind_param('s', $name);
  if ($ins->execute()) { $ins->close(); header("Location: manage_data_cbaa.php?success=exam_type_added"); exit(); }
  $ins->close(); header("Location: manage_data_cbaa.php?error=exam_type_add_failed"); exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_exam_type'])) {
  ...
  if ($id <= 0 || $newname === '') { header("Location: manage_data_cbaa.php?error=invalid_edit"); exit(); }
  $chk = $conn->prepare("SELECT id FROM board_exam_types WHERE LOWER(TRIM(exam_type_name)) = LOWER(TRIM(?)) AND department='Business Administration and Accountancy' AND id != ?");
  $chk->bind_param('si', $newname, $id); $chk->execute(); $res = $chk->get_result();
  if ($res && $res->num_rows > 0) { $chk->close(); header("Location: manage_data_cbaa.php?error=exam_type_exists"); exit(); }
  $chk->close();
  $up = $conn->prepare("UPDATE board_exam_types SET exam_type_name = ? WHERE id = ? AND department='Business Administration and Accountancy'");
  $up->bind_param('si', $newname, $id);
  if ($up->execute()) { $up->close(); header("Location: manage_data_cbaa.php?success=exam_type_updated"); exit(); }
  $up->close(); header("Location: manage_data_cbaa.php?error=exam_type_update_failed"); exit();
}

// Handle delete exam type (soft delete)
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_exam_type'])) {
  $id = intval($_POST['exam_type_id'] ?? 0);
  if ($id <= 0) { header("Location: manage_data_cbaa.php?error=invalid_delete"); exit(); }
  ...
  if ($cnt > 0) { header("Location: manage_data_cbaa.php?error=exam_type_in_use"); exit(); }
  $del = $conn->prepare("UPDATE board_exam_types SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department='Business Administration and Accountancy'"); $del->bind_param('i', $id);
  if ($del->execute()) { $del->close(); header("Location: manage_data_cbaa.php?success=exam_type_deleted"); exit(); }
  $del->close(); header("Location: manage_data_cbaa.php?error=exam_type_delete_failed"); exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_subject'])) {
  ...
  if ($exam_type_id <= 0 || $subject_name === '') { header("Location: manage_data_cbaa.php?error=invalid_subject"); exit(); }
  $ins = $conn->prepare("INSERT INTO subjects (subject_name, total_items, department) VALUES (?, ?, 'Business Administration and Accountancy')");
  $ins->bind_param('si', $subject_name, $total_items);
  if (!$ins->execute()) { $ins->close(); header("Location: manage_data_cbaa.php?error=subject_add_failed"); exit(); }
  ...
  header("Location: manage_data_cbaa.php?success=subject_added&exam_type_id=" . $exam_type_id);
  exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_subject'])) {
  ...
  if ($sid <= 0 || $name === '') { header("Location: manage_data_cbaa.php?error=invalid_subject_edit"); exit(); }
  ...
  if ($countUsage > 0) { header("Location: manage_data_cbaa.php?error=subject_in_use"); exit(); }
  $up = $conn->prepare("UPDATE subjects SET subject_name = ?, total_items = ? WHERE id = ? AND department = 'Business Administration and Accountancy'"); $up->bind_param('sii', $name, $items, $sid);
  if ($up->execute()) { $up->close(); header("Location: manage_data_cbaa.php?success=subject_updated"); exit(); }
  $up->close(); header("Location: manage_data_cbaa.php?error=subject_update_failed"); exit();
}

// Handle delete subject (soft delete)
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_subject'])) {
  $sid = intval($_POST['subject_id'] ?? 0);
  if ($sid <= 0) { header("Location: manage_data_cbaa.php?error=invalid_subject_delete"); exit(); }
  $chk = $conn->prepare("SELECT COUNT(*) as cnt FROM board_passer_subjects WHERE subject_id = ?"); $chk->bind_param('i', $sid); $chk->execute(); $cres = $chk->get_result(); $crow = $cres ? $cres->fetch_assoc() : null; $countUsage = intval($crow['cnt'] ?? 0); $chk->close();
  if ($countUsage > 0) { header("Location: manage_data_cbaa.php?error=subject_in_use"); exit(); }
  $del = $conn->prepare("UPDATE subjects SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department = 'Business Administration and Accountancy'"); $del->bind_param('i', $sid);
  if ($del->execute()) { $del->close(); header("Location: manage_data_cbaa.php?success=subject_deleted"); exit(); }
  $del->close(); header("Location: manage_data_cbaa.php?error=subject_delete_failed"); exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_exam_date'])) {
  ...
  if ($exam_type_id <= 0 || $exam_date === '') { header("Location: manage_data_cbaa.php?error=invalid_exam_date"); exit(); }
  // Convert YYYY-MM format to YYYY-MM-01 (first day of month)
  $exam_date_full = $exam_date . '-01';
  $chk = $conn->prepare("SELECT id FROM board_exam_dates WHERE exam_date = ? AND exam_type_id = ? AND department = 'Business Administration and Accountancy'");
  $chk->bind_param('si', $exam_date_full, $exam_type_id); $chk->execute(); $chk->store_result();
  if ($chk->num_rows > 0) { $chk->close(); header("Location: manage_data_cbaa.php?error=exam_date_exists&exam_type_id=" . $exam_type_id); exit(); }
  $chk->close();
  $ins = $conn->prepare("INSERT INTO board_exam_dates (exam_date, exam_description, exam_type_id, department) VALUES (?, ?, ?, 'Business Administration and Accountancy')");
  $ins->bind_param('ssi', $exam_date_full, $exam_desc, $exam_type_id);
  if ($ins->execute()) { $ins->close(); header("Location: manage_data_cbaa.php?success=exam_date_added&exam_type_id=" . $exam_type_id); exit(); }
  $ins->close(); header("Location: manage_data_cbaa.php?error=exam_date_add_failed&exam_type_id=" . $exam_type_id); exit(); }
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_exam_date'])) {
  ...
  if ($eid <= 0 || $edate === '') { header("Location: manage_data_cbaa.php?error=invalid_exam_date_edit"); exit(); }
  // Convert YYYY-MM format to YYYY-MM-01 (first day of month)
  $edate_full = $edate . '-01';
  $g = $conn->prepare("SELECT exam_type_id FROM board_exam_dates WHERE id = ? AND department = 'Business Administration and Accountancy'"); $g->bind_param('i', $eid); $g->execute(); $gres = $g->get_result(); $grow = $gres ? $gres->fetch_assoc() : null; $g->close();
  $exam_type_id = intval($grow['exam_type_id'] ?? 0);
  if ($exam_type_id <= 0) { header("Location: manage_data_cbaa.php?error=exam_date_not_found"); exit(); }
  $chk = $conn->prepare("SELECT id FROM board_exam_dates WHERE exam_date = ? AND exam_type_id = ? AND id != ? AND department = 'Business Administration and Accountancy'"); $chk->bind_param('sii', $edate_full, $exam_type_id, $eid); $chk->execute(); $res = $chk->get_result();
  if ($res && $res->num_rows > 0) { $chk->close(); header("Location: manage_data_cbaa.php?error=exam_date_exists&exam_type_id=" . $exam_type_id); exit(); }
  $chk->close();
  $up = $conn->prepare("UPDATE board_exam_dates SET exam_date = ?, exam_description = ? WHERE id = ? AND department = 'Business Administration and Accountancy'"); $up->bind_param('ssi', $edate_full, $edesc, $eid);
  if ($up->execute()) { $up->close(); header("Location: manage_data_cbaa.php?success=exam_date_updated&exam_type_id=" . $exam_type_id); exit(); }
  $up->close(); header("Location: manage_data_cbaa.php?error=exam_date_update_failed&exam_type_id=" . $exam_type_id); exit();
}

// Handle delete exam date (soft delete)
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_exam_date'])) {
  $eid = intval($_POST['exam_date_id'] ?? 0);
  if ($eid <= 0) { header("Location: manage_data_cbaa.php?error=invalid_exam_date_delete"); exit(); }
  $check_usage = $conn->prepare("SELECT COUNT(*) as cnt FROM board_passers WHERE board_exam_date = (SELECT exam_date FROM board_exam_dates WHERE id = ?) AND department = 'Business Administration and Accountancy'");
  ...
  if ($used > 0) { header("Location: manage_data_cbaa.php?error=exam_date_in_use&count=" . $used); exit(); }
  $del = $conn->prepare("UPDATE board_exam_dates SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department = 'Business Administration and Accountancy'"); $del->bind_param('i', $eid);
  if ($del->execute()) { $del->close(); header("Location: manage_data_cbaa.php?success=exam_date_deleted"); exit(); }
  $del->close(); header("Location: manage_data_cbaa.php?error=exam_date_delete_failed"); exit();
}

// Handle add course
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_course'])) {
  $new_course = trim($_POST['new_course'] ?? '');
  if ($new_course === '') { header("Location: manage_data_cbaa.php?error=empty_course"); exit(); }
  $chk = $conn->prepare("SELECT id FROM courses WHERE LOWER(TRIM(course_name)) = LOWER(TRIM(?)) AND department = 'Business Administration and Accountancy'");
  $chk->bind_param('s', $new_course); $chk->execute(); $chk->store_result();
  if ($chk->num_rows > 0) { $chk->close(); header("Location: manage_data_cbaa.php?error=course_exists"); exit(); }
  $chk->close();
  $ins = $conn->prepare("INSERT INTO courses (course_name, department) VALUES (?, 'Business Administration and Accountancy')"); $ins->bind_param('s', $new_course);
  if ($ins->execute()) { $ins->close(); header("Location: manage_data_cbaa.php?success=course_added"); exit(); }
  $ins->close(); header("Location: manage_data_cbaa.php?error=course_add_failed"); exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['edit_course'])) {
  ...
  if ($cid <= 0 || $cname === '') { header("Location: manage_data_cbaa.php?error=invalid_course_edit"); exit(); }
  $chk = $conn->prepare("SELECT id FROM courses WHERE LOWER(TRIM(course_name)) = LOWER(TRIM(?)) AND department = 'Business Administration and Accountancy' AND id != ?"); $chk->bind_param('si', $cname, $cid); $chk->execute(); $res = $chk->get_result();
  if ($res && $res->num_rows > 0) { $chk->close(); header("Location: manage_data_cbaa.php?error=course_exists"); exit(); }
  $chk->close();
  $up = $conn->prepare("UPDATE courses SET course_name = ? WHERE id = ? AND department = 'Business Administration and Accountancy'"); $up->bind_param('si', $cname, $cid);
  if ($up->execute()) { $up->close(); header("Location: manage_data_cbaa.php?success=course_updated"); exit(); }
  $up->close(); header("Location: manage_data_cbaa.php?error=course_update_failed"); exit();
}

// Handle delete course (soft delete)
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_course'])) {
  $cid = intval($_POST['course_id'] ?? 0);
  if ($cid <= 0) { header("Location: manage_data_cbaa.php?error=invalid_course_delete"); exit(); }
  $g = $conn->prepare("SELECT course_name FROM courses WHERE id = ? AND department = 'Business Administration and Accountancy'"); $g->bind_param('i', $cid); $g->execute(); $gres = $g->get_result(); $grow = $gres ? $gres->fetch_assoc() : null; $g->close();
  $cname = trim($grow['course_name'] ?? '');
  if ($cname === '') { header("Location: manage_data_cbaa.php?error=course_not_found"); exit(); }
  $chk = $conn->prepare("SELECT COUNT(*) as cnt FROM board_passers WHERE course = ? AND department = 'Business Administration and Accountancy'"); $chk->bind_param('s', $cname); $chk->execute(); $cres = $chk->get_result(); $crow = $cres ? $cres->fetch_assoc() : null; $countUsage = intval($crow['cnt'] ?? 0); $chk->close();
  if ($countUsage > 0) { header("Location: manage_data_cbaa.php?error=course_in_use"); exit(); }
  $del = $conn->prepare("UPDATE courses SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND department = 'Business Administration and Accountancy'"); $del->bind_param('i', $cid);
  if ($del->execute()) { $del->close(); header("Location: manage_data_cbaa.php?success=course_deleted"); exit(); }
  $del->close(); header("Location: manage_data_cbaa.php?error=course_delete_failed"); exit();
}

// Fetch exam types
$etres = $conn->query("SELECT id, TRIM(exam_type_name) as name FROM board_exam_types WHERE department='Business Administration and Accountancy' AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY exam_type_name ASC");
$exam_types = [];
...
if ($selected_exam_type_id > 0) {
  $stmt = $conn->prepare("SELECT s.id, TRIM(s.subject_name) as subject_name, s.total_items FROM subjects s JOIN subject_exam_types setmap ON setmap.subject_id = s.id WHERE setmap.exam_type_id = ? AND s.department = 'Business Administration and Accountancy' AND (s.is_deleted = 0 OR s.is_deleted IS NULL) ORDER BY s.subject_name ASC");
...
// Fetch courses
$courses = [];
$cres = $conn->query("SELECT id, TRIM(course_name) as name FROM courses WHERE department='Business Administration and Accountancy' AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY course_name ASC");
if ($cres) { while ($cr = $cres->fetch_assoc()) $courses[] = $cr; }

// Fetch exam dates
$exam_dates = [];
if ($selected_exam_type_id > 0) {
  $dstmt = $conn->prepare("SELECT id, exam_date, TRIM(exam_description) as exam_description FROM board_exam_dates WHERE exam_type_id = ? AND department = 'Business Administration and Accountancy' AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY exam_date ASC");
...
<head>
    ...
    * {
    ...
    .modal-content input[type="date"]:focus {
        outline: none;
        border-color: #F59E0B;
        box-shadow: 0 0 0 4px rgba(245, 158, 11, 0.15), 0 4px 12px rgba(245, 158, 11, 0.2);
        transform: translateY(-2px);
    }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="panel" style="margin-bottom:24px;">
            ...
            <form method="post" action="manage_data_cbaa.php" class="add-form-section" id="addCourseForm">
            ...
            <div class="small muted" style="text-align:center; padding:20px;">No courses defined for CBAA yet.</div>
            <?php else: ?>
        ...
        <div class="grid">
            <div class="panel" style="min-height:480px;">
                ...
                <form method="post" action="manage_data_cbaa.php" class="add-form-section" id="addExamTypeForm">
                    <div class="form-row">
                ...
                <div style="margin-top:6px;">
                    ...
                    <div class="list-item">
                        ...
                        <div class="actions">
                            <form method="get" action="manage_data_cbaa.php" style="display:inline;"><input
            ...
            <div class="right-column">
                <div class="panel">
                    ...
                    <div
                        style="margin-bottom:16px; padding:12px; background:linear-gradient(135deg, #FEF3C7 0%, #FDE68A 100%); border-radius:10px; border:2px solid #FBBF24;">
                        <strong style="color:#D97706; font-size:1.05rem;"><i class="fas fa-check-circle"></i>
                            <?= htmlspecialchars($etname ?? 'Selected') ?></strong>
                    </div>

                    <form method="post" action="manage_data_cbaa.php?exam_type_id=<?= $selected_exam_type_id ?>"
                ...
                <div class="panel">
                    ...
                    <form method="post" action="manage_data_cbaa.php?exam_type_id=<?= $selected_exam_type_id ?>"
                        ...
                        <div class="form-row">
                            <label for="new_exam_date" style="display: block; margin-bottom: 8px; color: #D97706; font-weight: 600; font-size: 0.9rem;">
                                <i class="fas fa-calendar-alt"></i> Select Month & Year
                            </label>
                            <input type="month" name="new_exam_date" id="new_exam_date" required 
                                   style="width: 100%; padding: 12px 16px; border: 2px solid #e2e8f0; border-radius: 12px; font-size: 1rem; font-family: 'Inter', sans-serif; transition: all 0.3s ease;"
                                   onfocus="this.style.borderColor='#F59E0B'; this.style.boxShadow='0 0 0 3px rgba(245, 158, 11, 0.1)'"
                                   onblur="this.style.borderColor='#e2e8f0'; this.style.boxShadow='none'">
                        </div>
                        <div class="form-row">
                            <label for="exam_description" style="display: block; margin-bottom: 8px; color: #D97706; font-weight: 600; font-size: 0.9rem;">
                                <i class="fas fa-info-circle"></i> Description (Optional)
                            </label>
                            <input type="text" name="exam_description" id="exam_description"
                    ...
                    <div class="list-item" style="background: linear-gradient(135deg, #FFFBEA 0%, #FEF3C7 100%); border-left: 4px solid #F59E0B;">
                        <div>
                            <div class="item-name" style="display: flex; align-items: center; gap: 10px; font-size: 1.1rem; color: #D97706;">
                                <span style="background: #F59E0B; color: white; width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-calendar-alt"></i>
                                </span>
                                <span style="font-weight: 600;"><?= htmlspecialchars(date('F Y', strtotime($ed['exam_date']))) ?></span>
                            </div>
                            <?php if (!empty($ed['exam_description'])): ?>
                                <div class="small" style="margin-left: 46px; color: #B45309; margin-top: 4px;">
                                    <i class="fas fa-info-circle"></i> <?= htmlspecialchars($ed['exam_description']) ?>
                                </div>
                            <?php endif; ?>
                        </div>
                        <div class="actions">
    ...
    <div id="editExamTypeModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form id="editExamTypeForm" method="post" action="manage_data_cbaa.php">
                <input type="hidden" name="exam_type_id" id="editExamTypeId">
                <div style="margin-bottom:20px;">
                    <label
                        style="display:block; margin-bottom:8px; color:#D97706; font-weight:600; font-size:0.9rem;"><i
    ...
    <div id="editSubjectModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form id="editSubjectForm" method="post" action="manage_data_cbaa.php">
    ...
    <div id="editCourseModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <form id="editCourseForm" method="post" action="manage_data_cbaa.php">
    ...
    <div id="editExamDateModal" class="modal-overlay">
        <div class="modal-content">
            <h3 style="color: #D97706; margin-bottom: 24px;"><i class="fas fa-calendar-alt"></i> Edit Exam Date</h3>
            <form id="editExamDateForm" method="post" action="manage_data_cbaa.php">
                <input type="hidden" name="exam_date_id" id="editExamDateId">
                <div style="margin-bottom:18px;">
                    <label
                        style="display:block; margin-bottom:8px; color:#D97706; font-weight:600; font-size:0.9rem;"><i
                            class="fas fa-calendar-alt"></i> Month & Year</label>
                    <input type="month" name="edit_exam_date_value" id="editExamDateValue" 
                           style="width: 100%; padding: 12px 16px; border: 2px solid #e2e8f0; border-radius: 12px; font-size: 1rem; font-family: 'Inter', sans-serif; transition: all 0.3s ease;"
                           onfocus="this.style.borderColor='#F59E0B'; this.style.boxShadow='0 0 0 3px rgba(245, 158, 11, 0.1)'"
                           onblur="this.style.borderColor='#e2e8f0'; this.style.boxShadow='none'">
                </div>
    ...
    window.openEditExamDate = (id, dateVal, desc) => {
        ...
        const monthValue = dateVal.substring(0, 7);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (saveCourseBtn) saveCourseBtn.addEventListener('click', (e) => {
            ...
            showCustomConfirm('Save changes?', (c) => {
                if (c) {
                    ...
                    // Create a hidden submit button and click it to trigger proper form submission
                    const hiddenSubmit = document.createElement('input');
                    hiddenSubmit.type = 'submit';
                    hiddenSubmit.name = 'edit_course';
        ...
        if (saveExamDateBtn) saveExamDateBtn.addEventListener('click', (e) => {
            ...
            showCustomConfirm('Save changes?', (c) => {
                if (c) {
                    const form = document.getElementById('editExamDateForm');
                    // Create a hidden submit button and click it to trigger proper form submission
                    const hiddenSubmit = document.createElement('input');
                    hiddenSubmit.type = 'submit';
                    hiddenSubmit.name = 'edit_exam_date';
                    hiddenSubmit.style.display = 'none';
                    form.appendChild(hiddenSubmit);
                    hiddenSubmit.click();
                }
        ...
        // Remove confirmation for add forms - they can submit directly
        // Only delete actions need confirmation
    });
    </script>
</body>
