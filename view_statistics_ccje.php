...
// Fetch board exam types and dates for filters — CCJE version
$betStmt = $conn->prepare("SELECT id, exam_type_name FROM board_exam_types WHERE department='Criminal Justice Education' AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY exam_type_name ASC");
$betStmt->execute();
$betRes = $betStmt->get_result();
$boardExamTypes = [];
while ($r = $betRes->fetch_assoc()) { 
    $boardExamTypes[] = $r; 
}

$bedStmt = $conn->prepare("SELECT id, exam_date, exam_description, exam_type_id FROM board_exam_dates WHERE department='Criminal Justice Education' AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY exam_date DESC");
$bedStmt->execute();
$bedRes = $bedStmt->get_result();
$boardExamDates = [];
while ($r = $bedRes->fetch_assoc()) { 
    $boardExamDates[] = $r; 
}
...
<head>
    ...
    <title>View Statistics - CCJE</title>
    <link rel="stylesheet" href="style.css" />
    <link rel="stylesheet" href="css/sidebar.css" />
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
    /* Modern CCJE red theme with animations */
    :root {
        --primary: #D32F2F;
        --primary-dark: #C62828;
        --primary-darker: #800020;
        --magenta: #f472b6;
        --muted: #64748b;
        --success: #10b981;
        --danger: #ef4444;
    ...
    * {
    ...
    select {
        ...
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        appearance: none;
        background-image: linear-gradient(45deg, transparent 50%, #D32F2F 50%),
    ...
    .tip-pill {
        ...
        padding: 12px 16px;
        border-radius: 14px;
        background: linear-gradient(145deg, #FDF3E7 0%, #FAD6A5 100%);
        color: #800020;
    ...
    .chart-card::before {
        ...
        right: 0;
        height: 3px;
        background: linear-gradient(90deg, #D32F2F 0%, #C62828 50%, #E63946 100%);
        opacity: 0;
...
<body>
    ...
    <div class="topbar">
        <div class="dashboard-title">CCJE Admin Dashboard</div>
            ...
            <div class="filter-panel">
                ...
                <div class="step">
                    ...
                    <select id="examTypeSelect">
                        ...
                        <option value="<?= htmlspecialchars($t['id']) ?>"><?= htmlspecialchars($t['exam_type_name']) ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="visual-panel">
                ...
                <div id="statsContainer" class="printable" style="position:relative;">
                    ...
                    <div id="printHeader" class="print-header" style="margin-bottom:12px;">
                        <h1 style="margin:0; font-size:20px;">CAS Statistics</h1>
                    ...
                    <div class="charts-grid">
                        ...
                        <div id="deptPassingCard" class="chart-card" style="grid-column: 1 / -1;">
                            <div
                                ...
                                <h4 style="margin: 0;">Board Passing Rate in the College of Arts and Sciences
                                    (2019–2024)</h4>
                            ...
                            <div id="deptDescription" class="muted"
                                style="display: none; margin-bottom: 16px; font-size: 0.92rem; line-height: 1.5; padding: 14px; background: linear-gradient(145deg, #FDF3E7 0%, #FAD6A5 100%); border-radius: 12px; border-left: 4px solid #D32F2F;">
                                <i class="fas fa-chart-line" style="color: #D32F2F; margin-right: 6px;"></i>
                                ...
                                licensure examinations across CAS programs during this period.
                            </div>
                            <canvas id="chartDeptPassingRate" height="220"></canvas>
                            <div class="print-caption">Board passing rate in the College of Arts and Sciences, 2019–2024
                            </div>
                        </div>
                        <div id="overallTrendsCard" class="chart-card" style="grid-column: 1 / -1;">
                            ...
                            <div id="trendDescription" class="muted"
                                style="display: none; margin-bottom: 16px; font-size: 0.92rem; line-height: 1.5; padding: 14px; background: linear-gradient(145deg, #FDF3E7 0%, #FAD6A5 100%); border-radius: 12px; border-left: 4px solid #D32F2F;">
                                <i class="fas fa-chart-bar" style="color: #D32F2F; margin-right: 6px;"></i>
                                ...
                                psychology, criminology, and other programs.
                            </div>
                        ...
                        <div id="overallStackedCard" class="chart-card" style="grid-column: 1 / -1;">
                            ...
                            <div id="compositionModeControls"
                                ...
                                <button id="compModePercent" class="btn-secondary active"
                        ...
                        <div class="chart-card" style="grid-column: 1 / -1;">
                            ...
                            <div id="subjectsModeControls"
                                style="display:flex;align-items:center;gap:8px;margin:6px 0 8px 0;">
                                <span class="muted">View:</span>
                                <button id="modeCounts" class="btn-secondary active"
                                    style="padding:6px 10px;border-radius:8px;line-height:1;">Counts</button>
        ...
        (function renderYearChips() {
            ...
            years.forEach(y => {
                ...
                chip.addEventListener('click', () => {
                    ...
                    selectedDateId = '';
                    examDateSelect.value = '';
                    confirmBtn.disabled = !selectedTypeId || (!selectedDateId && !
                        selectedYear);
    ...
    // Palette chosen to harmonize with primary teal (#D32F2F) and magenta (#f472b6)
    // Pastel color palette
    const PALETTE = [
        '#C62828', // burgundy (Male / primary accent)
    ...
    (function initSubjectsModeToggle() {
        ...
        function refresh() {
            ...
            btnCounts.style.background = isPercent ? '#fff' : '#D32F2F';
            btnCounts.style.color = isPercent ? '#D32F2F' : '#fff';
            btnPercent.style.background = isPercent ? '#D32F2F' : '#fff';
            btnPercent.style.color = isPercent ? '#fff' : '#D32F2F';
        }
    ...
    function showRecordsPopover(title, rows, count, totalCount, evt) {
        ...
        table.innerHTML =
            `<thead><tr style="background: linear-gradient(135deg, #D32F2F 0%, #C62828 100%); position:sticky; top:0; color: white; box-shadow: 0 4px 15px rgba(6, 182, 212, 0.2);"><th style="padding: 14px 12px; font-weight: 700; text-align:left; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Name</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Sex</th><th style="padding: 14px 12px; font-weight: 700; text-align:left; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Course</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Year Graduated</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Exam Date</th><th style="padding: 14px 12px; font-weight: 700; text-align:center; font-size: 0.9rem; border-bottom: 2px solid rgba(255, 255, 255, 0.2);">Result</th></tr></thead><tbody></tbody>`;
    ...
    function confirmLogout(event) {
        ...
        body.innerHTML =
            '<p style="margin: 0; font-size: 1.05rem; color: #334155; line-height: 1.6;"><i class="fas fa-question-circle" style="color: #D32F2F; margin-right: 8px;"></i>Are you sure you want to log out?</p>';
    ...
    function renderOverallSubjectsBars(groups) {
        ...
        function fetchSubjectsTotalsForType(typeId) {
            return fetch(`stats_ccje.php?action=subjects&boardExamTypeId=${encodeURIComponent(typeId)}`)
                .then(r => r.json())
    ...
    (function initOverallSubjectsMode() {
        ...
        function applyButtons() {
            ...
            btnC.style.background = isCounts ? '#D32F2F' : '#fff';
            btnC.style.color = isCounts ? '#fff' : '#D32F2F';
            btnP.style.background = isCounts ? '#fff' : '#D32F2F';
            btnP.style.color = isCounts ? '#D32F2F' : '#fff';
        }
    ...
    (function initCompositionToggle() {
        ...
        function applyButtons() {
            ...
            btnP.style.background = isPercent ? '#D32F2F' : '#fff';
            btnP.style.color = isPercent ? '#fff' : '#D32F2F';
            btnC.style.background = isPercent ? '#fff' : '#D32F2F';
            btnC.style.color = isPercent ? '#D32F2F' : '#fff';
        }
        btnP.addEventListener('click', () => {
