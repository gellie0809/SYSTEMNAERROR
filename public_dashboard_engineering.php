...
// Fetch all engineering board passers
$passers = $conn->query("SELECT * FROM anonymous_board_passers WHERE department='Engineering' AND (is_deleted IS NULL OR is_deleted = 0) ORDER BY board_exam_date DESC");
...
<head>
    ...
    <title>College of Engineering - Board Performance Analytics | LSPU</title>
    ...
    <style>
        :root {
            /* Sage Green Color Palette from Image */
            --sage-1: #B8C9B8;  /* Light sage */
            --sage-2: #C4D4C0;  /* Pale sage */
            --sage-3: #9DB496;  /* Medium sage */
            --sage-4: #5B7B5A;  /* Dark sage */
            --sage-5: #D6E5D6;  /* Very light sage */
            
            --primary: #5B7B5A;
            --primary-dark: #4A6349;
            --primary-light: #9DB496;
            --primary-lighter: #B8C9B8;
            --primary-bg: #D6E5D6;
            --primary-pale: #E8F0E8;
            
            --gradient: linear-gradient(135deg, #5B7B5A 0%, #9DB496 100%);
            --gradient-dark: linear-gradient(135deg, #4A6349 0%, #5B7B5A 100%);
            --gradient-light: linear-gradient(135deg, #B8C9B8 0%, #D6E5D6 100%);
            
            --text-dark: #2D3B2D;
            --text-medium: #4A5D4A;
            --text-light: #6B7D6B;
            --white: #ffffff;
            --shadow: rgba(91, 123, 90, 0.15);
            --shadow-strong: rgba(91, 123, 90, 0.25);
        }
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .bg-pattern {
            ...
            opacity: 0.4;
            background-image: 
                radial-gradient(circle at 20% 80%, var(--sage-3) 0%, transparent 50%),
                radial-gradient(circle at 80% 20%, var(--sage-2) 0%, transparent 50%),
                radial-gradient(circle at 40% 40%, var(--sage-5) 0%, transparent 30%);
        }
        ...
        .info-btn {
            ...
            gap: 8px;
            transition: all 0.3s ease;
            box-shadow: 0 4px 15px rgba(91, 123, 90, 0.3);
        }
        .info-btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 6px 20px rgba(91, 123, 90, 0.4);
        }
        .info-modal-overlay {
            display: none;
            position: fixed;
...
<body>
    ...
    <main class="main">
        ...
        <section class="kpi-section">
            <div class="section-header">
                ...
                <p>Overall board examination statistics for College of Engineering (<?php echo $year_range; ?>)</p>
            </div>
        ...
        <section class="exam-comparison-section">
            <div class="section-header">
                ...
                <p>Comparative analysis of different engineering licensure examinations</p>
            </div>
                ...
                <div class="chart-card">
                    <div class="chart-header">
                        <div class="chart-title-group">
                            ...
                            <p>Pass/Fail breakdown for all engineering board exams (<?php echo $year_range; ?>)</p>
                        </div>
                    </div>
                ...
                <div class="chart-card full-width">
                    <div class="chart-header">
                        <div class="chart-title-group">
                            ...
                            <p>Historical passing rate trends across all engineering board examinations</p>
                        </div>
                        <div class="chart-legend-custom">
                            <div class="legend-item"><span class="legend-dot" style="background: #5B7B5A;"></span> Passed</div>
                            <div class="legend-item"><span class="legend-dot" style="background: #94a3b8;"></span> Failed</div>
                            <div class="legend-item"><span class="legend-dot" style="background: #9DB496;"></span> Pass Rate %</div>
                        </div>
                    </div>
                ...
                <div class="chart-card">
                    <div class="chart-header">
                        <div class="chart-title-group">
                            ...
                            <p>Distribution of examinees across different engineering boards</p>
                        </div>
                    </div>
                    <div class="chart-container">
    ...
    <footer class="footer">
        ...
        <p style="margin-top: 8px; opacity: 0.7; font-size: 0.8rem;">College of Engineering Board Performance Analytics Dashboard</p>
    </footer>
    ...
    <script>
            ...
            data: {
                ...
                datasets: [
                    ...
                    },
                    {
                        ...
                        backgroundColor: 'rgba(157, 180, 150, 0.2)',
                        fill: true,
                        tension: 0.4,
                        pointRadius: 6,
                        pointHoverRadius: 8,
        ...
        new Chart(document.getElementById('examDistributionChart').getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: [
                        'rgba(91, 123, 90, 0.8)',
                        'rgba(157, 180, 150, 0.8)',
                        'rgba(184, 201, 184, 0.8)',
                        'rgba(74, 99, 73, 0.8)',
                        'rgba(196, 212, 192, 0.8)',
                        'rgba(122, 155, 121, 0.8)'
                    ],
                    borderWidth: 2,
                    borderColor: '#fff'
                }]
        ...
        async function fetchPredictions() {
            try {
                const response = await fetch('http://localhost:5000/api/predict', {
                    ...
                    body: JSON.stringify({ department: 'Engineering', historical_rate: <?php echo $passing_rate; ?> })
                });
        ...
        function exportCSV(data, includeStats) {
            ...
            if (includeStats) {
                csv += 'COLLEGE OF ENGINEERING - BOARD EXAMINATION STATISTICS\n';
            ...
            downloadFile(csv, 'engineering_board_exam_data.csv', 'text/csv');
        }
        ...
        function exportJSON(data, includeStats) {
            ...
            downloadFile(JSON.stringify(exportData, null, 2), 'engineering_board_exam_data.json', 'application/json');
        }
        ...
        function exportExcel(data, includeStats) {
            ...
            downloadFile(csv, 'engineering_board_exam_data.xlsx', 'application/vnd.ms-excel');
        }
        ...
        function exportPDF(data, includeStats) {
            ...
            let html = `
                ...
                <html>
                <head>
                    <title>Engineering Board Exam Report</title>
                    <style>
                        body { font-family: Arial, sans-serif; padding: 20px; }
                        h1 { color: #5B7B5A; border-bottom: 2px solid #5B7B5A; padding-bottom: 10px; }
                        .stats { display: flex; gap: 20px; margin: 20px 0; }
                        .stat-box { background: #D6E5D6; padding: 15px 25px; border-radius: 8px; text-align: center; }
                        .stat-value { font-size: 24px; font-weight: bold; color: #5B7B5A; }
                        .stat-label { font-size: 12px; color: #666; }
                        table { width: 100%; border-collapse: collapse; margin-top: 20px; }
                        th { background: #5B7B5A; color: white; padding: 10px; text-align: left; }
                        td { padding: 8px 10px; border-bottom: 1px solid #ddd; }
                        tr:nth-child(even) { background: #f9f9f9; }
                        .passed { color: #5B7B5A; font-weight: bold; }
                        .failed { color: #dc2626; }
                        .footer { margin-top: 30px; text-align: center; color: #666; font-size: 12px; }
                    </style>
                </head>
                <body>
                    <h1>⚙️ College of Engineering - Board Examination Report</h1>
        ...
        function showNotification(message) {
            ...
            notification.style.cssText = `
                ...
                padding: 16px 24px;
                border-radius: 12px;
                box-shadow: 0 4px 20px rgba(91, 123, 90, 0.3);
                z-index: 1001;
        ...
        <div class="info-modal">
            ...
            <div class="info-modal-body">
                ...
                <div class="viz-section">
                    ...
                    <p>The heatmap provides a <strong>detailed breakdown by exam type and year</strong>, using color intensity to represent performance levels. Darker greens indicate higher passing rates, while reds indicate lower rates.</p>
                    <p><strong>How to read:</strong> Scan for color patterns - consistent dark green indicates strong programs, while red cells may highlight areas needing attention.</p>
                    <div class="viz-tags">
                ...
                <div class="data-source-box">
                    ...
                    <p>All visualizations are generated from <strong>anonymous board examination records</strong> stored in the LSPU Board Performance System database. Data includes examination results from <?php echo $year_range; ?>, covering all licensure examinations under the College of Engineering. Personal identifying information is not displayed to protect student privacy. Data is updated as new examination results become available.</p>
                </div>
            </div>
        </div>
    </div>
